...
function citeDoiOutputFormat() {
  ...
  for ($i = null; $i < 10; $i++) {
    if (strpos($p["author$i"][0], ', ')) {
      ...
      print_r($au);
      $p["last$i"][0] = $au[0];
      $p["first$i"][0] = preg_replace("~(\w)\w*\.? ?~", "$1.", $au[1]);
      unset($p['author' . ($i)]);
    } else if (is("first$i")) {
      $p["first$i"][0] = preg_replace("~(\w)\w*\.? ?~", "$1.", $p["first$i"][0]);
     }
  }
  if ($p['pages'][0]) {
    ...
    // I could probably copy it from a similar function elsewhere - if I can find it! //
  }
}
...
function findMoreAuthors($doi, $a1, $pages) {
  ...
  $stopRegexp = "[\n\(:]|\bAff"; // Not used currently - aff may not be necessary.
	...
	echo "\n\n\n ======Looking for more authors @ doi landing page====\n";
  ...
  $source = preg_replace(
    ...
    // Pages
		if (preg_match("~^[\d\w]+$~", trim($pages), $page)) {
