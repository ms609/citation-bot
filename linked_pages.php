...
if (empty($pages_in_category)) {
  ...
  if (count($pages_in_category) > MAX_PAGES) {
    report_warning('Number of links is huge (' . (string) count($pages_in_category) . ')  Cancelling run (use less than ' . (string)(MAX_PAGES) . ').  Listen to Obi-Wan Kenobi:  You want to go home and rethink your life.');
    exit("\n </pre></body></html>");
  }
