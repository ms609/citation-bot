...
define("siciRegExp", "~(\d{4}-\d{4})\((\d{4})(\d\d)?(\d\d)?\)(\d+):?([+\d]*)[<\[](\d+)::?\w{6}[>\]]2\.0\.CO;2~");
...
function jstorData($jid){
  ...
  $xml = @simplexml_load_file($jurl);
    ...
    $result['month'] = (string) $xml->{'issue-meta'}->numerations->{'pub-date'}->month;
    $result['day'] = (string) $xml->{'issue-meta'}->numerations->{'pub-date'}->day;
    $result['volume'] = (string) $xml->{'issue-meta'}->numerations->{'string-volume'};
    $result['issue'] = (string) $xml->{'issue-meta'}->numerations->{'string-issue'};
    $result['pages'] = (string) $xml->{'issue-meta'}->pagerange;
    ...
    foreach ($xml->front->{'article-meta'}->{'contrib-group'}->contrib as $author) {
      ...
      if ($author->name->xref) {
        ...
        $result['first' . ($i>1?$i:"")] = (string) $author->name->xref->{'given-names'};
      } else {
        $result['last' . ($i>1?$i:"")] = (string) $author->name->surname;
        $result['first' . ($i>1?$i:"")] = (string) $author->name->{'given-names'};
      }
    }
  } else echo "Error loading JSTOR file from JSTOR ID $jid!<br>";
