...
final class Template
{
 ...
 public static $all_templates = []; // List of all the Template() on the Page() including this one.  Can only be set by the page class after all templates are made
 public static $date_style = DATES_WHATEVER;
 public static $name_list_style = NAME_LIST_STYLE_DEFAULT;
 /** @psalm-suppress PropertyNotSetInConstructor */
 private $rawtext; // Must start out as unset
 public $last_searched_doi = '';
 private $example_param = '';
 private $name = '';
 /** @var array<Parameter> $param */
 private $param = [];
 /** @var array<string> $initial_param */
 private $initial_param = [];
 /** @var array<string> $initial_author_params */
 private $initial_author_params = [];
 private $initial_name = '';
 private $doi_valid = false;
 private $had_initial_editor = false;
 private $had_initial_publisher = false;
 private $mod_dashes = false;
 private $mod_names = false;
 private $no_initial_doi = false;
 ...
 private $this_array = []; // Unset after using to avoid pointer loop that makes garbage collection harder
 ...
 public function parse_text(string $text): void
 {
  ...
  if (strtolower($trim_name) === 'cite article') {
   ...
   if (!$this->blank_other_than_comments('journal')) {
    ...
    !$this->blank_other_than_comments(['encyclopedia', 'encyclopaedia'])
   ) {
  ...
  if (
   ...
   if (
    ($this->get('title') === "Wayback Machine" ||
     $this->get('title') === "Internet Archive Wayback Machine") &&
  ...
  if ($this->should_be_processed()) {
   ...
   if (
    ...
    (!$this->blank(['pmc', 'pmid', 'doi', 'jstor']) ||
     (stripos($this->get('journal') . $this->get('title'), 'arxiv') !== false &&
    ...
    if (
     ...
     // IEEE/ACM/etc "book"
     $data_to_check =
      $the_title . $the_journal . $the_chapter . $this->get('series');
     if (
      stripos($data_to_check, 'IEEE Standard for') !== false &&
      $this->blank('journal')
     ) {
      // Do nothing
     } elseif (
      stripos($data_to_check, 'SIGCOMM Computer Communication Review') !== false
     ) {
      ...
      stripos($the_journal, 'Visual Languages and Human-Centric Computing') ===
       false &&
      stripos(
       $the_journal,
    ...
    if (
     (stripos($the_journal, 'arXiv:') === 0 || $the_journal === 'arXiv') &&
     !$this->blank(ARXIV_ALIASES)
    ) {
     $this->forget('journal');
     $the_journal = '';
    ...
    if (
     $the_title !== '' &&
     stripos(
      str_replace('CITATION_BOT_PLACEHOLDER_TEMPLATE', '', $the_title),
      'CITATION'
     ) === false
    ) {
    ...
    if ($bad_data) {
     ...
     if ($this->has('CITATION_BOT_PLACEHOLDER_title')) {
      if ($this->has('title')) {
       ...
       $older = str_replace(
        [".", ",", ":", ";", "?", "!", " ", "-", "'", '"'],
        '',
        mb_strtolower($this->get('CITATION_BOT_PLACEHOLDER_title'))
       );
       if ($newer !== $older && strpos($older, $newer) === 0) {
        ...
        $this->get('title') === $this->get('CITATION_BOT_PLACEHOLDER_title')
       ) {
     ...
     if ($this->has('CITATION_BOT_PLACEHOLDER_chapter')) {
      if ($this->has('chapter')) {
       ...
       $older = str_replace(
        [".", ",", ":", ";", "?", "!", " ", "-", "'", '"'],
        '',
        mb_strtolower($this->get('CITATION_BOT_PLACEHOLDER_chapter'))
       );
       if ($newer !== $older && strpos($older, $newer) === 0) {
        ...
        $this->get('chapter') === $this->get('CITATION_BOT_PLACEHOLDER_chapter')
       ) {
     ...
     if ($this->has('CITATION_BOT_PLACEHOLDER_issue')) {
      if (
       $this->has('issue') &&
       $this->get('issue') !== $this->get('CITATION_BOT_PLACEHOLDER_issue')
      ) {
     ...
     if ($this->has('CITATION_BOT_PLACEHOLDER_volume')) {
      if (
       $this->has('volume') &&
       $this->get('volume') !== $this->get('CITATION_BOT_PLACEHOLDER_volume')
      ) {
     ...
     if ($this->has('CITATION_BOT_PLACEHOLDER_year')) {
      if (
       $this->has('year') &&
       $this->get('year') !== $this->get('CITATION_BOT_PLACEHOLDER_year')
      ) {
       $this->move_and_forget('CITATION_BOT_PLACEHOLDER_year');
      } elseif (
       $this->has('date') &&
       $this->get('date') !== $this->get('CITATION_BOT_PLACEHOLDER_year')
      ) {
       $this->move_and_forget('CITATION_BOT_PLACEHOLDER_year');
      } elseif (
       $this->has('date') &&
   ...
   if ($this->wikiname() === 'cite document') {
    ...
    foreach (
     ['archive-url', 'archiveurl', 'archivedate', 'archive-date', 'url-status']
     as $thing
    ) {
     if ($this->blank($thing)) {
      $this->forget($thing);
 ...
 public function incomplete(): bool
 {
  ...
  $possible_extra_authors =
   $this->get('author') . $this->get('authors') . $this->get('vauthors');
  if (
  ...
  if (
   ...
   (preg_match('~^1[^0-9]~', $this->get('pages') . $this->get('page') . '-') &&
    ($this->blank('year') || 2 > (int) date("Y") - (int) $this->get('year'))) // It claims to be on page one
  ) {
   return true;
  }
  if ($this->display_authors() >= $this->number_of_authors()) {
 ...
 public function add_if_new(
  ...
  if (
   in_array($low_value, ['null', 'n/a', 'undefined', '0 0', '(:none)'], true)
  ) {
  ...
  if (
   mb_stripos($this->get($param_name), 'CITATION_BOT_PLACEHOLDER_COMMENT') !==
   false
  ) {
  ...
  switch ($param_name) {
   ...
   case "author99":
    if (
     $this->blank(
      array_merge(COAUTHOR_ALIASES, ["last$auNo", "author$auNo"])
     ) &&
   ...
   case "first99":
    if (
     $this->blank(
      array_merge(COAUTHOR_ALIASES, [$param_name, "author" . $auNo])
     ) &&
   ...
   case "year":
    ...
    if (
     ($this->blank('date') ||
      in_array(
       trim(strtolower($this->get_without_comments_and_placeholders('date'))),
       IN_PRESS_ALIASES,
       true
      )) &&
     ($this->blank('year') ||
      in_array(
       trim(strtolower($this->get_without_comments_and_placeholders('year'))),
       IN_PRESS_ALIASES,
       true
      ))
    ) {
   ...
   case 'issn':
    if (
     $this->blank(["journal", "periodical", "work", $param_name]) &&
   ...
   case 'magazine':
    ...
    if (
     !$this->blank(array_merge(['agency', 'publisher'], WORK_ALIASES)) &&
    ...
    if (
     ...
     if (
      ...
      if ($this->blank('work')) {
       ...
       if (
        stripos($this->get('publisher'), 'bbc') !== false &&
     ...
     if (
      ($param_name === 'newspaper' || $param_name === 'journal') &&
      $this->has('publisher') &&
      str_equivalent($this->get('publisher'), $value) &&
      $this->blank('website')
     ) {
   ...
   case 'title':
    ...
    if (
     ...
     foreach (
      ['encyclopedia', 'encyclopaedia', 'work', 'dictionary', 'journal']
      as $worky
     ) {
   ...
   case "pages":
    ...
    if (
     ...
     ($all_page_values === '1' ||
      substr(str_replace($en_dash, $en_dash_X, $all_page_values), 0, 2) ===
       '1X') && // Old is 1-
   ...
   case 'class':
    if (
     $this->blank($param_name) &&
     strpos($this->get('eprint') . $this->get('arxiv'), '/') === false
    ) {
   ...
   case 'isbn':
    if (
     in_array(
      $value,
      ['9780918678072', '978-0-918678-07-2', '0918678072', '0-918678-07-2'],
      true
     )
    ) {
     return false;
 ...
 public function validate_and_add(
  ...
  if (
   ...
   in_array(mb_strtolower($forename . ' ' . $author), BAD_AUTHORS, true) ===
    false &&
    ...
    foreach ($Items as $item) {
     if ((string) $item->attributes()->Name === 'Title') {
      ...
      foreach (THINGS_THAT_ARE_TITLES as $possible) {
       if (
        $this->has($possible) &&
        titles_are_similar($this->get($possible), $new_title)
       ) {
    ...
    if ($data = $this->get_without_comments_and_placeholders('title')) {
     ...
     foreach ($data_array as $val) {
      if (
       ...
       $query .=
        " AND (" . str_replace("%E2%80%93", "-", urlencode($val)) . "[$key])";
      }
     }
    }
   } elseif ($term === "page") {
 ...
 public function expand_by_adsabs(): void
 {
  ...
  if ($result->numFound !== 1 && $this->has('title')) {
   ...
   if (
    !$this->blank(
     array_merge(['doi', 'pmc', 'pmid', 'eprint', 'arxiv'], ALL_URL_TYPES)
    ) &&
    $this->blank(['issn', 'journal', 'volume', 'issue', 'number']) &&
    mb_strtolower($record->title[0]) !==
     mb_strtolower($this->get_without_comments_and_placeholders('title'))
   ) {
  ...
  if (
   ...
   $result = query_adsabs(
    ($this->has('journal')
     ...
     ($this->has('volume')
      ? "&fq=volume:" . urlencode('"' . $this->get('volume') . '"')
      : '') .
   ...
   if (
    ...
    strpos(
     mb_strtolower(safe_preg_replace($journal_fuzzyer, "", $journal)),
  ...
  if ($result->numFound === 1) {
   ...
   if ($this->blank('bibcode')) {
    ...
    stripos($this->get('bibcode'), 'CITATION_BOT_PLACEHOLDER') === false
   ) {
    report_info(
     "Updating " .
 ...
 public function expand_by_RIS(string &$dat, bool $add_url): void
 {
  ...
  if (preg_match('~(?:T[I1]).*-(.*)$~m', $dat, $match)) {
   if (
    in_array(strtolower(trim($match[1])), BAD_ACCEPTED_MANUSCRIPT_TITLES, true)
   ) {
    return;
   }
  }
  ...
  foreach ($ris as $ris_line) {
   ...
   if (trim($ris_part[0]) === "TY") {
    if (
     in_array(
      trim($ris_part[1]),
      ['CHAP', 'BOOK', 'EBOOK', 'ECHAP', 'EDBOOK', 'DICT', 'ENCYC', 'GOVDOC'],
      true
     )
    ) {
 ...
 private function get_semanticscholar_url(string $doi): void
 {
  ...
  $url =
   'https://api.semanticscholar.org/v1/paper/' . doi_encode(urldecode($doi));
  curl_setopt($ch, CURLOPT_URL, $url);
 ...
 public function get_unpaywall_url(string $doi): string
 {
  ...
  if (
   in_array($doi, ['10.4135/9781529742343', '10.1017/9781108859745'], true)
  ) {
   return 'wrong';
  } // TODO - maybe all ISBN
  set_time_limit(120);
  ...
  if ($json) {
   ...
   if ($oa !== false && isset($oa->best_oa_location)) {
    ...
    if (
     ...
     $oa->journal_name === "Cochrane Database of Systematic Reviews"
    ) {
     report_warning(
 ...
 public function clean_google_books(): void
 {
  foreach (ALL_URL_TYPES as $url_type) {
   if ($this->has($url_type)) {
    ...
    if (
     preg_match('~^https?://www\.google\.[^/]+/books\?(.+)$~', $url, $matches)
    ) {
     $url = 'https://books.google.com/books?' . $matches[1];
    }
    if (
     preg_match('~^https?://books\.google\.[^/\?]+\?id=(.+)$~', $url, $matches)
    ) {
     $url = 'https://books.google.com/books?id=' . $matches[1];
    }
    if (
    ...
    if (
     ...
     $url =
      'https://books.google.com/books?id=' . $matches[1] . '&' . $matches[2];
    }
    if (
     preg_match('~^https?://books\.google\..*id\&\#61\;.*$~', $url, $matches)
    ) {
     $url = str_replace('&#61;', '=', $url);
    }
    if (
 ...
 private function expand_by_google_books_inner(
  ...
  if ($url_type) {
   ...
   if ($isbn) {
    ...
    /** @psalm-taint-escape ssrf */
    $google_book_url =
     'https://www.google.com/search?tbo=p&tbm=bks&q=isbn:' . $isbn;
 ...
 private function google_book_details(string $gid): void
 {
  ...
  if ($xml->dc___title[1]) {
   ...
   $this->add_if_new(
    'title',
    wikify_external_text(str_replace("___", ":", (string) $xml->title))
   );
  }
  $isbn = '';
  ...
  if (
   ...
   foreach ($xml->dc___creator as $author) {
    if (
     strtolower(str_replace("___", ":", (string) $author)) === "gale group"
    ) {
     break;
    }
 ...
 private function use_unnamed_params(): void
 {
  ...
  for ($i = 0; $i < $n_dup_params; $i++) {
   ...
   if ($is_same) {
    ...
    report_modification(
     "Marking duplicate parameter: " . echoable($this->param[$the_dup]->param)
    );
   }
  }
  ...
  foreach ($this->param as &$p) {
     ...
     switch ($endnote_linetype) {
      ...
      case "@": // ISSN / ISBN
       if (preg_match("~@\s*([\d\-]{9,}[\dxX])~", $endnote_line, $matches)) {
        ...
        preg_match("~@\s*(\d{4}\-?\d{3}[\dxX])~", $endnote_line, $matches)
       ) {
        $endnote_datum = $matches[1];
   ...
   if (
    $shortest < 3 &&
    strlen($test_dat) > 0 &&
    (float) similar_text($closest, $test_dat) / (float) strlen($test_dat) >
     0.4 &&
   ...
   for ($i = 0; $i < count($matches[1]); $i++) {
    ...
    switch ($subtemplate_name) {
     case "arxiv":
      if ($subtemplate->get('id')) {
       $archive_parameter = trim(
        $subtemplate->get('archive') ? $subtemplate->get('archive') . '/' : ''
       );
     ...
     case "biorxiv":
      ...
      if (
       $subtemplate_name === 'ol' &&
       stripos($subtemplate->parsed_text(), "ia:") !== false
      ) {
       report_info("{{OL}} ia: parameter not supported: cannot convert.");
       break;
      }
      if (
       ($subtemplate_name === 'jstor' && $subtemplate->has('sici')) ||
     ...
     default:
      report_minor_error(
       "No match found for subtemplate type: " . echoable($subtemplate_name)
      ); // @codeCoverageIgnore
    }
   }
  }
 ...
 private function correct_param_spelling(): void
 {
  ...
  foreach ($this->param as $p) {
   if (
    strlen($p->param) > 0 &&
    !(
     in_array(preg_replace('~\d+~', '#', $p->param), $parameter_list, true) ||
     in_array($p->param, $parameter_list, true)
    ...
    if (trim($p->val) === '') {
     if (stripos($p->param, 'DUPLICATE_') === 0) {
      ...
      report_forget(
       "Dropping empty unrecognized parameter " . echoable($p->param) . " "
      );
     }
    ...
    if (stripos($p->param, 'DUPLICATE_') === 0) {
     report_modification(
      "Left-over duplicate parameter " . echoable($p->param) . " "
     );
     continue;
    } else {
    ...
    if (in_array($p->param, $parameter_dead, true)) {
     ...
     $similarity =
      (float) similar_text($p->param, $closest) / (float) strlen($p->param);
     if ($similarity > 0.6) {
      bot_debug_log("levenshtein replaced " . $p->param . " with " . $closest);
      $p->param = $closest;
      report_inline(
 ...
 public function tidy_parameter(string $param): void
 {
  ...
  if (
   mb_stripos($this->get($param), 'CITATION_BOT_PLACEHOLDER_COMMENT') !==
    false &&
   $param !== 'ref'
  ) {
  ...
  if ($this->has($param)) {
   if (
    ...
    $this->set(
     $param,
     safe_preg_replace('~&#x2013;~u', '&ndash;', $this->get($param))
    );
    $this->set(
     $param,
     safe_preg_replace('~&#x2014;~u', '&mdash;', $this->get($param))
    );
    $this->set(
     $param,
     safe_preg_replace('~&#x00026;~u', '&', $this->get($param))
    ...
    $this->set(
     $param,
     safe_preg_replace('~(?<!\&)&[Aa]pos;(?!&)~u', "'", $this->get($param))
    ); // $apos;
    $this->set(
     $param,
     safe_preg_replace('~(?<!\&)&[Aa]mp;(?!&)~u', '&', $this->get($param))
    ...
    if (
     ...
     if (
      $matches[1] !== $matches[3] ||
      ($param !== 'title' && $param !== 'chapter' && $param !== 'publisher')
     ) {
   ...
   if (
    ...
    $this->set(
     $param,
     safe_preg_replace("~^\xE2\x80\x8B~", " ", $this->get($param))
    ); // Zero-width at start
    $this->set(
     $param,
     safe_preg_replace("~\xE2\x80\x8B$~", " ", $this->get($param))
  ...
  if (
   in_array(strtolower($param), ['series', 'journal', 'newspaper'], true) &&
   $this->has($param)
  ) {
  ...
  if (!preg_match('~^(\D+)(\d*)(\D*)$~', $param, $pmatch)) {
   ...
   switch ($pmatch[1]) {
    ...
    case 'author':
     ...
     if (
      $this->blank('agency') &&
      in_array(strtolower($the_author), ['associated press', 'reuters'], true)
     ) {
     ...
     if (!$this->initial_author_params) {
      if ($pmatch[2]) {
       ...
       if (preg_match($translator_regexp, trim($this->get($param)), $match)) {
        ...
        $this->set(
         $param,
         trim(preg_replace($translator_regexp, "", $this->get($param)))
        );
       }
      }
     }
     if ($pmatch[2] && $pmatch[1] === 'last') {
      ...
      if (
       ...
       if (preg_match(REGEXP_PLAIN_WIKILINK_ONLY, $the_author, $matches)) {
        ...
        preg_match(REGEXP_PIPED_WIKILINK_ONLY, $the_author, $matches)
       ) {
        $this->set($param, $matches[2]);
        $this->add_if_new('author' . $pmatch[2] . '-link', $matches[1]);
        $did_something = true;
       ...
       if (
        $did_something &&
        strpos($this->get('first' . $pmatch[2]), '[') !== false
       ) {
    ...
    case 'chapter':
     if ($this->has('chapter')) {
      ...
      if (
       ...
       strpos($this->get('url') . $this->get('chapter-url'), 'anu.edu.au') !==
        false
      ) {
       $this->forget('chapter');
       return;
    ...
    case 'class':
     if ($this->blank('class')) {
      if (
       ...
       !$this->blank(['doi', 'pmid', 'pmc', 'journal', 'series', 'isbn'])
      ) {
       $this->forget('class');
      }
     }
    ...
    case 'deadurl':
     ...
     if (
      in_array(
       $the_data,
       ['y', 'yes', 'dead', 'si', 'sì', 'ja', 'evet', 'ei tööta'],
       true
      )
     ) {
    ...
    case 'doi':
     ...
     if (
      !doi_works($doi) &&
      strpos($doi, '10.1111/j.1475-4983.' . $this->year()) === 0
     ) {
     ...
     if (!doi_works($doi)) {
      if (
       preg_match('~^10.1093\/oi\/authority\.\d{10,}$~', $doi) &&
       preg_match(
     ...
     if (!preg_match(REGEXP_DOI_ISSN_ONLY, $doi) && doi_works($doi)) {
      if (
       !in_array(strtolower($doi), NON_JOURNAL_DOIS, true) &&
       ...
       $the_journal =
        $this->get('journal') . $this->get('work') . $this->get('periodical');
       if (
        str_replace(NON_JOURNALS, '', $the_journal) === $the_journal &&
        !$this->blank(WORK_ALIASES)
       ) {
        $this->change_name_to('cite journal', false);
       }
     ...
     if (
      ...
      $this->add_if_new(
       'jstor',
       substr($this->get_without_comments_and_placeholders('doi'), 8)
      );
     }
     if ($this->wikiname() === 'cite arxiv') {
      $this->change_name_to('cite journal');
    ...
    case 'edition':
     ...
     $this->set(
      $param,
      preg_replace("~\s+ed(ition)?\.?\s*$~i", "", $this->get($param))
     );
    ...
    case 'periodical':
     ...
     if (
      ...
      $this->set(
       $param,
       preg_replace(REGEXP_PLAIN_WIKILINK, "$1", $periodical)
      );
      $this->set(
       $param,
       preg_replace(REGEXP_PIPED_WIKILINK, "$2", $this->get($param))
      );
     }
     $periodical = trim($this->get($param));
     if (substr($periodical, 0, 1) !== "[" && substr($periodical, -1) !== "]") {
      if (strlen($periodical) - mb_strlen($periodical) < 9) {
       ...
       if (
        str_ireplace(OBVIOUS_FOREIGN_WORDS, '', ' ' . $periodical . ' ') ===
         ' ' . $periodical . ' ' &&
      ...
      if (preg_match(REGEXP_PLAIN_WIKILINK_ONLY, $periodical, $matches)) {
       ...
       if (
        str_ireplace(OBVIOUS_FOREIGN_WORDS, '', ' ' . $linked_text . ' ') ===
         ' ' . $linked_text . ' ' &&
     ...
     if (
      preg_match(REGEXP_PLAIN_WIKILINK, $the_param, $matches) ||
    ...
    case 'publisher':
     ...
     if (
      ...
      stripos($this->get('title'), 'Magazines for Libraries') === false
     ) {
      $this->forget($param);
      if ($this->blank('via')) {
       $this_big_url =
     ...
     if (
      ...
      $this->has('title') &&
      !$this->blank(['pmc', 'pmid']) &&
      (strpos($publisher, 'national center for biotechnology information') !==
       false ||
     ...
     if (
      stripos($this->get('url'), 'developers.google.com') !== false &&
      stripos($publisher, 'google') !== false
     ) {
     ...
     if (
      stripos($this->get('url'), 'support.google.com') !== false &&
      stripos($publisher, 'google') !== false
     ) {
      $this->set($param, 'Google Inc.'); // Case when Google actually IS a publisher
      return;
     ...
     if (
      mb_strtolower(str_replace(['[', ' ', ']'], '', $publisher)) ===
      'amazon.com'
     ) {
      $all_urls = '';
     ...
     if ($this->blank(WORK_ALIASES)) {
      ...
      if (
       ...
       if ($webby === $pubby) {
        if (
         stripos($this->get('website'), 'www') === 0 ||
     ...
     if (
      ...
      $pubby = str_replace(
       ['the ', ' company', ' digital archive', ' communications llc'],
       '',
       $publisher
      );
      foreach (WORK_ALIASES as $work) {
     ...
     if ($publisher === 'the times digital archive') {
      foreach (WORK_ALIASES as $work) {
       if (
        stripos($this->get($work), 'the times') !== false ||
     ...
     if (
      ...
      foreach (WORK_ALIASES as $work) {
       if (
        stripos($this->get($work), 'new york times') !== false ||
     ...
     if (
      ...
      foreach (WORK_ALIASES as $work) {
       if (
        stripos($this->get($work), 'The Guardian') !== false ||
     ...
     if (
      ...
      foreach (WORK_ALIASES as $work) {
       if (
        stripos($this->get($work), 'the economist') !== false ||
     ...
     if (
      ...
      foreach (WORK_ALIASES as $work) {
       if (
        stripos($this->get($work), 'mercurynews.com') !== false ||
     ...
     if (
      ...
      foreach (WORK_ALIASES as $work) {
       if (
        stripos($this->get($work), 'sandiegouniontribune.com') !== false ||
     ...
     if (
      ...
      foreach (WORK_ALIASES as $work) {
       ...
       if ($this->blank('agency')) {
        if (
         stripos($this->get($work), 'AFX News') !== false ||
     ...
     if (
      ...
      foreach (WORK_ALIASES as $work) {
       if (
        stripos($this->get($work), 'latimes') !== false ||
    ...
    case 'title':
     ...
     if (
      $this->wikiname() === 'cite book' &&
      $this->blank('edition') &&
      preg_match('~^(.+)\(([^\(\)]+) edition\)$~i', $title, $matches)
     ) {
     ...
     if (
      ...
      if (preg_match(REGEXP_PLAIN_WIKILINK, $title, $matches)) {
       if (
        strlen($matches[1]) > 0.7 * (float) strlen($title) &&
        $title !== '[[' . $matches[1] . ']]'
       ) {
       ...
       preg_match(REGEXP_PIPED_WIKILINK_ONLY, $title, $matches) &&
       strpos($title, ':') === false
      ) {
     ...
     if (
      $title &&
      str_equivalent($this->get($param), $this->get('encyclopedia'))
     ) {
      $this->forget($param);
     }
     if (
      $title &&
      str_equivalent($this->get($param), $this->get('encyclopaedia'))
     ) {
      $this->forget($param);
     }
     if (
      ...
      preg_match('~^(.+) \(PDF\)$~i', trim($this->get($param)), $match)
     ) {
    ...
    case 'archiveurl':
     ...
     if (
      preg_match('~^(?:web\.|www\.).+$~', $this->get($param), $matches) &&
     ...
     if (
      ...
      if (
       ...
       $this->set(
        $param,
        'https://www.researchgate.net/publication/' . $matches[1]
       );
    ...
    case 'url':
     ...
     if (
      ...
      if (base64_decode($matches[1])) {
       ...
       $this->set(
        $param,
        'https://www.bloomberg.com' . base64_decode($matches[1])
       );
      }
     } elseif (
      preg_match(
      ...
      $this->set(
       $param,
       'https://figshare.com/articles/journal_contribution/' . $matches[1]
      );
     }

     if (
      preg_match("~ebscohost.com.*AN=(\d+)$~", $this->get($param), $matches)
     ) {
      $this->set(
       $param,
       'http://connection.ebscohost.com/c/articles/' . $matches[1]
      );
     }
     if (
      preg_match(
     ...
     if (
      ...
      if (strpos($this->get('doi'), 'forefront') !== false) {
       if (
        strpos(
         $this->get('archiveurl') . $this->get('archive-url'),
         'healthaffairs'
        ) !== false
       ) {
     ...
     if (stripos($this->get($param), 'proxy') !== false) {
      ...
      if (
       ...
       $this->set(
        $param,
        'https://ieeexplore.ieee.org/document/' . $matches[1]
       );
       if (
        $this->has('via') &&
       ...
       $this->set(
        $param,
        'https://www.oxfordhandbooks.com/view/' . $matches[1]
       );
       ...
       $this->set(
        $param,
        'https://www.oxfordartonline.com/view/' . $matches[1]
       );
     ...
     if (
      ...
      stripos($this->get($param), 'ancestryinstitution') === false
     ) {
      $oclc_found = false;
      if (
       preg_match(
       ...
       $this->set(
        $param,
        'https://' . $matches[1] . '.' . $matches[2] . '.com/' . $matches[3]
       );
       $oclc_found = true;
      } elseif (
       preg_match(
       ...
       $this->set(
        $param,
        'https://' . $matches[1] . '.' . $matches[2] . '/' . $matches[3]
       );
       $oclc_found = true;
      } elseif (
       preg_match(
     ...
     if (
      stripos($this->get($param), 'https://access.newspaperarchive.com/') === 0
     ) {
      $this->set(
       $param,
       str_ireplace(
     ...
     if (
      stripos($this->get($param), 'http://access.newspaperarchive.com/') === 0
     ) {
      $this->set(
       $param,
       str_ireplace(
     ...
     if (stripos($this->get($param), 'galegroup') !== false) {
      if (
       ...
       $this->set(
        $param,
        'https://link.galegroup.com' . urldecode($matches[1])
       );
       report_info("Remove proxy from Gale URL");
       if (
        $this->has('via') &&
     ...
     if (stripos($this->get($param), 'proquest') !== false) {
      if (
       ...
       $this->set(
        $param,
        'https://www.proquest.com' . $matches[1] . '/docview/' . $matches[2]
       );
      ...
      if (
       ...
       $this->set(
        $param,
        'https://www.proquest.com/dissertations/docview/' . $matches[1]
       );
      }
      if (
       preg_match(
      ...
      if (
       ...
       if (bot_curl_exec($ch) !== "") {
        ...
        if (
         ...
         preg_match("~^https?://.+\.proquest\.com(?:|/)$~", $redirectedUrl)
        ) {
         $changed = true;
    ...
    case 'work':
     ...
     if (
      $this->has('url') &&
      str_replace(ENCYCLOPEDIA_WEB, '', $this->get('url')) !== $this->get('url')
     ) {
     ...
     if ($this->has_but_maybe_blank($param) && $this->blank($work_becomes)) {
      ...
      if (
       ...
       in_array(
        $this->wikiname(),
        ['cite dictionary', 'cite encyclopedia', 'citation'],
        true
       )
      ) {
    ...
    case 'via': // Should just remove all 'via' with no url, but do not want to make people angry
     if ($this->blank(ALL_URL_TYPES)) {
      ...
      if (
       ...
       stripos($this->get('via'), 'google book') !== false &&
       $this->has('isbn')
      ) {
       $this->forget('via');
      } elseif (
     ...
     foreach (array_merge(['publisher'], WORK_ALIASES) as $others) {
      if ($this->has($others)) {
       if (
        str_equivalent($this->get($others), $this->get('via')) ||
        (stripos($this->get($others), 'bbc') !== false &&
    ...
    case 'pp': # And case 'year': case 'issue':, following from previous
     ...
     if (
      preg_match('~^p\.?p\.?[(?:&nbsp;)\s]*(\d+[–-]?\d+)$~u', $value, $matches)
     ) {
      $value = $matches[1];
      $this->set($param, $value);
     }
     ...
     if (
      ...
      report_modification(
       "Upgrading to en-dash in " . echoable($param) . " parameter"
      );
     ...
     if (strpos($this->get($param), '&') === false) {
      ...
      $this->set(
       $param,
       safe_preg_replace("~^[.,;]*\s*(.*?)\s*[,.]*$~", "$1", $this->get($param))
      ); // Not trailing ;
     }
     ...
     if ($param === 'page' || $param === 'pages') {
      if (
       preg_match("~^pg\.? +(\d+)$~i", $this->get($param), $matches) ||
       preg_match("~^pg\.? +(\d+–\d+)$~iu", $this->get($param), $matches)
      ) {
       $this->set($param, $matches[1]);
      }
     }
    ...
    case 'website':
     ...
     if (
      ...
      stripos(
       $this->get('url') . $this->get('chapter-url') . $this->get('chapterurl'),
       'archive.org'
      ) === false
     ) {
      $this->forget($param);
     ...
     if (
      (strtolower($the_param) === 'www.britishnewspaperarchive.co.uk' ||
       strtolower($the_param) === 'britishnewspaperarchive.co.uk') &&
      $this->blank('via')
     ) {
 ...
 public function final_tidy(): void
 {
  ...
  if ($this->should_be_processed()) {
   ...
   if (
    ...
    if ($crossRef) {
     ...
     if ($possible_issue !== $possible_volume) {
      // They don't match
      if (
       (strpos($possible_issue, '-') > 0 || (int) $possible_issue > 1) &&
   ...
   if (
    $this->wikiname() === 'cite arxiv' &&
    $this->get_without_comments_and_placeholders('doi') &&
    stripos($this->get_without_comments_and_placeholders('doi'), 'arxiv') ===
     false
   ) {
    $this->change_name_to('cite journal');
   }
   ...
   if (
    ...
    if ($this->blank(['chapter', 'isbn'])) {
     ...
     in_array(
      ...
      array_merge(NON_PUBLISHERS, BAD_TITLES, DUBIOUS_JOURNALS, ['amazon.com']),
      true
     )
    ) {
     report_forget(
   ...
   if ($this->wikiname() === 'cite book') {
    foreach (WORK_ALIASES as $worky) {
     ...
     if (
      strtolower($this->get('publisher')) === strtolower($this->get($worky))
     ) {
      $this->forget($worky);
     }
    }
   ...
   if (!empty($this->param)) {
    ...
    foreach ($this->param as $p) {
     ...
     if ($alias !== '' && $this->blank($alias)) {
      if (
       preg_match('~^author(\d+)\-?link$~', $alias, $matches) ||
       preg_match('~^author\-?link(\d+)$~', $alias, $matches)
      ) {
   ...
   if (
    ...
    $this->blank(['archive-url', 'archivedate', 'archiveurl', 'archived-date'])
   ) {
 ...
 public function verify_doi(): bool
 {
  ...
  if ($doi_status === null) {
   report_warning(
    "DOI status unknown.  doi.org failed to respond to: " . doi_link($doi)
   ); // @codeCoverageIgnore
 ...
 public function rename(
  ...
  foreach ($this->param as $p) {
   if ($p->param === $old_param) {
    ...
    if (
     strpos($old_param . $new_param, 'CITATION_BOT_PLACEHOLDER_year') ===
      false &&
     strpos($old_param . $new_param, 'CITATION_BOT_PLACEHOLDER_date') ===
      false &&
     strpos($old_param . $new_param, 'CITATION_BOT_PLACEHOLDER_title') ===
      false &&
     strpos($old_param . $new_param, 'CITATION_BOT_PLACEHOLDER_volume') ===
      false &&
     strpos($old_param . $new_param, 'CITATION_BOT_PLACEHOLDER_issue') ===
      false &&
     strpos($old_param . $new_param, 'CITATION_BOT_PLACEHOLDER_journal') ===
      false
    ) {
     report_modification(
      "Renamed \"" .
 ...
 public function set(string $par, string $val): bool
 {
  ...
  if (
   mb_stripos($this->get($par), 'CITATION_BOT_PLACEHOLDER_COMMENT') !== false &&
   $par !== 'ref'
  ) {
   return false;
  }
   ...
   if ($param === 'volume') {
    if (
     ...
     if ($this->blank(['issue', 'number'])) {
      ...
      stripos($this->get('issue') . $this->get('number'), $data) !== false
     ) {
      $this->forget($param); // Duplicate data
     }
    }
