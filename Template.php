      ...
      if (!$this->blank(['pmc', 'pmid', 'doi', 'jstor']) ||
         (stripos($this->get('journal') . $this->get('title'), 'arxiv') !== FALSE && !$this->blank(ARXIV_ALIASES))) { // Have some good data
          ...
          if ($the_title === 'null' || $the_title === '[No title found]' || $the_title === 'Archived copy' || $the_title === 'JSTOR' ||
              $the_title === 'ShieldSquare Captcha' || $the_title === 'Shibboleth Authentication Request' || $the_title === 'Pubmed' || $the_title === 'Pubmed Central') { // title=none is often because title is "reviewed work....
              $this->rename('title', 'CITATION_BOT_PLACEHOLDER_title');
              $the_title = '';
              $bad_data = TRUE;
          }
