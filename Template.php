...
final class Template {
  ...
  public function get_identifiers_from_url($url_sent = NULL) {
    ...
    if (stripos($url, "jstor.org") !== FALSE) {
      ...
      } elseif (preg_match("~^https?://(?:www\.|)pubmedcentral\.nih\.gov/articlerender.fcgi\?.*\bartid=(\d+)~i", $url, $match) ||
                preg_match("~^https?://(?:www\.|)ncbi\.nlm\.nih\.gov/(?:m/)?pmc/articles/PMC(\d+)~i", $url, $match)) {
