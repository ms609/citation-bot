...
final class Template
{
  ...
  public const REGEXP = [
    ...
    "~\{\{[^\{\}]+\}\}~su",
    "~\{\{(?>[^\{]|\{[^\{])+?\}\}~su",
  ]; // Please see https://stackoverflow.com/questions/1722453/need-to-prevent-php-regex-segfault for discussion of atomic regex
  ...
  public function parse_text(string $text): void
  {
    ...
    if (strpos($trim_name, "_") !== false) {
      ...
      if (
        in_array(
          strtolower($tmp_name),
          array_merge(
            TEMPLATES_WE_PROCESS,
            TEMPLATES_WE_SLIGHTLY_PROCESS,
            TEMPLATES_WE_BARELY_PROCESS,
            TEMPLATES_WE_RENAME
          ),
          true
        )
    ...
    if (strtolower($trim_name) === "cite article") {
      ...
      strtolower($trim_name) === "cite paper" ||
      strtolower($trim_name) === "cite document"
    ) {
      ...
      if (!$this->blank_other_than_comments("journal")) {
        ...
        !$this->blank_other_than_comments("website") &&
        $this->has("url")
      ) {
        ...
        !$this->blank_other_than_comments([
          "encyclopedia",
          "encyclopaedia",
        ])
      ) {
        ...
        strpos($this->get("doi"), "/978-") !== false ||
        strpos($this->get("doi"), "/978019") !== false ||
        strpos($this->get("isbn"), "978-0-19") === 0 ||
        strpos($this->get("isbn"), "978019") === 0
      ) {
        $this->name = $cite_caps . "book" . $spacing[2];
      } elseif (
        !$this->blank_other_than_comments("chapter") ||
        !$this->blank_other_than_comments("isbn")
      ) {
        $this->name = $cite_caps . "book" . $spacing[2];
      } elseif (
        !$this->blank_other_than_comments(["journal", "pmid", "pmc"])
      ) {
        $this->name = $cite_caps . "journal" . $spacing[2];
      } elseif (
        !$this->blank_other_than_comments("publisher") &&
        $this->blank(["url", "citeseerx", "s2cid"])
      ) {
    ...
    if (
      substr($this->wikiname(), 0, 5) === "cite " ||
      $this->wikiname() === "citation"
    ) {
      if (
        preg_match("~< */? *ref *>~i", $this->rawtext) &&
        strpos($this->wikiname(), "cite check") !== 0
      ) {
        report_warning(
          "reference within citation template: most likely unclosed template. " .
            "\n" .
    ...
    foreach ($this->param as $p) {
      ...
      if (
        in_array($p->param, FLATTENED_AUTHOR_PARAMETERS, true) &&
        $p->val
      ) {
  ...
  public function parsed_text(): string
  {
    if ($this->has(strtolower("CITATION_BOT_PLACEHOLDER_BARE_URL"))) {
      if (
        ...
        report_action(
          "Converted Bare reference to template: " .
            echoable(
              trim(
                base64_decode(
                  $this->get(
        ...
        $this->quietly_forget(
          strtolower("CITATION_BOT_PLACEHOLDER_BARE_URL")
        );
      } else {
        return base64_decode(
          $this->get(strtolower("CITATION_BOT_PLACEHOLDER_BARE_URL"))
        );
      }
    }
  ...
  public function prepare(): void
  {
    set_time_limit(120);
    if (
      in_array($this->wikiname(), TEMPLATES_WE_PROCESS) ||
      in_array($this->wikiname(), TEMPLATES_WE_SLIGHTLY_PROCESS, true)
    ) {
      // Clean up bad data
      ...
      if (
        ($this->get("title") === "Wayback Machine" ||
          $this->get("title") ===
            "Internet Archive Wayback Machine") &&
        !$this->blank(["archive-url", "archiveurl"]) &&
        $this->get("url") !== "https://archive.org/web/"
      ) {
        $this->set("title", "");
      }
      if (
        $this->get("last1") === "Published" ||
        $this->get("last1") === "published"
      ) {
        $this->forget("last1");
        if ($this->has("first1")) {
    ...
    if ($this->should_be_processed()) {
      ...
      if (!empty($this->param)) {
        ...
        foreach ($this->param as $key => $p) {
          if (
            @$p->val === "" &&
            in_array(@$p->param, $drop_me_maybe, true)
          ) {
      ...
      if (
        $this->get("last") === "Archive" ||
        $this->get("last1") === "Archive"
      ) {
        if (
      ...
      if (
        stripos($this->rawtext, "citation_bot_placeholder_comment") ===
          false &&
        stripos($this->rawtext, "graph drawing") === false &&
        stripos($this->rawtext, "Lecture Notes in Computer Science") ===
          false &&
        stripos($this->rawtext, "LNCS ") === false &&
        stripos($this->rawtext, " LNCS") === false &&
        (!$this->blank(["pmc", "pmid", "doi", "jstor"]) ||
          (stripos(
            $this->get("journal") . $this->get("title"),
            "arxiv"
          ) !== false &&
            !$this->blank(ARXIV_ALIASES)))
      ) {
        // Have some good data
        $the_title = $this->get("title");
        $the_journal = str_replace(
          ["[", "]"],
          "",
          $this->get("journal")
        );
        ...
        if (
          ...
          stripos($the_title, "Advances in Cryptology") === 0
        ) {
          $the_journal = "";
          $this->forget("journal");
          $bad_data = true;
        ...
        if (
          ...
          in_array(
            $this->wikiname(),
            ["cite journal", "cite web"],
            true
          ) &&
          ($this->has("isbn") ||
            (stripos($the_title, "proceedings") !== false &&
              stripos($the_journal, "proceedings") !== false) ||
            (stripos($the_title, "proc. ") !== false &&
              stripos($the_journal, "proc. ") !== false) ||
            (stripos($the_title, "Conference") !== false &&
              stripos($the_journal, "Conference") !== false) ||
            (stripos($the_title, "Colloquium") !== false &&
              stripos($the_journal, "Colloquium") !== false) ||
            (stripos($the_title, "Symposium") !== false &&
              stripos($the_journal, "Symposium") !== false) ||
            (stripos($the_title, "Extended Abstracts") !== false &&
              stripos($the_journal, "Extended Abstracts") !==
                false) ||
            (stripos($the_title, "Meeting on ") !== false &&
          ...
          $data_to_check =
            ...
            $this->get("series");
          if (
            stripos($data_to_check, "IEEE Standard for") !==
              false &&
            $this->blank("journal")
          ) {
            // Do nothing
            ...
            stripos($data_to_check, "Extended Abstracts") ===
              false &&
            stripos(
              $the_journal,
            ...
            stripos(
              $the_journal,
              "Active and Passive Microwave Remote Sensing for"
            ) === false
          ) {
            ...
            if ($the_journal !== "") {
              $this->rename(
                "journal",
                "CITATION_BOT_PLACEHOLDER_journal"
              );
              $the_journal = "";
            }
            if ($the_title !== "") {
              $this->rename(
                "title",
                "CITATION_BOT_PLACEHOLDER_title"
              );
              $the_title = "";
            }
            if ($the_chapter !== "") {
              $this->rename(
                "chapter",
                "CITATION_BOT_PLACEHOLDER_chapter"
              );
              $the_chapter = "";
            }
            $bad_data = true;
        ...
        if (
          ...
          stripos($the_journal, "IEEE International Conference") !==
            false ||
          stripos($the_journal, "ACM International Symposium") !==
            false ||
          stripos($the_journal, "ACM Symposium") !== false ||
          stripos($the_journal, "Extended Abstracts") !== false ||
          stripos($the_journal, "IEEE International Symposium") !==
            false ||
          stripos(
            $the_journal,
          ...
          stripos($the_journal, "International Conference on ") !==
            false ||
          stripos($the_journal, "ACM International Conference") !==
            false ||
          (stripos($the_journal, "Proceedings of ") !== false &&
            stripos($the_journal, "Conference") !== false) ||
          (stripos($the_journal, "Proc. ") !== false &&
            stripos($the_journal, "Conference") !== false) ||
          (stripos($the_journal, "International") !== false &&
            stripos($the_journal, "Conference") !== false) ||
          (stripos($the_journal, "International") !== false &&
            stripos($the_journal, "Meeting") !== false) ||
          (stripos($the_journal, "International") !== false &&
            stripos($the_journal, "Colloquium") !== false) ||
          stripos($the_journal, "SIGGRAPH") !== false ||
          stripos($the_journal, "Design Automation Conference") !==
            false
        ) {
          $this->rename(
            "journal",
        ...
        if (
          strlen($the_title) > 15 &&
          strpos($the_title, " ") !== false &&
          mb_strtoupper($the_title) === $the_title &&
          strpos($the_title, "CITATION") === false &&
        ...
        if (
          strlen($the_journal) > 15 &&
          strpos($the_journal, " ") !== false &&
          mb_strtoupper($the_journal) === $the_journal &&
          strpos($the_journal, "CITATION") === false &&
        ...
        if (
          strlen($the_chapter) > 15 &&
          strpos($the_chapter, " ") !== false &&
          mb_strtoupper($the_chapter) === $the_chapter &&
          strpos($the_chapter, "CITATION") === false &&
        ...
        if (
          str_i_same($the_journal, "Biochimica et Biophysica Acta") ||
          str_i_same(
            $the_journal,
        ...
        if (
          ...
          stripos(
            str_replace(
              "CITATION_BOT_PLACEHOLDER_TEMPLATE",
              "",
              $the_title
            ),
            "CITATION"
          ...
          if (
            str_i_same($the_title, $the_journal) &&
            str_i_same($the_title, $the_chapter)
          ) {
            ...
            substr($the_title, -20, 20) === "IEEE Xplore Document"
          ) {
            $this->rename(
              "title",
              "CITATION_BOT_PLACEHOLDER_title"
            ...
            preg_match(
              "~.+(?: Volume| Vol\.| V. | Number| No\.| Num\.| Issue ).*\d+.*page.*\d+~i",
              $the_title
            )
          ) {
            $this->rename(
            ...
            preg_match('~^\[No title found\]$~i', $the_title)
          ) {
            $this->rename(
              "title",
              "CITATION_BOT_PLACEHOLDER_title"
        ...
        if ($this->has("coauthors")) {
          if ($this->has("first")) {
            $this->rename(
              "first",
              "CITATION_BOT_PLACEHOLDER_first"
            );
          }
          if ($this->has("last")) {
          ...
          if ($this->has("first1")) {
            $this->rename(
              "first1",
              "CITATION_BOT_PLACEHOLDER_first1"
            );
          }
          if ($this->has("last1")) {
            $this->rename(
              "last1",
              "CITATION_BOT_PLACEHOLDER_last1"
            );
          }
          if ($this->has("author1")) {
            $this->rename(
              "author1",
              "CITATION_BOT_PLACEHOLDER_author1"
            );
          }
          if ($this->has("author")) {
            $this->rename(
              "author",
              "CITATION_BOT_PLACEHOLDER_author"
            );
          }
          $this->rename(
            "coauthors",
            "CITATION_BOT_PLACEHOLDER_coauthors"
          );
        ...
        if ($bad_data) {
          ...
          if (
            $this->blank(["pmid", "pmc", "jstor"]) &&
            ($this->has("eprint") || $this->has("arxiv"))
          ) {
          ...
          if ($this->has("CITATION_BOT_PLACEHOLDER_journal")) {
            if (
              ...
              "[[" . $this->get("journal") . "]]" !==
                $this->get("CITATION_BOT_PLACEHOLDER_journal")
            ) {
              $this->move_and_forget(
                "CITATION_BOT_PLACEHOLDER_journal"
              );
            } else {
              $this->rename(
          ...
          if ($this->has("CITATION_BOT_PLACEHOLDER_title")) {
            if ($this->has("title")) {
              ...
              $older = str_replace(
                ...
                "",
                mb_strtolower(
                  $this->get("CITATION_BOT_PLACEHOLDER_title")
                )
              );
              if (
                $newer !== $older &&
                strpos($older, $newer) === 0
              ) {
                $this->rename(
                  "CITATION_BOT_PLACEHOLDER_title",
                  "title"
                ); // New title lost sub-title
              } elseif (
                str_replace(" ", "", $this->get("title")) ===
                str_replace(
                  [" ", "'"],
                  "",
                  $this->get("CITATION_BOT_PLACEHOLDER_title")
          ...
          if ($this->has("CITATION_BOT_PLACEHOLDER_chapter")) {
            if ($this->has("chapter")) {
              $newer = str_replace(
                ...
                "",
                mb_strtolower($this->get("chapter"))
              );
              $older = str_replace(
                [
                  ".",
              ...
              if (
                ...
                $this->rename(
                  "CITATION_BOT_PLACEHOLDER_chapter",
                  "chapter"
                ); // New chapter lost sub-chapter
              } elseif (
                str_replace(" ", "", $this->get("chapter")) ===
                str_replace(
                  [" ", "'"],
                  "",
                  $this->get(
                ...
                $this->rename(
                  "CITATION_BOT_PLACEHOLDER_chapter",
                  "chapter"
                ); // New chapter lost italics
              } elseif (
                $this->get("chapter") ===
                $this->get("CITATION_BOT_PLACEHOLDER_chapter")
              ) {
                $this->rename(
                  "CITATION_BOT_PLACEHOLDER_chapter",
                  "chapter"
          ...
          if ($this->has("CITATION_BOT_PLACEHOLDER_issue")) {
            if (
              $this->has("issue") &&
              $this->get("issue") !==
                $this->get("CITATION_BOT_PLACEHOLDER_issue")
            ) {
              $this->move_and_forget(
                "CITATION_BOT_PLACEHOLDER_issue"
              );
            } else {
              $this->rename(
                "CITATION_BOT_PLACEHOLDER_issue",
          ...
          if ($this->has("CITATION_BOT_PLACEHOLDER_volume")) {
            if (
              $this->has("volume") &&
              $this->get("volume") !==
                $this->get("CITATION_BOT_PLACEHOLDER_volume")
            ) {
              $this->move_and_forget(
                "CITATION_BOT_PLACEHOLDER_volume"
              );
            } else {
              $this->rename(
                "CITATION_BOT_PLACEHOLDER_volume",
          ...
          if ($this->has("CITATION_BOT_PLACEHOLDER_page")) {
            if (
              ($this->has("page") || $this->has("pages")) &&
              $this->get("page") . $this->get("pages") !==
                $this->get("CITATION_BOT_PLACEHOLDER_page")
            ) {
              $this->move_and_forget(
                "CITATION_BOT_PLACEHOLDER_page"
              );
            } else {
              $this->rename(
                "CITATION_BOT_PLACEHOLDER_page",
          ...
          if ($this->has("CITATION_BOT_PLACEHOLDER_pages")) {
            if (
              ($this->has("page") || $this->has("pages")) &&
              $this->get("page") . $this->get("pages") !==
                $this->get("CITATION_BOT_PLACEHOLDER_pages")
            ) {
              $this->move_and_forget(
                "CITATION_BOT_PLACEHOLDER_pages"
              );
            } else {
              $this->rename(
                "CITATION_BOT_PLACEHOLDER_pages",
          ...
          if ($this->has("CITATION_BOT_PLACEHOLDER_year")) {
            if (
              $this->has("year") &&
              $this->get("year") !==
                $this->get("CITATION_BOT_PLACEHOLDER_year")
            ) {
              $this->move_and_forget(
                "CITATION_BOT_PLACEHOLDER_year"
              );
            } elseif (
              $this->has("date") &&
              $this->get("date") !==
          ...
          if ($this->has("CITATION_BOT_PLACEHOLDER_coauthors")) {
            if ($this->has("last1") || $this->has("author1")) {
              ...
              if (
                $this->has("CITATION_BOT_PLACEHOLDER_author1")
              ) {
                $this->rename(
                  "CITATION_BOT_PLACEHOLDER_author1",
                  "author1"
      ...
      foreach (
        ...
        if ($this->has($possible)) {
          ...
          if (
            ...
            $this->rename(
              $possible,
              "CITATION_BOT_PLACEHOLDER_possible"
            );
      ...
      $this->wikiname() === "cite magazine" &&
      $this->blank("magazine") &&
      $this->has_but_maybe_blank("work")
    ) {
  ...
  public function fix_rogue_etal(): void
  {
    if ($this->blank(DISPLAY_AUTHORS)) {
      $i = 2;
      while (
        !$this->blank(["author" . (string) $i, "last" . (string) $i])
      ) {
        $i = $i + 1;
      }
      $i = $i - 1;
  ...
  public function incomplete(): bool
  {
    ...
    if (
      ...
      ($this->wikiname() === "citation" && $this->has("isbn"))
    ) {
      // Assume book
    ...
    if (
      ...
      preg_match(
        "~no.+no|n/a|in press|none~",
        $this->get("pages") . $this->get("page") . $this->get("at")
      ) ||
      (preg_match(
        "~^1[^0-9]~",
        $this->get("pages") . $this->get("page") . "-"
      ) &&
        ($this->blank("year") ||
          2 > (int) date("Y") - (int) $this->get("year"))) // It claims to be on page one
    ) {
      return true;
    }
    if ($this->display_authors() >= $this->number_of_authors()) {
  ...
  public function blank($param): bool
  {
    ...
    foreach ($this->param as $p) {
      if (
        in_array($p->param, $param, true) &&
        trim($p->val) !== "" &&
        !str_i_same("Epub ahead of print", $p->val)
      ) {
        return false;
      }
    }
    ...
    foreach ($this->param as $p) {
      if (in_array($p->param, $param, true)) {
        ...
        if (
          ...
          $value = trim(
            preg_replace(
              "~^# # # CITATION_BOT_PLACEHOLDER_COMMENT \d+ # # #~i",
              "",
              $value
            )
          );
  ...
  public function add_if_new(
    ...
    $value = safe_preg_replace(
      "~[\x{2000}-\x{200B}\x{00A0}\x{202F}\x{205F}\x{3000}]~u",
      " ",
      $value
    ); // Non-standard spaces
    ...
    if (array_key_exists($param_name, COMMON_MISTAKES)) {
      ...
      report_minor_error(
        "Attempted to add invalid parameter: " . echoable($param_name)
      ); // @codeCoverageIgnore
    }
    ...
    if (
      $this->initial_author_params &&
      in_array($param_name, FLATTENED_AUTHOR_PARAMETERS, true)
    ) {
      return false;
    }
    ...
    if ($param_name !== "s2cid") {
      if (
        strpos($param_name, "last") === 0 ||
        strpos($param_name, "first") === 0 ||
        strpos($param_name, "author") === 0
      ) {
        if (
          (int) substr($param_name, -4) > 0 ||
          (int) substr($param_name, -3) > 0 ||
          (int) substr($param_name, -2) > 30
        ) {
          // Stop at 30 authors - or page codes will become cluttered!
          if (
            (bool) $this->get("last29") ||
            (bool) $this->get("author29") ||
            (bool) $this->get("surname29")
          ) {
      ...
      if (strpos($param_name, "editor") === 0) {
        if (
          ...
          if (
            (bool) $this->get("editor29") ||
            (bool) $this->get("editor-last29") ||
            (bool) $this->get("editor29-last")
          ) {
    ...
    switch ($param_name) {
      ...
      case (bool) preg_match('~^editor(\d{1,})$~', $param_name, $match):
        ...
        if (
          $this->blank([
            "editor" . $match[1],
            "editor" . $match[1] . "-last",
            "editor" . $match[1] . "-first",
            "editor-last" . $match[1],
            "editor-first" . $match[1],
          ])
        ) {
      ...
      case (bool) preg_match(
        ...
        if (
          $this->blank([
            "translator" . $match[1],
            "translator" . $match[1] . "-last",
            "translator" . $match[1] . "-first",
          ])
        ) {
          return $this->add($param_name, clean_up_full_names($value));
        }
      ...
      case "authors":
        if ($this->blank(FIRST_AUTHOR_ALIASES)) {
          ...
          if (!empty($au) && substr($param_name, 0, 3) === "aut") {
            $this->add(
              "last" .
                (substr($param_name, -1) === "1" ? "1" : ""),
              clean_up_last_names(format_surname($au[0]))
            );
            return $this->add_if_new(
              "first" .
                (substr($param_name, -1) === "1" ? "1" : ""),
              clean_up_first_names(format_forename(trim($au[1])))
            );
      ...
      case "author99":
        if (
          $this->blank(
            array_merge(COAUTHOR_ALIASES, [
              "last$auNo",
              "author$auNo",
            ])
          ) &&
          strpos(
            $this->get("author") . $this->get("authors"),
            " and "
          ) === false &&
          strpos(
            $this->get("author") . $this->get("authors"),
      ...
      case "date":
        if (
          self::$date_style !== DATES_WHATEVER ||
        ...
        if (
          ($this->blank("date") ||
            in_array(
              trim(
                strtolower(
                  $this->get_without_comments_and_placeholders(
                    "date"
                  )
                )
              ),
      ...
      case "magazine":
        ...
        if (
          preg_match("~Conference Proceedings.*IEEE.*IEEE~", $value)
        ) {
          return false;
        }
        ...
        if (
          in_array(
            strtolower(sanitize_string($value)),
            BAD_TITLES,
            true
          )
        ) {
        ...
        if (in_array(strtolower($value), ARE_MANY_THINGS, true)) {
          if (
            $this->wikiname() === "cite news" &&
            $param_name === "newspaper"
          ) {
        ...
        if (
          in_array(
            strtolower(sanitize_string($this->get("journal"))),
            BAD_TITLES,
            true
          )
        ) {
        ...
        if (
          $this->has("title") &&
          str_equivalent($this->get("title"), $value)
        ) {
          return false;
        } // Messed up already or in database
        if (
          !$this->blank(
            array_merge(["agency", "publisher"], WORK_ALIASES)
          ) &&
          in_array(strtolower($value), DUBIOUS_JOURNALS, true)
        ) {
          return false;
        } // non-journals that are probably same as agency or publisher that come from zotero
        if (
          $this->get($param_name) === "none" ||
          $this->blank([
            "journal",
            "periodical",
            "encyclopedia",
          ...
          if (
            $this->has("series") &&
            str_equivalent($this->get("series"), $value)
          ) {
            return false;
          }
          if ($this->has("work")) {
            if (
              str_equivalent($this->get("work"), $value) &&
              !in_array(strtolower($value), ARE_MANY_THINGS, true)
            ) {
          ...
          if (
            $param_name === "newspaper" &&
            in_array(strtolower($value), WEB_NEWSPAPERS, true)
          ) {
            if (
              $this->has("publisher") &&
              str_equivalent($this->get("publisher"), $value)
            ) {
              return false;
            }
            if ($this->blank("work")) {
              ...
              if (
                stripos($this->get("publisher"), "bbc") !==
                  false &&
                stripos($value, "bbc") !== false
              ) {
            ...
            report_error(
              "Unreachable code reached in newspaper add"
            ); // @codeCoverageIgnore
          }
          if ($param_name === "newspaper" && $this->has("via")) {
            if (
              stripos($value, "times") !== false &&
              stripos($this->get("via"), "times") !== false
            ) {
              $this->forget("via"); // eliminate via= that matches newspaper mostly
            }
            if (
              stripos($value, " post") !== false &&
              stripos($this->get("via"), "post") !== false
            ) {
              $this->forget("via"); // eliminate via= that matches newspaper mostly
            }
            if (
              stripos($value, " bloomberg") !== false &&
              stripos($this->get("via"), "bloomberg") !== false
            ) {
              $this->forget("via"); // eliminate via= that matches newspaper mostly
            }
          }
      ...
      case "series":
        if ($this->blank($param_name)) {
          ...
          if (
            $this->has("journal") &&
            str_equivalent($this->get("journal"), $value)
          ) {
            return false;
          }
          if (
      ...
      case "section": //  We do not add article/section, but sometimes found floating in a template
        if (
          !$this->blank(["booktitle", "book-title"]) &&
          $this->has("title")
        ) {
          return false;
        }
      ...
      case "title":
        ...
        if (
          ...
          (stripos($this->get($param_name), "EZProxy") !== false &&
            stripos($value, "EZProxy") === false)
        ) {
          foreach (
            [
              "encyclopedia",
      ...
      case "volume":
        if (
          $this->blank($param_name) ||
          str_i_same("in press", $this->get($param_name))
        ) {
          if ($value === "0") {
            return false;
          }
          ...
          if ($value === "1") {
            ...
            if (
              stripos($this->rawtext, "escholarship.org") !==
              false
            ) {
              return false;
            }
          ...
          if (
            preg_match("~^volume[:\s]+0*(.*)~i", $value, $matches)
          ) {
            $value = $matches[1];
          }
          if (
            intval($value) > 1820 &&
            stripos($this->get("doi"), "10.1515/crll") === 0
          ) {
            return false;
          }
          $temp_string = strtolower($this->get("journal"));
          if (
            substr($temp_string, 0, 2) === "[[" &&
            substr($temp_string, -2) === "]]"
          ) {
      ...
      case "number":
        ...
        if (
          ...
          $this->get("issue") . $this->get("number") === "1" &&
          $value !== "1" &&
          $this->blank("volume")
        ) {
      ...
      case "pages":
        ...
        if (
          ...
          mb_stripos($all_page_values, ",") !== false ||
          mb_stripos($all_page_values, "(") !== false ||
          mb_stripos($all_page_values, "CITATION_BOT_PLACEHOLDER") !==
            false
        ) {
        ...
        if (
          ...
          (str_i_same($all_page_values, "no") ||
            str_i_same($all_page_values, "none")) || // Is exactly "no" or "none"
          (strpos(strtolower($all_page_values), "no") !== false &&
            $this->blank("at")) || // "None" or "no" contained within something other than "at"
          (str_replace($en_dash, $en_dash_X, $value) !== $value && // dash in new `pages`
            str_replace($en_dash, $en_dash_X, $pages_value) ===
              $pages_value) || // No dash already // Document with bogus pre-print page ranges
          ($value !== "1" &&
          substr(str_replace($en_dash, $en_dash_X, $value), 0, 2) !==
            "1X" && // New is not 1-
          ($all_page_values === "1" ||
            substr(
              str_replace($en_dash, $en_dash_X, $all_page_values),
              0,
              2
            ) === "1X") && // Old is 1-
            ($this->blank("year") ||
              2 > (int) date("Y") - (int) $this->get("year"))) // Less than two years old
        ) {
          if (
            $param_name === "pages" &&
          ...
          if (
            $all_page_values !== "" &&
            preg_match(
              "~^[a-zA-Z]?[a-zA-Z]?(\d+)[a-zA-Z]?[a-zA-Z]?[-–—‒]+[a-zA-Z]?[a-zA-Z]?(\d+)[a-zA-Z]?[a-zA-Z]?$~u",
              $value,
              $newpagenos
            ) && // Adding a range
            preg_match(
              "~^[a-zA-Z]?[a-zA-Z]?(\d+)[a-zA-Z]?[a-zA-Z]?~u",
              $all_page_values,
              $oldpagenos
            )
          ) {
            ...
            if ($last_page < $first_page) {
              ...
              if ($last_page < 10) {
                $last_page =
                  $last_page + 10 * (int) ($first_page / 10);
              } else {
                $last_page =
                  $last_page +
                  100 * (int) ($first_page / 100);
              }
            }
            if (
              $old_page > $first_page &&
          ...
          if (
            preg_match('~^1[-–]\d+$~u', $value) &&
            preg_match('~^[a-zA-Z1-9]\d{3,}$~', $all_page_values)
          ) {
            return false;
          }
          if ($param_name !== "pages") {
      ...
      case "url":
        ...
        if (
          !$this->blank(
            array_merge([$param_name], TITLE_LINK_ALIASES)
          )
        ) {
          return false;
        }
      ...
      case "class":
        if (
          $this->blank($param_name) &&
          strpos($this->get("eprint") . $this->get("arxiv"), "/") ===
            false
        ) {
      ...
      case "doi":
        ...
        if (preg_match(REGEXP_DOI, $value, $match)) {
          if ($this->blank($param_name)) {
            ...
            !str_i_same($this->get("doi"), $match[0]) &&
            !$this->blank(DOI_BROKEN_ALIASES) &&
            doi_active($match[0])
          ) {
            report_action(
              "Replacing non-working DOI with a working one"
            );
            ...
            report_action(
              "Replacing ResearchGate DOI with publisher's"
            );
            $this->set("doi", $match[0]);
            $this->tidy_parameter("doi");
            return true;
      ...
      case "doi-broken-date":
        ...
        foreach (
          array_diff(DOI_BROKEN_ALIASES, ["doi-broken-date"])
          as $alias
        ) {
          if ($this->blank($alias)) {
            $this->forget($alias);
        ...
        if (
          $existing === false ||
          $existing + 7 * 2592000 < $the_new ||
          2592000 * 7 + $the_new < $existing
        ) {
      ...
      case "pmid":
        ...
        if ($this->blank($param_name)) {
          ...
          $this->expand_by_pubmed(
            $this->blank("pmc") || $this->blank("doi")
          ); //Force = true if missing DOI or PMC
      ...
      case "bibcode":
        if (
          stripos($value, "arxiv") === false &&
          stripos($value, "tmp") === false &&
      ...
      case "asin":
        if ($this->blank($param_name)) {
          if ($this->has("isbn")) {
            ...
            $possible_isbn13 = $this->isbn10Toisbn13(
              $possible_isbn,
              true
            );
            if ($possible_isbn === $possible_isbn13) {
              ...
              return $this->add(
                "isbn",
                $this->isbn10Toisbn13($possible_isbn)
              );
            }
          } else {
            // NOT ISBN
      ...
      case "publisher":
        ...
        if (
          $this->has("via") &&
          str_equivalent($this->get("via"), $value)
        ) {
          $this->rename("via", $param_name);
        }
        if (
  ...
  public function validate_and_add(
    string $author_param,
    string $author,
    string $forename,
    string $check_against,
    bool $add_even_if_existing
  ): void {
    if (
      !$add_even_if_existing &&
      ($this->initial_author_params || $this->had_initial_editor)
    ) {
      return;
    ...
    if (
      ...
      while (
        preg_match(
          '~^(?:rabbi|prof\.|doctor|professor|dr\.)\s([\s\S]+)$~i',
          $forename,
          $match
        )
      ) {
      ...
      if (
        in_array(strtolower($author_ending), PUBLISHER_ENDINGS, true) ||
        ...
        $this->add_if_new(
          $author_param,
          format_author($author . ($forename ? ", $forename" : ""))
        );
      }
    }
  }
  ...
  private function get_doi_from_text(): void
  {
    ...
    if (
      ...
      if (
        $this->has("quote") &&
        strpos($this->get("quote"), $doi) !== false
      ) {
        return;
      }
      if (doi_active($doi)) {
  ...
  public function get_doi_from_crossref(): void
  {
    ...
    if (
      ($data["journal"] || $data["issn"]) &&
      ($data["start_page"] || $data["author"])
    ) {
      ...
      $url =
        ...
        ($data["author"]
          ? "&aulast=" . urlencode($data["author"])
          : "") .
        ($data["start_page"]
          ? "&spage=" . urlencode($data["start_page"])
          : "") .
        ($data["end_page"]
          ? "&epage=" . urlencode($data["end_page"])
          : "") .
        ($data["year"] ? "&date=" . urlencode($data["year"]) : "") .
        ($data["volume"]
          ? "&volume=" . urlencode($data["volume"])
          : "") .
        ($data["issn"]
          ? "&issn=" . urlencode($data["issn"])
          : "&title=" . urlencode($data["journal"])) .
        "&mailto=" .
      ...
      if ((string) $result->attributes()->status === "malformed") {
        report_minor_error(
          "Cannot search CrossRef: " . echoable((string) $result->msg)
        ); // @codeCoverageIgnore
      ...
      if ($this->has("title") && !in_array("doi", $results[2], true)) {
        ...
        foreach ($Items as $item) {
          if ((string) $item->attributes()->Name === "Title") {
            ...
            // @codeCoverageIgnoreStart
            report_inline(
              "Similar matching pubmed title not similar enough. Rejected: " .
                pubmed_link("pmid", $results[0])
            );
            return;
            // @codeCoverageIgnoreEnd
          }
  ...
  private function do_pumbed_query(array $terms): array
  {
    ...
    foreach ($terms as $term) {
      ...
      if ($term === "title") {
        if (
          $data = $this->get_without_comments_and_placeholders(
            "title"
          )
        ) {
          $key = "Title";
          ...
          foreach ($data_array as $val) {
            if (
              ...
              $query .=
                " AND (" .
                str_replace("%E2%80%93", "-", urlencode($val)) .
                "[$key])";
            }
          }
        }
        ...
        if (
          ...
          $query .=
            " AND (" .
            "\"" .
            str_replace(["%E2%80%93", ";"], ["-", "%3B"], $val) .
            "\"" .
            "[$key])"; // PubMed does not like escaped /s in DOIs, but other characters seem problematic.
        }
      } else {
        ...
        if (
          ...
          if (preg_match(REGEXP_PLAIN_WIKILINK, $val, $matches)) {
            ...
            preg_match(REGEXP_PIPED_WIKILINK, $val, $matches)
          ) {
    ...
    if (isset($xml->IdList->Id[0]) && isset($xml->Count)) {
      return [
        (string) $xml->IdList->Id[0],
        (int) (string) $xml->Count,
        $terms,
      ]; // first results; number of results
    } else {
      return ["", 0, []];
    }
  }
  ...
  public function expand_by_adsabs(): void
  {
    ...
    if (
      ...
      stripos($this->get("bibcode"), "arxiv") === false &&
      ($this->has("doi") ||
        AdsAbsControl::get_bib2doi($this->get("bibcode")) === "X")
    ) {
    ...
    if (
      !$this->blank_other_than_comments("bibcode") &&
      stripos($this->get("bibcode"), "tmp") === false &&
      stripos($this->get("bibcode"), "arxiv") === false
    ) {
      return;
    ...
    if (
      ...
      preg_match(
        REGEXP_DOI,
        $this->get_without_comments_and_placeholders("doi"),
        $doi
      )
    ) {
      $result = query_adsabs(
        "identifier:" . urlencode('"' . $doi[0] . '"')
      ); // In DOI we trust
    } elseif ($this->has("eprint")) {
      $result = query_adsabs(
        "identifier:" . urlencode('"' . $this->get("eprint") . '"')
      );
    } elseif ($this->has("arxiv")) {
      $result = query_adsabs(
        "identifier:" . urlencode('"' . $this->get("arxiv") . '"')
      ); // @codeCoverageIgnore
    } else {
    ...
    if ($result->numFound === 0) {
      ...
      if (
        ...
        ($this->wikiname() === "citation" &&
          $this->has("isbn") &&
          $this->has("chapter")) || // "complete" enough for a book
    ...
    if ($result->numFound !== 1 && $this->has("title")) {
      ...
      if (
        titles_are_dissimilar(
          $this->get_without_comments_and_placeholders("title"),
          $record->title[0]
        )
      ) {
    ...
    if (
      $result->numFound !== 1 &&
      ($this->has("journal") || $this->has("issn"))
    ) {
      ...
      $result = query_adsabs(
        ($this->has("journal")
          ? "pub:" . urlencode('"' . remove_brackets($journal) . '"')
          : "&fq=issn:" . urlencode($this->get("issn"))) .
          ($this->year()
            ? "&fq=year:" . urlencode($this->year())
            : "") .
          ($this->has("volume")
            ? "&fq=volume:" .
              urlencode('"' . $this->get("volume") . '"')
            : "") .
      ...
      $journal_fuzzyer =
        "~\([iI]ncorporating.+|\bof\b|\bthe\b|\ba|eedings\b|\W~";
      if (
        strlen($journal_string[0]) &&
        strpos(
          mb_strtolower(
            safe_preg_replace($journal_fuzzyer, "", $journal)
          ),
          mb_strtolower(
            safe_preg_replace(
              $journal_fuzzyer,
    ...
    if ($result->numFound === 1) {
      ...
      if (
        $this->has("title") &&
        titles_are_dissimilar($this->get("title"), $record->title[0]) &&
        !in_array(
          $this->get("title"),
          [
            "Archived copy",
      ...
      if ($this->looksLikeBookReview($record)) {
        ...
        report_info(
          "Suspect that BibCode " .
            bibcode_link((string) $record->bibcode) .
            " is book review. Rejecting."
        );
        return;
      }
      ...
      if ($this->blank("bibcode")) {
        ...
        $this->get("bibcode") !== (string) $record->bibcode &&
        stripos($this->get("bibcode"), "CITATION_BOT_PLACEHOLDER") ===
          false
      ) {
        report_info(
          "Updating " .
            bibcode_link($this->get("bibcode")) .
            " to " .
            bibcode_link((string) $record->bibcode)
        );
  ...
  public function looksLikeBookReview(object $record): bool
  {
    if (
      ...
      if (
        stripos($this->get("url"), "google") !== false &&
        stripos($this->get("url"), "book") !== false
      ) {
        $book_count += 2;
      }
      if (
  ...
  public function expand_by_RIS(string &$dat, bool $add_url): void
  {
    ...
    if (preg_match('~(?:T[I1]).*-(.*)$~m', $dat, $match)) {
      if (
        in_array(
          ...
          BAD_ACCEPTED_MANUSCRIPT_TITLES,
          true
        )
      ) {
        return;
    ...
    foreach ($ris as $ris_line) {
      ...
      if (trim($ris_part[0]) === "TY") {
        ...
        trim($ris_part[0]) === "SP" &&
        (trim($ris_part[1]) === "i" || trim($ris_part[1]) === "1")
      ) {
        $bad_SP = true;
      } elseif (
        trim($ris_part[0]) === "EP" &&
        preg_match('~^\d{3,}$~', trim($ris_part[1]))
      ) {
        $bad_EP = true;
      }
    }
    ...
    foreach ($ris as $ris_line) {
      ...
      switch (trim($ris_part[0])) {
        ...
        case "PY":
          $ris_parameter = "date";
          $ris_part[1] = preg_replace(
            "~([\-\s]+)$~",
            "",
            str_replace("/", "-", $ris_part[1])
          );
          break;
        ...
        default:
          if (isset($ris_part[1])) {
            report_minor_error(
              "Unexpected RIS data type ignored: " .
                echoable(trim($ris_part[0])) .
                " set to " .
                echoable(trim($ris_part[1]))
            ); // @codeCoverageIgnore
          }
      }
      unset($ris_part[0]);
  ...
  public function expand_by_pubmed(bool $force = false): void
  {
    ...
    if ($pm = $this->get("pmid")) {
      report_action(
        "Checking " . pubmed_link("pmid", $pm) . " for more details"
      );
      query_pmid_api([$pm], $this->this_array);
    } elseif ($pm = $this->get("pmc")) {
      report_action(
        "Checking " . pubmed_link("pmc", $pm) . " for more details"
      );
  ...
  public function get_open_access_url(): void
  {
    ...
    if (
      in_array($return, ["publisher", "projectmuse", "have free"], true)
    ) {
      return;
    } // Do continue on
  ...
  private function get_semanticscholar_url(string $doi): void
  {
    ...
    if ($response) {
      ...
      if (
        $oa !== false &&
        isset($oa->url) &&
        isset($oa->is_publisher_licensed) &&
        $oa->is_publisher_licensed &&
        isset($oa->openAccessPdf) &&
        $oa->openAccessPdf
      ) {
        $url = $oa->url;
        unset($oa);
  ...
  public function get_unpaywall_url(string $doi): string
  {
    ...
    if ($json) {
      ...
      if ($oa !== false && isset($oa->best_oa_location)) {
        ...
        if (
          isset($oa->journal_name) &&
          $oa->journal_name ===
            "Cochrane Database of Systematic Reviews"
        ) {
          report_warning(
            "Ignored a OA from Cochrane Database of Systematic Reviews for DOI: " .
              echoable($doi)
          ); // @codeCoverageIgnore
        ...
        if (
          stripos($oa_url, "orbit.dtu.dk/en/publications") !== false
        ) {
          return "nothing";
        } // Abstract only
        ...
        if (
          ($this->has("pmc") &&
            preg_match(
              "~^https?://europepmc\.org/articles/pmc\d" .
                "|^https?://www\.pubmedcentral\.nih\.gov/articlerender.fcgi\?.*\bartid=\d" .
                "|^https?://www\.ncbi\.nlm\.nih\.gov/(?:m/)?pmc/articles/PMC\d~",
              $oa_url
            )) ||
          ($this->has("arxiv") &&
        ...
        if (
          $this->has("doi") &&
          preg_match(
            "~^https?://doi\.library\.ubc\.ca/|^https?://(?:dx\.|)doi\.org/~",
            $oa_url
          )
        ) {
          return "publisher";
        ...
        if (
          ...
          $this->has("rfc") ||
          ($this->has("doi") &&
            $this->get("doi-access") === "free") ||
          ($this->has("jstor") &&
            $this->get("jstor-access") === "free") ||
          ($this->has("osti") &&
            $this->get("osti-access") === "free") ||
        ...
        if (
          ...
          report_minor_error(
            " OA database gave invalid URL: " . echoable($oa_url)
          ); // @codeCoverageIgnore
        ...
        if (
          ($this->has("osti") &&
            stripos($oa_hostname, "osti.gov") !== false) ||
          ($this->has("ssrn") &&
            stripos($oa_hostname, "ssrn.com") !== false) ||
          ($this->has("jstor") &&
            stripos($oa_hostname, "jstor.org") !== false) ||
          ($this->has("pmid") &&
            stripos($oa_hostname, "nlm.nih.gov") !== false) ||
          ($this->has("jstor") &&
            stripos($oa_hostname, "jstor") !== false) ||
        ...
        if ($this->has($url_type) && !$has_url_already) {
          ...
          if ($headers_test === "") {
            ...
            report_warning(
              "Open access URL was unreachable from Unpaywall API for doi: " .
                echoable($doi)
            );
            return "nothing";
          }
          ...
          if ($response_code > 400) {
            ...
            $this->forget($url_type);
            report_warning(
              "Open access URL gave response code " .
                (string) $response_code .
                " from oiDOI API for doi: " .
                echoable($doi)
            );
            return "nothing";
          }
  ...
  public function clean_google_books(): void
  {
    foreach (ALL_URL_TYPES as $url_type) {
      if ($this->has($url_type)) {
        ...
        if (
          preg_match(
            '~^https?://books\.google\.[^/]+\/books\/about\/[^/]+\.html$~',
            $url,
            $matches
          )
        ) {
        ...
        if (
          preg_match(
            '~^https?://(?:books|www)\.google\.[^/]+\/books\/edition\/[a-zA-Z0-9\_]+\/?$~',
            $url,
            $matches
          )
        ) {
        ...
        if (
          preg_match(
            '~^https?://(?:books|www)\.google\.[^/]+\/books\?pg=P\S\S\S\S*$~',
            $url,
            $matches
          )
        ) {
        ...
        if (
          preg_match(
            '~^https?://(?:books|www)\.google\.[^/]+\/books\/edition\/[a-zA-Z0-9\_]+\/([a-zA-Z0-9\-]+)\?(.+)$~',
            $url,
            $matches
          )
        ) {
        ...
        if (
          preg_match(
            '~^https?://books\.google\.[^/]+/(?:books|)\?[qv]id=(.+)$~',
            $url,
            $matches
          )
        ) {
        ...
        if (
          preg_match(
            '~^https?://books\.google\.([^/]+)/books\?((?:isbn|vid)=.+)$~',
            $this->get($url_type),
            $matches
          )
        ) {
          if ($matches[1] !== "com") {
            $this->set(
              $url_type,
              "https://books.google.com/books?" . $matches[2]
            );
          }
        }
      }
  ...
  private function expand_by_google_books_inner(
    string $url_type,
    bool $use_it
  ): bool {
    static $ch = null;
    ...
    if ($url_type) {
      ...
      if (
        ...
        preg_match(
          '~^https?:\/\/(?:www|books)\.google\.[a-zA-Z\.][a-zA-Z\.][a-zA-Z\.]?[a-zA-Z\.]?[a-zA-Z\.]?[a-zA-Z\.]?[a-zA-Z\.]?[a-zA-Z\.]?\/books\/(?:editions?|about)\/_\/([^\s\<\|\{\}\>\]\&\?\%]+)$~i',
          $url,
          $matches
        )
      ) {
        ...
        preg_match(
          '~^https?:\/\/(?:www|books)\.google\.[a-zA-Z\.][a-zA-Z\.][a-zA-Z\.]?[a-zA-Z\.]?[a-zA-Z\.]?[a-zA-Z\.]?[a-zA-Z\.]?[a-zA-Z\.]?\/books\/(?:editions?|about)\/[^\/\/\s\<\|\{\}\>\]]+\/([^\? \]\[\&\%]+)$~i',
          $url,
          $matches
        )
      ) {
      ...
      if (
        preg_match(
          "~^https?://www\.google\.(?:[^\./]+)/books/(?:editions?|about)/_/(.+)$~",
          $url,
          $matches
        )
      ) {
      ...
      if (
        !preg_match(
          "~(?:[Bb]ooks|[Ee]ncrypted)\.[Gg]oogle\.[\w\.]+/.*\bid=([\w\d\-]+)~",
          $url,
          $gid
        ) &&
        !preg_match(
          "~\.[Gg]oogle\.com/books/edition/_/([a-zA-Z0-9]+)(?:\?.+|)$~",
          $url,
          $gid
        )
      ) {
      ...
      if ($isbn) {
        ...
        /** @psalm-taint-escape ssrf */
        $google_book_url =
          "https://www.google.com/search?tbo=p&tbm=bks&q=isbn:" .
          $isbn;
        ...
        if (
          $google_content &&
          preg_match_all(
            "~[Bb]ooks\.[Gg]oogle\.com/books\?id=(............)&amp~",
            $google_content,
            $google_results
          )
        ) {
    ...
    if (
      $url &&
      (preg_match(
        "~[Bb]ooks\.[Gg]oogle\.[\w\.]+/.*\bid=([\w\d\-]+)~",
        $url,
        $gid
      ) ||
        preg_match(
          "~[Ee]ncrypted\.[Gg]oogle\..+book.*\bid=([\w\d\-]+)~",
          $url,
          $gid
        ))
    ) {
    ...
    if (
      preg_match(
        "~^(.+\.google\.com/books/edition/[^\/]+/)([a-zA-Z0-9]+)(\?.+|)$~",
        $url,
        $gid
      )
    ) {
      if ($url_type && $gid[3] === "?hl=en") {
        report_forget(
          "Anonymized/Standardized/Denationalized Google Books URL"
        );
  ...
  private function google_book_details(string $gid): void
  {
    ...
    set_time_limit(120);
    $google_book_url =
      "https://books.google.com/books/feeds/volumes/" . $gid;
    curl_setopt($ch, CURLOPT_URL, $google_book_url);
    ...
    if ($xml->dc___title[1]) {
      $this->add_if_new(
        ...
        wikify_external_text(
          str_replace(
            "___",
            ":",
            $xml->dc___title[0] . ": " . $xml->dc___title[1]
          )
        )
      );
    } else {
      $this->add_if_new(
        "title",
        wikify_external_text(
          str_replace("___", ":", (string) $xml->title)
        )
      );
    }
    $isbn = "";
    ...
    if (
      $this->blank(
        array_merge(FIRST_EDITOR_ALIASES, FIRST_AUTHOR_ALIASES, [
          "publisher",
          "journal",
          "magazine",
          "periodical",
      ...
      foreach ($xml->dc___creator as $author) {
        if (
          strtolower(str_replace("___", ":", (string) $author)) ===
          "gale group"
        ) {
          break;
        }
        ...
        $this->validate_and_add(
          "author" . (string) ++$i,
          str_replace("___", ":", (string) $author),
          "",
          "",
          true
        );
    ...
    if (
      ...
      $this->add_if_new(
        "publisher",
        str_replace("___", ":", (string) $xml->dc___publisher)
      );
    }
  ...
  private function parameter_names_to_lowercase(): void
  {
    ...
    foreach ($keys as $the_key) {
      $this->param[$the_key]->param = str_replace(
        "duplicate_",
        "DUPLICATE_",
        strtolower($this->param[$the_key]->param)
      );
    }
  }
  ...
  private function use_unnamed_params(): void
  {
    ...
    foreach ($this->param as $pointer => $par) {
      if ($par->param && isset($param_occurrences[$par->param])) {
        ...
        array_unshift(
          $duplicate_identical,
          mb_strtolower(trim($par->val)) ===
            mb_strtolower(trim($this->param[$duplicate_pos]->val))
        ); // Drop duplicates that differ only by case
      }
    ...
    for ($i = 0; $i < $n_dup_params; $i++) {
      ...
      if ($is_same) {
        ...
        $this->param[$the_dup]->param = str_ireplace(
          "DUPLICATE_DUPLICATE_",
          "DUPLICATE_",
          "DUPLICATE_" . $this->param[$the_dup]->param
        );
        report_modification(
    ...
    foreach ($this->param as &$p) {
      ...
      if (empty($p->param)) {
        $p->param =
          "CITATION_BOT_PLACEHOLDER_EMPTY_" . (string) $blank_count++;
        $p->eq = " = ";
      }
    }
    ...
    foreach ($this->param as &$p) {
      if (
        stripos($p->param, "CITATION_BOT_PLACEHOLDER_EMPTY") === false
      ) {
        continue;
      }
      $dat = $p->val;
      ...
      if (isset($endnote_test[1])) {
        ...
        foreach ($endnote_test as $endnote_line) {
          ...
          switch ($endnote_linetype) {
            case "A":
              $this->add_if_new(
                "author" . (string) ++$endnote_authors,
                format_author($endnote_datum)
              );
              $dat = trim(
                str_replace("\n%$endnote_line", "", "\n" . $dat)
              );
      ...
      foreach ($parameter_list as $parameter) {
        if (
          ...
          $parameter_value = trim(
            mb_substr($dat, mb_strlen($match[1]))
          );
          report_add(
            "Found " .
              echoable($parameter) .
              " floating around in template; converted to parameter"
          );
        ...
        $test_dat = preg_replace(
          "~\d~",
          "_$0",
          preg_replace(
            "~[ -+].*$~",
            "",
            substr(mb_strtolower($dat), 0, $para_len)
          )
        );
        if (preg_match("~\d~", $parameter)) {
          $lev = (float) levenshtein(
            $test_dat,
            preg_replace("~\d~", "_$0", $parameter)
          );
        } else {
      ...
      if (
        $shortest < 3 &&
        strlen($test_dat) > 0 &&
        (float) similar_text($closest, $test_dat) /
          (float) strlen($test_dat) >
          0.4 &&
        ...
        if (
          preg_match(
            "~^[ -+]*(.+)~",
            (string) substr($dat, strlen($closest)),
            $match
          )
        ) {
          ...
          $this->add_if_new(
            $closest,
            $match[1] /* . " [$shortest / $comp = $shortish]"*/
          );
  ...
  private function id_to_param(): void
  {
    ...
    while (
      preg_match(
        "~\b(PMID|DOI|ISBN|ISSN|ARXIV|LCCN|CiteSeerX|s2cid|PMC)[\s:]*(\d[\d\s\-][^\s\}\{\|,;]*)(?:[,;] )?~iu",
        $id,
        $match
      )
    ) {
    ...
    if (
      preg_match_all(
        "~" . sprintf(self::PLACEHOLDER_TEXT, "(\d+)") . "~",
        $id,
        $matches
      )
    ) {
        ...
        switch ($subtemplate_name) {
          case "arxiv":
            if ($subtemplate->get("id")) {
              $archive_parameter = trim(
                $subtemplate->get("archive")
                  ? $subtemplate->get("archive") . "/"
                  : ""
              );
              $this->add_if_new(
                "arxiv",
                $archive_parameter . $subtemplate->get("id")
              );
            } elseif (!is_null($subtemplate->param_with_index(1))) {
              $this->add_if_new(
                "arxiv",
                trim($subtemplate->param_value(0)) .
                  "/" .
          ...
          case "biorxiv":
            ...
            if (
              $subtemplate_name === "asin" &&
              $subtemplate->has("country")
            ) {
              report_info(
            ...
            if (
              $subtemplate_name === "ol" &&
              stripos($subtemplate->parsed_text(), "ia:") !==
                false
            ) {
              report_info(
                "{{OL}} ia: parameter not supported: cannot convert."
              );
              break;
            }
            if (
              ($subtemplate_name === "jstor" &&
                $subtemplate->has("sici")) ||
              $subtemplate->has("issn")
            ) {
              report_info(
            ...
            if (
              $subtemplate_name === "oclc" &&
              !is_null($subtemplate->param_with_index(1))
            ) {
              report_info(
            ...
            if (
              $subtemplate_name === "issn" &&
              !is_null($subtemplate->param_with_index(1))
            ) {
              report_info(
                "{{ISSN}} has multiple parameters: cannot convert."
              );
              report_info(echoable($subtemplate->parsed_text()));
              break;
            }
            if (
              $subtemplate_name === "ismn" &&
              !is_null($subtemplate->param_with_index(1))
            ) {
              report_info(
                "{{ISMN}} has multiple parameters: cannot convert."
              );
              report_info(echoable($subtemplate->parsed_text()));
              break;
            }
            if (
              $subtemplate_name === "biorxiv" &&
              !is_null($subtemplate->param_with_index(1))
            ) {
              report_info(
                "{{biorxiv}} has multiple parameters: cannot convert."
              );
              report_info(echoable($subtemplate->parsed_text()));
              break;
            }
            if ($subtemplate_name === "lcc") {
              if (
                ...
                report_minor_error(
                  "Possible bad LCC template (did they mean LCCN) : " .
                    echoable($subtemplate->param_value(0))
                ); // @codeCoverageIgnore
              }
              break;
            }
            ...
            $did_it = $this->add_if_new(
              $subtemplate_name,
              $subtemplate_identifier
            );
            if ($did_it) {
              $id = str_replace($matches[0][$i], "", $id);
            }
          ...
          default:
            report_minor_error(
              "No match found for subtemplate type: " .
                echoable($subtemplate_name)
            ); // @codeCoverageIgnore
        }
      }
    }
  ...
  public function correct_param_mistakes(): void
  {
    ...
    foreach ($this->param as $p) {
      if (
        substr_count($p->val, "=") === 1 &&
        !in_array($p->param, PARAMETER_LIST, true)
      ) {
        $param = $p->param;
        $value = $p->val;
  ...
  private function correct_param_spelling(): void
  {
    ...
    foreach ($this->param as $p) {
      if (
        ...
        !(
          in_array(
            preg_replace("~\d+~", "#", $p->param),
            $parameter_list,
            true
          ) || in_array($p->param, $parameter_list, true)
        ) && // Some parameters have actual numbers in them
        stripos($p->param, "CITATION_BOT") === false
      ) {
        if (trim($p->val) === "") {
          if (stripos($p->param, "DUPLICATE_") === 0) {
            report_forget(
              "Dropping empty left-over duplicate parameter " .
                echoable($p->param) .
                " "
            );
          } else {
            report_forget(
              "Dropping empty unrecognized parameter " .
                echoable($p->param) .
                " "
            );
          }
        ...
        if (stripos($p->param, "DUPLICATE_") === 0) {
          ...
          report_modification(
            "Unrecognized parameter " . echoable($p->param) . " "
          );
        }
        $mistake_id = array_search($p->param, $mistake_keys);
        if ($mistake_id) {
          ...
          report_modification(
            "replaced with " .
              echoable($mistake_corrections[$mistake_id]) .
              " (common mistakes list)"
          );
          continue;
        }
        ...
        if ($str_len < 4) {
          $shortest *=
            (float) $str_len /
            (float) (similar_text($p->param, $closest)
              ? similar_text($p->param, $closest)
              : 0.001);
          $shortish *=
            (float) $str_len /
            (float) (similar_text($p->param, $comp)
              ? similar_text($p->param, $comp)
              : 0.001);
        }

        if (in_array($p->param, $parameter_dead, true)) {
          report_inline(
            "Could not fix outdated " . echoable($p->param)
          );
        } elseif ($shortest < 12 && $shortest < $shortish) {
          bot_debug_log(
            "levenshtein replaced " .
              $p->param .
              " with " .
              $closest
          );
          $p->param = $closest;
          report_inline(
            "replaced with $closest (likelihood " .
              (string) round(24.0 - $shortest, 1) .
              "/24)"
          ); // Scale arbitrarily re-based by adding 12 so users are more impressed by size of similarity
        } else {
          $similarity =
            (float) similar_text($p->param, $closest) /
            (float) strlen($p->param);
          if ($similarity > 0.6) {
            bot_debug_log(
              "levenshtein replaced " .
                $p->param .
            ...
            $p->param = $closest;
            report_inline(
              "replaced with $closest (similarity " .
                (string) round(24.0 * $similarity, 1) .
                "/24)"
            ); // Scale arbitrarily re-based by multiplying by 2 so users are more impressed by size of similarity
          } else {
            bot_debug_log("levenshtein could not fix " . $p->param);
            report_inline(
              "could not be replaced with confidence. Please check the citation yourself."
            );
          }
        }
      }
  ...
  {
    return is_array($this->initial_author_params) &&
      sizeof($this->initial_author_params) > 0;
  }

  private function join_params(): string
  {
    if (
      self::$name_list_style === NAME_LIST_STYLE_VANC &&
      !$this->had_initial_author() &&
      !$this->had_initial_editor
    ) {
      ...
      foreach ($vanc_attribs as $vanc_idx => $vanc_attrib) {
        ...
        $v = "";
        if (
          !array_key_exists($vanc_attrib, $this->param) ||
          !isset($this->param[$vanc_attrib])
        ) {
          $arr = [];
          foreach ($this->param as $k => $p) {
            if (
              str_starts_with($p->param, $vanc_f) ||
              str_starts_with($p->param, $vanc_l)
            ) {
              $arr[$p->param] = $p->val;
              unset($this->param[$k]);
            }
  ...
  public function change_name_to(
    string $new_name,
    bool $rename_cite_book = true,
    bool $rename_anything = false
  ): void {
    if (
      strpos($this->get("doi"), "10.1093") !== false &&
      $this->wikiname() !== "cite web"
    ) {
      return;
    }
    ...
    foreach (WORK_ALIASES as $work) {
      ...
      if (
        preg_match(REGEXP_PLAIN_WIKILINK, $worky, $matches) ||
        preg_match(REGEXP_PIPED_WIKILINK, $worky, $matches)
      ) {
    ...
    if (
      $this->wikiname() === "cite book" &&
      !$this->blank_other_than_comments(CHAPTER_ALIASES_AND_SCRIPT)
    ) {
      return; // Changing away leads to error
    }
    if (
      $this->wikiname() === "cite document" &&
      in_array(strtolower($this->get("work")), ARE_WORKS, true)
    ) {
    ...
    if (
      $this->has("conference") &&
      $this->wikiname() === "cite conference"
    ) {
      return;
    } // Need to lose conference first
    if (
      (in_array($this->wikiname(), TEMPLATES_WE_RENAME, true) &&
        ($rename_cite_book || $this->wikiname() !== "cite book")) ||
      ($this->wikiname() === "cite news" &&
        $new_name === "cite magazine") ||
      ...
      if (substr($this->name, 0, 1) === "c") {
        ...
        $this->name =
          $spacing[1] . $invoke . mb_ucfirst($new_name) . $spacing[2];
      }
    ...
    if ($new_name === "cite book" && $this->wikiname() === "cite book") {
      ...
      if ($this->should_url2chapter(true)) {
        ...
        !$this->blank(["chapter-url", "chapterurl"]) &&
        str_i_same($this->get("chapter-url"), $this->get("url"))
      ) {
        $this->forget("url");
  ...
  public function tidy_parameter(string $param): void
  {
    ...
    if (
      ...
      $this->wikiname() !== "citation" &&
      preg_match(
        '~^(?:# # # CITATION_BOT_PLACEHOLDER_COMMENT \d+ # # #)\s*(?:# # # CITATION_BOT_PLACEHOLDER_TEMPLATE \d+ # # #|)$~i',
        $this->get("postscript")
      )
    ) {
    ...
    if ($this->has($param)) {
      if (
        ...
        (($param !== "chapter" && $param !== "title") ||
          strlen($this->get($param)) > 4) // Avoid tiny titles that might be a smiley face
      ) {
        $this->set(
          $param,
          safe_preg_replace(
            "~[\x{2000}-\x{200A}\x{00A0}\x{202F}\x{205F}\x{3000}]~u",
            " ",
            $this->get($param)
          )
        ); // Non-standard spaces
        ...
        ); // tabs, linefeeds, null bytes
        $this->set(
          $param,
          safe_preg_replace("~﻿~u", " ", $this->get($param))
        );
        $this->set(
          $param,
          safe_preg_replace("~  +~u", " ", $this->get($param))
        ...
        $this->set(
          $param,
          safe_preg_replace('~(?<!:)[:,]$~u', "", $this->get($param))
        ); // Remove trailing commas, colons, but not semi-colons--They are HTML encoding stuff
        $this->set(
          $param,
          safe_preg_replace("~^[:,;](?!:)~u", "", $this->get($param))
        ); // Remove leading commas, colons, and semi-colons
        $this->set(
          $param,
          safe_preg_replace(
            '~^\=+\s*(?![^a-zA-Z0-9\[\'\"])~u',
        ...
        ); // Remove leading ='s sign if in front of letter or number
        $this->set(
          $param,
          safe_preg_replace("~&#x2010;~u", "-", $this->get($param))
        );
        $this->set(
          $param,
          safe_preg_replace("~\x{2010}~u", "-", $this->get($param))
        );
        $this->set(
          $param,
          safe_preg_replace(
        ...
        ); // &Amp; => & but not if next character is & or previous character is ;
        ...
        $this->set(
          $param,
          safe_preg_replace("~\x{00AD}~u", "", $this->get($param))
        ); // Remove soft hyphen
      }
      if (
        ...
        $this->set(
          $param,
          safe_preg_replace("~\x{200B}~u", " ", $this->get($param))
        ); //Zero-width anywhere
        while (
          preg_match("~^&nbsp;(.+)$~u", $this->get($param), $matches)
        ) {
    ...
    if (
      ...
      $this->set(
        $param,
        safe_preg_replace("~_~u", " ", $this->get($param))
      );
    }

    if (!preg_match('~^(\D+)(\d*)(\D*)$~', $param, $pmatch)) {
      report_minor_error(
        "Unrecognized parameter name format in " . echoable($param)
      ); // @codeCoverageIgnore
      return; // @codeCoverageIgnore
    } else {
      ...
      switch ($pmatch[1]) {
            ...
            if (
              ...
              if (stripos($the_url, "apnews.com") !== false) {
                if ($this->get("work") === "AP") {
                  ...
                  $this->get("work") === "Associated Press"
                ) {
        ...
        case "author":
          ...
          if (
            $pmatch[2] &&
            $this->has("first" . $pmatch[2]) &&
            $this->blank("last" . $pmatch[2])
          ) {
            $this->rename(
              "author" . $pmatch[2],
              "last" . $pmatch[2]
        ...
        case "surname":
          if (!$this->initial_author_params) {
            if ($pmatch[2]) {
              $translator_regexp =
                "~\b([Tt]r(ans(lat...?(by)?)?)?\.?)\s([\w\p{L}\p{M}\s]+)$~u";
              if (
                preg_match(
                  $translator_regexp,
                  trim($this->get($param)),
          ...
          if ($pmatch[2] && $pmatch[1] === "last") {
            ...
            if (
              ...
              if (
                ...
                $this->add_if_new(
                  "author" . $pmatch[2] . "-link",
                  $matches[1]
                );
                $did_something = true;
              } elseif (
                preg_match(
                  REGEXP_PIPED_WIKILINK_ONLY,
                  $the_author,
                  $matches
                )
              ) {
          ...
          if (
            !$pmatch[2] &&
            $pmatch[1] === "last" &&
            !$this->blank(["first1", "first2", "last2"])
          ) {
            if ($this->blank("last1")) {
        ...
        case "bibcode":
          ...
          foreach (NON_JOURNAL_BIBCODES as $exception) {
            if (
              substr($bibcode_journal, 0, strlen($exception)) ===
              $exception
            ) {
              return;
            }
        ...
        case "chapter":
          if ($this->has("chapter")) {
            ...
            if (
              $this->get("chapter") === "Cultural Advice" &&
              strpos(
                $this->get("url") . $this->get("chapter-url"),
                "anu.edu.au"
              ) !== false
            ) {
              $this->forget("chapter");
        ...
        case "month":
          ...
          if ($this->has("date")) {
            if (
              stripos($this->get("date"), $this->get($param)) !==
              false
            ) {
          ...
          if (
            $day === "" &&
            preg_match('~^([a-zA-Z]+) 0*(\d+)$~', $month, $matches)
          ) {
            $day = $matches[2];
            $month = $matches[1];
          }
          if (
            $day === "" &&
            preg_match('~^0*(\d+) ([a-zA-Z]+)$~', $month, $matches)
          ) {
            $day = $matches[1];
            $month = $matches[2];
          }
        ...
        case "doi":
          ...
          if (substr($doi, 0, 8) === "10.5555/") {
            ...
            if ($this->blank("url")) {
              /** @psalm-taint-escape ssrf */
              $test_url =
                "https://plants.jstor.org/stable/" . $doi;
              $ch = bot_curl_init(1.5, [
          ...
          if (
            !doi_works($doi) &&
            stripos($doi, "10.1043/0003-3219(") === 0
          ) {
          ...
          if (doi_works($doi) === null) {
            ...
            if (
              stripos($doi, "10.1258/jrsm.") === 0 ||
              stripos($doi, "10.1525/as.") === 0 ||
              stripos($doi, "10.1525/sp.") === 0 ||
              stripos($doi, "10.1067/mva.") === 0
            ) {
              $doi = $this->get("doi");
            ...
            if (
              stripos($doi, "10.2979/new.") === 0 ||
              stripos($doi, "10.2979/FSR.") === 0 ||
              stripos($doi, "10.2979/NWS") === 0 ||
              stripos($doi, "10.1353/nwsa.") === 0
            ) {
          ...
          if (
            !doi_works($doi) &&
            strpos($doi, "10.1111/j.1475-4983." . $this->year()) ===
              0
          ) {
          ...
          if (!doi_works($doi)) {
            if (
              preg_match(
                '~^10.1093\/oi\/authority\.\d{10,}$~',
                $doi
              ) &&
              preg_match(
                "~(?:oxfordreference\.com|oxfordindex\.oup\.com)\/[^\/]+\/10.1093\/oi\/authority\.\d{10,}~",
                $this->get("url")
              )
            ) {
              $this->forget("doi");
              ...
              preg_match(
                "~ouplaw.com\/view\/10\.1093/law\:epil\/9780199231690\/law\-9780199231690~",
                $this->get("url")
              )
            ) {
              $this->forget("doi");
          ...
          if (
            stripos($doi, "10.1093/law:epil") === 0 ||
            stripos($doi, "10.1093/oi/authority") === 0
          ) {
            return;
          }
          if (
            ...
            if (
              ...
              $the_journal =
                ...
                $this->get("periodical");
              if (
                str_replace(NON_JOURNALS, "", $the_journal) ===
                  $the_journal &&
                !$this->blank(WORK_ALIASES)
              ) {
                $this->change_name_to("cite journal", false);
        ...
        case "hdl":
          ...
          if (
            preg_match('~^(.+)(%3Bownerid=.*)$~', $handle, $matches)
          ) {
        ...
        case "format": // clean up bot's old (pre-2018-09-18) edits
          if (
            $this->get($param) === "Accepted manuscript" ||
          ...
          if (
            ...
            if (
              $this->blank("url") ||
              strtolower(substr($this->get("url"), -4)) === ".pdf"
            ) {
              $this->forget($param);
            }
          }
        ...
        case "chapter-format":
          ...
          if (
            ...
            if ($this->has("chapter-url")) {
              if (
                substr($this->get("chapter-url"), -4) ===
                  ".pdf" ||
                substr($this->get("chapter-url"), -4) === ".PDF"
              ) {
                $this->forget($param);
              }
            } elseif ($this->has("chapterurl")) {
              if (
                substr($this->get("chapterurl"), -4) ===
                  ".pdf" ||
                substr($this->get("chapterurl"), -4) === ".PDF"
              ) {
                $this->forget($param);
              }
            } else {
        ...
        case "isbn":
          ...
          $this->set(
            "isbn",
            safe_preg_replace("~\s?-\s?~", "-", $this->get("isbn"))
          ); // a White space next to a dash
          $this->set(
            "isbn",
            $this->isbn10Toisbn13($this->get("isbn"))
          );
          if (
            $this->blank("journal") ||
            $this->has("chapter") ||
        ...
        case "eissn":
          ...
          if (
            preg_match('~^(\d{4})\s?(\d{3}[\dX])$~', $new, $matches)
          ) {
        ...
        case "periodical":
          ...
          if (
            $this->get($param) === "Undefined" ||
            $this->get($param) === "Semantic Scholar" ||
            $this->get($param) === "[[Semantic Scholar]]"
          ) {
            $this->forget($param);
            return;
          ...
          if (
            preg_match(
              '~^(|[a-zA-Z0-9][a-zA-Z0-9]+\.)([a-zA-Z0-9][a-zA-Z0-9][a-zA-Z0-9]+)\.(org|net|com)$~',
              $this->get($param)
            )
          ) {
          ...
          if (
            str_equivalent($this->get($param), $this->get("work"))
          ) {
            $this->forget("work");
          }
          ...
          if (
            substr(strtolower($periodical), 0, 7) === "http://" ||
            substr(strtolower($periodical), 0, 8) === "https://"
          ) {
            if ($this->blank("url")) {
            ...
            substr(strtolower($periodical), 0, 4) === "www."
          ) {
          ...
          if (
            $this->blank(["chapter", "isbn"]) &&
            $param === "journal" &&
            stripos($this->get($param), "arxiv") === false
          ) {
          ...
          if (
            (mb_substr($periodical, 0, 2) !== "[[" || // Only remove partial wikilinks
              ...
              mb_substr_count($periodical, "]]") !== 1) &&
            !preg_match(
              "~^(?:the |)(?:publications|publication|journal|transactions|letters|annals|bulletin|reports|history) of the ~i",
              $periodical
            ) &&
            !preg_match(
              "~(magazin für |magazin fur |magazine for |section )~i",
              $periodical
            )
          ) {
            $this->set(
          ...
          if (
            ...
            if (strlen($periodical) - mb_strlen($periodical) < 9) {
              ...
              $this->set(
                $param,
                title_capitalization($periodical, true)
              );
            }
            ...
            if (
              ...
              $new_periodical = title_capitalization(
                mb_ucwords($periodical),
                true
              );
              if (
                str_ireplace(
              ...
              $new_linked_text = title_capitalization(
                mb_ucwords($linked_text),
                true
              );
              if (
                str_ireplace(
          ...
          if (
            ...
            preg_match(REGEXP_PIPED_WIKILINK, $the_param, $matches)
          ) {
            $the_param = $matches[1]; // Always the wikilink for easier standardization
          }
          if (
            in_array(strtolower($the_param), ARE_MAGAZINES, true) &&
            $this->blank(["pmc", "doi", "pmid"])
          ) {
            $this->change_name_to("cite magazine");
            $this->rename($param, "magazine");
            return;
          } elseif (
            in_array(strtolower($the_param), ARE_NEWSPAPERS, true)
          ) {
            ...
            in_array(strtolower($the_param), ARE_WORKS, true)
          ) {
          ...
          foreach (CONFERENCE_LIST as $conf) {
            if (
              stripos($the_title, $conf) !== false &&
              stripos($the_title, $the_param) !== false
            ) {
              $this->forget($param);
            }
          }
        ...
        case "mr":
          if (
            preg_match("~mr(\d+)$~i", $this->get($param), $matches)
          ) {
            $this->set($param, $matches[1]);
          }
          return;
        ...
        case "pmc":
          if (
            preg_match("~pmc(\d+)$~i", $this->get($param), $matches)
          ) {
            $this->set($param, $matches[1]);
          }
        ...
        case "publisher":
          ...
          if (
            stripos($this->get($param), "proquest") !== false &&
            stripos($this->get($param), "llc") === false &&
            stripos(
              $this->get("title"),
          ...
          if (
            ...
            foreach (["journal", "newspaper"] as $the_same) {
              // Prefer wiki-linked
              if (
                mb_strtolower($this->get($the_same)) ===
                $publisher
              ) {
          ...
          if (
            stripos($this->get("url"), "maps.google") !== false &&
            stripos($publisher, "google") !== false
          ) {
            $this->set($param, "Google Maps"); // Case when Google actually IS a publisher
            return;
          }
          if (
            stripos($this->get("url"), "developers.google.com") !==
              false &&
            stripos($publisher, "google") !== false
          ) {
          ...
          if (
            stripos($this->get("url"), "support.google.com") !==
              false &&
            stripos($publisher, "google") !== false
          ) {
            $this->set($param, "Google Inc."); // Case when Google actually IS a publisher
          ...
          if (stripos($publisher, "google") !== false) {
            $this_host = (string) parse_url(
              $this->get("url"),
              PHP_URL_HOST
            );
            if (
              stripos($this_host, "google") === false ||
          ...
          if (
            ...
            if (
              strpos($all_urls, "/dp/") !== false &&
              strpos($all_urls, "/feature/") === false &&
              strpos($all_urls, "/exec/obidos/") === false
            ) {
              $this->forget($param);
              return;
            }
          ...
          if ($this->blank(WORK_ALIASES)) {
            ...
            if (
              ...
              $webby = str_replace(
                ...
                "",
                mb_strtolower($this->get("website"))
              );
              $pubby = str_replace(
                [
                  "[",
          ...
          if ($publisher === "the times digital archive") {
            foreach (WORK_ALIASES as $work) {
              if (
                stripos($this->get($work), "the times") !==
                  false ||
                stripos($this->get($work), "times (london") !==
                  false ||
                stripos($this->get($work), "times [london") !==
                  false
              ) {
          ...
          if (
            $this->blank("via") &&
            $publisher === "the washington post – via legacy.com"
          ) {
          ...
          if (
            ...
            foreach (WORK_ALIASES as $work) {
              if (
                stripos($this->get($work), "new york times") !==
                  false ||
                stripos($this->get($work), "nytimes.com") !==
                  false
              ) {
                $this->forget($param);
                if (
          ...
          if (
            ...
            foreach (WORK_ALIASES as $work) {
              if (
                stripos($this->get($work), "The Guardian") !==
                  false ||
                stripos(
                  $this->get($work),
                  "theguardian.com"
          ...
          if (
            ...
            foreach (WORK_ALIASES as $work) {
              if (
                stripos($this->get($work), "the economist") !==
                  false ||
                stripos($this->get($work), "economist.com") !==
                  false
              ) {
                $this->forget($param);
                if (
          ...
          if ($publisher === "news uk") {
            foreach (WORK_ALIASES as $work) {
              if (
                stripos($this->get($work), "the times") !==
                  false ||
                stripos($this->get($work), "thetimes.co.uk") !==
                  false
              ) {
                $this->forget($param);
                if (
          ...
          if (
            ...
            foreach (WORK_ALIASES as $work) {
              if (
                ...
                stripos($this->get($work), "mercury news") !==
                  false
              ) {
                $this->forget($param);
                if (
          ...
          if (
            ...
            foreach (WORK_ALIASES as $work) {
              if (
                stripos($this->get($work), "forbes") !== false
              ) {
                $this->forget($param);
                if (
                  stripos($this->get($work), "forbes.com") !==
                  false
                ) {
              ...
              if ($this->blank("agency")) {
                if (
                  stripos($this->get($work), "AFX News") !==
                    false ||
                  stripos(
                    $this->get($work),
          ...
          if (
            ...
            foreach (WORK_ALIASES as $work) {
              ...
              if ($this->blank("via")) {
                if (
                  stripos($this->get($work), "laweekly") !==
                  false
                ) {
          ...
          if (
            ...
            if ($this->blank("via")) {
              ...
              stripos($this->get("via"), "pressreader") !== false
            ) {
              $this->forget($param);
            }
            return;
          ...
          if (
            stripos($publisher, "sify.com") !== false ||
            stripos($publisher, "sify ") !== false ||
            $publisher === "sify"
          ) {
            if ($this->blank(WORK_ALIASES)) {
              $this->rename($param, "website");
          ...
          if (
            $publisher === "www.bollywoodhungama.com" ||
          ...
          if (
            stripos($publisher, "bollywoodhungama.com") !== false ||
            stripos($publisher, "bollywood hungama") !== false ||
            stripos($publisher, "BH News Network") !== false
          ) {
            if ($this->blank(WORK_ALIASES)) {
              $this->rename($param, "website");
            } else {
              ...
              if (
                strpos($lower, "bollywoodhungama") !== false ||
                strpos($lower, "bollywood hungama") !== false
              ) {
                $this->forget($param);
            ...
            if (
              $authtmp === "Bollywood Hungama News Network" ||
              $authtmp === "Bollywood Hungama"
            ) {
              $this->forget("author");
            }
            return;
        ...
        case "ref":
          ...
          if ($content === "" || $content === "harv") {
            ...
            preg_match(
              '~^harv( *# # # CITATION_BOT_PLACEHOLDER_COMMENT.*?# # #)$~sui',
              $content,
              $matches
            )
          ) {
        ...
        case "title":
          ...
          if (
            $title === "Validate User" ||
            $title === "Join Ancestry" ||
            $title === "Join Ancestry.com" ||
            $title === "Ancestry - Sign Up"
          ) {
            $this->set("title", "");
            return;
          }
          ...
          if (
            ...
            report_warning(
              "The quotes around the title are most likely an editor's error: " .
                echoable(mb_substr($title, 1, -1))
            );
          }
          ...
          if (
            ...
            if (
              preg_match(REGEXP_PLAIN_WIKILINK, $title, $matches)
            ) {
              if (
                strlen($matches[1]) >
                  0.7 * (float) strlen($title) &&
                $title !== "[[" . $matches[1] . "]]"
              ) {
        ...
        case "archivedate":
          if (
            $this->has("archivedate") &&
            $this->get("archive-date") === $this->get("archivedate")
          ) {
        ...
        case "archiveurl":
          if ($this->blank(["archive-date", "archivedate"])) {
            if (
              preg_match(
                "~^https?://(?:web\.archive\.org/web/|archive\.today/|archive\.\S\S/|webarchive\.loc\.gov/all/|www\.webarchive\.org\.uk/wayback/archive/)(\d{4})(\d{2})(\d{2})\d{6}~",
                $this->get($param),
                $matches
              )
            ) {
            ...
            if (
              preg_match(
                "~^https?://wayback\.archive\-it\.org/\d{4}/(\d{4})(\d{2})(\d{2})\d{6}~",
                $this->get($param),
                $matches
              )
            ) {
          ...
          if (
            ...
            stripos($this->get($param), "citation") === false
          ) {
            $this->set($param, "http://" . $matches[0]);
          }
          if (
            preg_match(
              "~^https?://(?:web\.archive\.org/web|archive\.today|archive\.\S\S|webarchive\.loc\.gov/all|www\.webarchive\.org\.uk/wayback/archive)/(?:save|\*)/~",
              $this->get($param)
            ) ||
            preg_match(
          ...
          if (
            preg_match(
              '~^(https?://(?:www\.|)webcitation\.org/)([0-9a-zA-Z]{9})(?:|\?url=.*)$~',
              $this->get($param),
              $matches
            )
          ) {
            ...
            if ($this->blank(["archive-date", "archivedate"])) {
              ...
              for ($i = 0; $i < 9; $i++) {
                $time =
                  62 * $time +
                  (int) strpos($base62, $num62[$i]);
              }
              $this->add_if_new(
                "archive-date",
                date("Y-m-d", (int) ($time / 1000000))
              );
            }
            return;
          }
          ...
          if (
            ...
            stripos($this->get($param), "petabox.bibalex.org") ===
              false
          ) {
            if (
              preg_match(
                "~^https?://(?:www\.|)researchgate\.net/[^\s]*publication/([0-9]+)_*~i",
                $this->get($param),
                $matches
              )
            ) {
              ...
              preg_match(
                "~^https?://(?:www\.|)academia\.edu/(?:documents/|)([0-9]+)/*~i",
                $this->get($param),
                $matches
              )
            ) {
              $this->set(
                $param,
                "https://www.academia.edu/" . $matches[1]
              );
            } elseif (
              preg_match(
                "~^https?://(?:www\.|)zenodo\.org/record/([0-9]+)(?:#|/files/)~i",
                $this->get($param),
                $matches
              )
            ) {
              $this->set(
                $param,
                "https://zenodo.org/record/" . $matches[1]
              );
            } elseif (
              preg_match(
              ...
              $this->set(
                $param,
                simplify_google_search($this->get($param))
              );
            } elseif (
              preg_match(
                "~^(https?://(?:www\.|)sciencedirect\.com/\S+)\?via(?:%3d|=)\S*$~i",
                $this->get($param),
                $matches
              )
            ) {
              ...
              preg_match(
                "~^(https?://(?:www\.|)bloomberg\.com/\S+)\?(?:utm_|cmpId=)\S*$~i",
                $this->get($param),
                $matches
              )
            ) {
              ...
              preg_match(
                "~^https?://onlinelibrarystatic\.wiley\.com/store/~",
                $this->get($param)
              )
            ) {
              $this->forget($param);
            ...
            if (
              $this->get_identifiers_from_url($this->get($param))
            ) {
          ...
          // Remove trailing #
          if (
            preg_match("~^(\S+)#$~u", $this->get($param), $matches)
          ) {
            $this->set($param, $matches[1]);
        ...
        case "chapterurl":
          ...
          if (
            $this->blank("url") &&
            $this->blank(CHAPTER_ALIASES_AND_SCRIPT)
          ) {
            $this->rename($param, "url");
        ...
        case "url":
          ...
          if (
            ...
            preg_match(
              "~^https?://(?:www\.|)bloomberg\.com/tosv2\.html\?vid=&uuid=(?:.+)&url=([a-zA-Z0-9/\+]+=*)$~",
              $this->get($param),
              $matches
            )
          ) {
            ...
            preg_match(
              "~^https:?//myprivacy\.dpgmedia\.nl/.+callbackUrl=(.+)$~",
              $this->get($param),
              $matches
            )
          ) {
            ...
            if (
              preg_match(
                "~^(https.+)/privacy-gate/accept-tcf2\?redirectUri=(/.+)$~",
                $the_match,
                $matches
              )
            ) {
            ...
            preg_match(
              "~^https?://academic\.oup\.com/crawlprevention/governor\?content=([^\s]+)$~",
              $this->get($param),
              $matches
            )
          ) {
            ...
            $this->set(
              ...
              "https://academic.oup.com" .
                preg_replace(
                  '~(?:\?login=false|\?redirectedFrom=fulltext|\?login=true)$~i',
                  "",
                  urldecode($matches[1])
                )
            );
            ...
            preg_match(
              "~^https?://.*ebookcentral.proquest.+/lib/.+docID(?:%3D|=)(\d+)(|#.*|&.*)(?:|\.)$~i",
              $this->get($param),
              $matches
            )
          ) {
            ...
            preg_match(
              "~^https?://(?:www\.|)figshare\.com/articles/journal_contribution/[^/]+/([0-9]+)$~i",
              $this->get($param),
              $matches
            )
          ) {
          ...
          if (
            preg_match(
              "~https?://www\.britishnewspaperarchive\.co\.uk/account/register.+viewer\%252fbl\%252f(\d+)\%252f(\d+)\%252f(\d+)\%252f(\d+)(?:\&|\%253f)~",
              $this->get($param),
              $matches
            )
          ) {
          ...
          if (
            preg_match(
              "~^https?(://pubs\.rsc\.org.+)#!divAbstract$~",
              $this->get($param),
              $matches
            )
          ) {
          ...
          if (
            preg_match(
              "~^https?://www.healthaffairs.org/do/10.1377/hblog(\d+\.\d+)/full/$~",
              $this->get($param),
              $matches
            )
          ) {
            $this->set(
              $param,
              "https://www.healthaffairs.org/do/10.1377/forefront." .
                $matches[1] .
                "/full/"
            );
            ...
            $this->add_if_new(
              "doi",
              "10.1377/forefront." . $matches[1]
            );
            if (strpos($this->get("doi"), "forefront") !== false) {
              if (
                strpos(
                  $this->get("archiveurl") .
                    $this->get("archive-url"),
                  "healthaffairs"
                ) !== false
              ) {
          ...
          if (stripos($this->get($param), "youtube") !== false) {
            if (
              preg_match(
                "~^(https?://(?:|www\.|m\.)youtube\.com/watch)(%3F.+)$~",
                $this->get($param),
                $matches
              )
            ) {
          ...
          if (stripos($this->get($param), "proxy") !== false) {
            ...
            if (
              preg_match(
                "~^https?://ieeexplore.ieee.org.+proxy.*/document/(.+)$~",
                $this->get($param),
                $matches
              )
            ) {
              ...
              if (
                $this->has("via") &&
                stripos($this->get("via"), "library") !== false
              ) {
                $this->forget("via");
              }
            } elseif (
              preg_match(
                "~^https?://(?:www.|)oxfordhandbooks.com.+proxy.*/view/(.+)$~",
                $this->get($param),
                $matches
              )
            ) {
              $this->set(
                $param,
                "https://www.oxfordhandbooks.com/view/" .
                  $matches[1]
              );
              report_info(
                "Remove proxy from Oxford Handbooks URL"
              );
              if (
                $this->has("via") &&
                stripos($this->get("via"), "library") !== false
              ...
              preg_match(
                "~^https?://(?:www.|)oxfordartonline.com.+proxy.*/view/(.+)$~",
                $this->get($param),
                $matches
              )
            ) {
              $this->set(
                $param,
                "https://www.oxfordartonline.com/view/" .
                  $matches[1]
              );
              ...
              preg_match(
                "~^https?://(?:www.|)sciencedirect.com[^/]+/(\S+)$~i",
                $this->get($param),
                $matches
              )
            ) {
              ...
              $this->set(
                $param,
                "https://www.sciencedirect.com/" . $matches[1]
              );
              if ($this->has("via")) {
                if (
                  stripos($this->get("via"), "library") !==
                    false ||
                  stripos($this->get("via"), "direct") ===
                    false
                ) {
                  $this->forget("via");
                }
              ...
              preg_match(
                "~^https?://(?:login\.|)(?:lib|)proxy\.[^\?\/]+\/login\?q?url=(https?://)(.+)$~",
                $this->get($param),
                $matches
              )
            ) {
              ...
              preg_match(
                "~^https?://(?:login\.|)(?:lib|)proxy\.[^\?\/]+\/login\?q?url=(https?%3A%2F%2F.+)$~i",
                $this->get($param),
                $matches
              )
            ) {
          ...
          if (
            preg_match(
              "~^https://wikipedialibrary\.idm\.oclc\.org/login\?auth=production&url=(https?://.+)$~i",
              $this->get($param),
              $matches
            )
          ) {
          ...
          if (
            preg_match(
              "~^(https://www\.ancestry(?:institution|).com/discoveryui-content/view/\d+:\d+)\?.+$~i",
              $this->get($param),
              $matches
            )
          ) {
          ...
          if (
            preg_match(
              "~ancestry\.com/cs/offers/join.*url=(http.*)$~i",
              $this->get($param),
              $matches
            )
          ) {
            $this->set(
              $param,
              str_replace(" ", "+", urldecode($matches[1]))
            );
          }
          if (
            preg_match(
              "~ancestry\.com/account/create.*returnurl=(http.*)$~i",
              $this->get($param),
              $matches
            )
          ) {
          ...
          if (
            preg_match(
              "~^https://search\.ancestry(?:|institution)\.com.*cgi-bin/sse.dll.*_phcmd.*(http.+)\'\,\'successSource\'\)$~i",
              $this->get($param),
              $matches
            )
          ) {
          ...
          if (
            preg_match(
              "~^https://search\.ancestry(?:|institution)\.com.*cgi-bin/sse.dll.*_phcmd.*(http.+)%27\,%27successSource%27\)$~i",
              $this->get($param),
              $matches
            )
          ) {
          ...
          if (
            preg_match(
              "~^https://www\.ancestry(?:|institution)\.com/facts.*_phcmd.*(http.+)\'\,\'successSource\'\)$~i",
              $this->get($param),
              $matches
            )
          ) {
          ...
          if (
            preg_match(
              "~^https://www\.ancestry(?:|institution)\.com/facts.*_phcmd.*(http.+)%27\,%27successSource%27\)$~i",
              $this->get($param),
              $matches
            )
          ) {
          ...
          // idm.oclc.org Proxy
          if (
            stripos($this->get($param), "idm.oclc.org") !== false &&
            stripos($this->get($param), "ancestryinstitution") ===
              false
          ) {
            $oclc_found = false;
            if (
              preg_match(
                "~^https://([^\.\-\/]+)-([^\.\-\/]+)-([^\.\-\/]+)\.[^\.\-\/]+\.idm\.oclc\.org/(.+)$~i",
                $this->get($param),
                $matches
              )
            ) {
              ...
              preg_match(
                "~^https://(?:login.?|)[^\.\-\/]+\.idm\.oclc\.org/login\?q?url=(https?://[^\.\-\/]+\.[^\.\-\/]+\.[^\.\-\/]+/.*)$~i",
                $this->get($param),
                $matches
              )
            ) {
              ...
              preg_match(
                "~^https://(?:login.?|)[^\.\-\/]+\.idm\.oclc\.org/login\?q?url=(https?://[^\.\-\/\%]+\.[^\.\-\/\%]+\.[^\.\-\/\%]+)(\%2f.*)$~i",
                $this->get($param),
                $matches
              )
            ) {
            ...
            if ($oclc_found) {
              ...
              if (
                stripos($this->get("via"), "wiki") !== false ||
          ...
          if (
            stripos(
              $this->get($param),
              "https://access.newspaperarchive.com/"
            ) === 0
          ) {
            $this->set(
              $param,
              str_ireplace(
                "https://access.newspaperarchive.com/",
                "https://www.newspaperarchive.com/",
                $this->get($param)
              )
            );
          }
          ...
          if (
            ...
            if (
              preg_match(
                "~^https?://(www\.[^\./\-]+\.com)\.[^/]*(?:proxy|library|\.lib\.|mutex\.gmu)[^/]*/(\S+)$~i",
                $this->get($param),
                $matches
              )
            ) {
          ...
          if (stripos($this->get($param), "galegroup") !== false) {
            if (
              preg_match(
                "~^(?:http.+url=|)https?://go.galegroup.com(%2fps.+)$~i",
                $this->get($param),
                $matches
              )
            ) {
              ...
              if (
                $this->has("via") &&
                stripos($this->get("via"), "gale") === false
              ) {
                $this->forget("via");
              }
            } elseif (
              preg_match(
                "~^http.+url=https?://go\.galegroup\.com/(.+)$~i",
                $this->get($param),
                $matches
              )
            ) {
              $this->set(
                $param,
                "https://go.galegroup.com/" . $matches[1]
              );
              report_info("Remove proxy from Gale URL");
              if (
                $this->has("via") &&
              ...
              preg_match(
                "~^(?:http.+url=|)https?://link.galegroup.com(%2fps.+)$~i",
                $this->get($param),
                $matches
              )
            ) {
              ...
              preg_match(
                "~^http.+url=https?://link\.galegroup\.com/(.+)$~",
                $this->get($param),
                $matches
              )
            ) {
              $this->set(
                $param,
                "https://link.galegroup.com/" . $matches[1]
              );
              report_info("Remove proxy from Gale URL");
              if (
                $this->has("via") &&
          ...
          if (stripos($this->get($param), "proquest") !== false) {
            if (
              preg_match(
                "~^(?:http.+/login/?\?url=|)https?://(?:0\-|)(?:search|www).proquest.com[^/]+(|/[^/]+)+/docview/(.+)$~",
                $this->get($param),
                $matches
              )
            ) {
              ...
              if (
                $this->has("via") &&
                stripos($this->get("via"), "proquest") === false
              ) {
                $this->forget("via");
              }
            } elseif (
              preg_match(
                '~^https?://(.*)proquest.umi.com(.*)/(pqd.+)$~',
                $this->get($param),
                $matches
              )
            ) {
              if ($matches[1] || $matches[2]) {
                $this->set(
                  $param,
                  "http://proquest.umi.com/" . $matches[3]
                );
                report_info("Remove proxy from ProQuest URL");
                if (
                  $this->has("via") &&
              ...
              preg_match(
                "~^(?:http.+/login/?\?url=|)https?://(?:0\-|)(?:www|search).proquest.+scoolaid\.net(|/[^/]+)+/docview/(.+)$~",
                $this->get($param),
                $matches
              )
            ) {
            ...
            if (
              preg_match(
                "~^https?://(?:|search|www).proquest.com/(.+)/docview/(.+)$~",
                $this->get($param),
                $matches
              )
            ) {
              if ($matches[1] !== "dissertations") {
                ...
                $this->set(
                  ...
                  "https://www.proquest.com/docview/" .
                    $matches[2]
                ); // Remove specific search engine
              }
            }
            if (
              preg_match(
                "~^https?://(?:search|www)\.proquest\.com/docview/(.+)/(?:abstract|fulltext|preview|page).*$~i",
                $this->get($param),
                $matches
              )
            ) {
            ...
            if (
              preg_match(
                "~^https?://(?:www|search)\.proquest\.com/docview/([0-9]+)/[0-9A-Z]+/[0-9]+\??$~",
                $this->get($param),
                $matches
              )
            ) {
            ...
            if (
              preg_match(
                "~^https?://search\.proquest\.com/dissertations/docview/(.+)$~",
                $this->get($param),
                $matches
              )
            ) {
              ...
              $this->set(
                $param,
                "https://www.proquest.com/dissertations/docview/" .
                  $matches[1]
              );
            }
            if (
            ...
            if (
              ...
              if (bot_curl_exec($ch) !== "") {
                $redirectedUrl = (string) @curl_getinfo(
                  $ch,
                  CURLINFO_EFFECTIVE_URL
                ); // Final URL
                if (
                  preg_match(
                    "~^https?://.+(\.proquest\.com/docview/\d{4,})(?:|/abstract.*|/fulltext.*|/preview.*)$~",
                    $redirectedUrl,
                    $matches
                  ) ||
                  preg_match(
                    "~^https?://.+(\.proquest\.com/openurl/handler/.+)$~",
                    $redirectedUrl,
                    $matches
                  )
                ) {
                  ...
                  $changed = true;
                  report_forget(
                    "Proquest.umi.com URL does not work.  Forgetting"
                  );
                  $this->forget($param);
                }
              }
          ...
          if (
            stripos($this->get("url"), "cinemaexpress.com") !==
            false
          ) {
            foreach (WORK_ALIASES as $worky) {
              $lower = strtolower($this->get($worky));
        ...
        case "work":
          ...
          if (
            strtolower($this->get("work")) === "latimes" ||
            strtolower($this->get("work")) === "latimes.com"
          ) {
            $this->set("work", "[[Los Angeles Times]]");
          }
          if (
            strtolower($this->get("work")) === "nytimes" ||
          ...
          if (
            $this->has("url") &&
            str_replace(ENCYCLOPEDIA_WEB, "", $this->get("url")) !==
              $this->get("url")
          ) {
          ...
          if (
            $this->has("work") &&
            str_ireplace(
              ["encyclopedia", "encyclopædia", "encyclopaedia"],
              "",
              $this->get("work")
            ) !== $this->get("work")
          ) {
          ...
          if (
            $this->has_but_maybe_blank($param) &&
            $this->blank($work_becomes)
          ) {
            if (
              $work_becomes === "encyclopedia" &&
              $this->wikiname() === "cite web"
          ...
          if (
            ...
            strtolower($the_param) ===
              "pressreader.com (archived)" ||
            strtolower($the_param) === "www.pressreader.com/"
          ) {
            if ($this->blank("via")) {
              $this->set($param, "PressReader");
        ...
        case "via": // Should just remove all 'via' with no url, but do not want to make people angry
          if ($this->blank(ALL_URL_TYPES)) {
            // Include blank via
            if (
              stripos($this->get("via"), "PubMed") !== false &&
              ($this->has("pmc") || $this->has("pmid"))
            ) {
              $this->forget("via");
            } elseif (
              stripos($this->get("via"), "JSTOR") !== false &&
              $this->has("jstor")
            ) {
              $this->forget("via");
            } elseif (
              stripos($this->get("via"), "google book") !==
                false &&
              $this->has("isbn")
            ) {
              $this->forget("via");
          ...
          foreach (
            array_merge(["publisher"], WORK_ALIASES)
            as $others
          ) {
            if ($this->has($others)) {
              if (
                ...
                (stripos($this->get($others), "bbc") !==
                  false &&
                  stripos($this->get("via"), "bbc")) !== false
              ) {
                $this->forget("via");
                return;
              }
        ...
        case "volume":
          ...
          if (
            in_array($temp_string, PREFER_VOLUMES, true) &&
            $this->has("volume")
          ) {
            if ($this->get("volume") === $this->get("issue")) {
              $this->forget("issue");
            } elseif (
              $this->get("volume") === $this->get("number")
            ) {
              $this->forget("number");
            }
          }
          if (
            in_array($temp_string, PREFER_ISSUES, true) &&
            $this->has("volume")
          ) {
            if ($this->get("volume") === $this->get("issue")) {
              $this->forget("volume");
        ...
        case "year":
          ...
          if (
            preg_match("~\d\d*\-\d\d*\-\d\d*~", $this->get("year"))
          ) {
        ...
        case "number":
          ...
          if ($param === "issue" || $param === "number") {
            if (
              preg_match(
                '~^(?:iss\.|iss|issue|number|num|num\.|no|no:|no\.|№|№\.)\s*(\d+)$~iu',
                $value,
                $matches
              )
            ) {
        ...
        case "pp": # And case 'year': case 'issue':, following from previous
          ...
          if (
            preg_match('~^p\.? *(\d+[–-]?\d+)$~u', $value, $matches)
          ) {
            $value = $matches[1];
            $this->set($param, $value);
          }
          if (
            ...
            mb_ereg(REGEXP_TO_EN_DASH, $value) &&
            can_safely_modify_dashes($value) &&
            $pmatch[1] !== "page"
          ) {
            $this->mod_dashes = true;
            report_modification(
              "Upgrading to en-dash in " .
                echoable($param) .
                " parameter"
            );
        ...
        case "website":
          if (
            $this->get($param) === "Undefined" ||
            $this->get($param) === "undefined" ||
            $this->get($param) === "myprivacy.dpgmedia.nl"
          ) {
            $this->forget($param);
            return;
          }
          if (
            $this->wikiname() === "cite book" &&
            (str_i_same($this->get($param), "google.com") ||
          ...
          if (
            stripos($this->get($param), "archive.org") !== false &&
            stripos(
              $this->get("url") .
                $this->get("chapter-url") .
                $this->get("chapterurl"),
          ...
          if (
            strtolower($this->get($param)) === "latimes" ||
          ...
          if (
            strtolower($this->get($param)) === "nytimes" ||
          ...
          if (
            strtolower($the_param) === "www.sify.com" ||
          ...
          if (
            strtolower($the_param) === "www.bollywoodhungama.com" ||
            ...
            stripos($the_param, "bollywoodhungama") !== false ||
            stripos($the_param, "bollywood hungama") !== false
          ) {
            $authtmp = $this->get("author");
            if (
              $authtmp === "Bollywood Hungama News Network" ||
          ...
          if (
            strtolower($the_param) === "www.sciencedirect.com" ||
  ...
  public function final_tidy(): void
  {
    ...
    if ($this->should_be_processed()) {
      ...
      if (
        $this->has("chapter") &&
        $this->get("chapter") === $this->get("title")
      ) {
        // Leave only one
      ...
      if (
        $this->has("journal") &&
        str_equivalent($this->get("title"), $this->get("journal"))
      ) {
        if ($this->wikiname() === "cite book" || $this->has("isbn")) {
          $this->forget("journal");
        }
      ...
      if (
        $this->get("issue") === "n/a" &&
        preg_match('~^\d+$~', $this->get("volume"))
      ) {
        $this->forget("issue");
      }
      if (
        $this->get("volume") === "n/a" &&
        preg_match('~^\d+$~', $this->get("issue"))
      ) {
        $this->forget("volume");
      }
      if (
        ...
        $this->get("volume") ===
          $this->get_without_comments_and_placeholders("volume")
      ) {
        // No comments to flag problems
        $doi_template = $this->get_without_comments_and_placeholders(
          "doi"
        );
        ...
        if ($crossRef) {
          ...
          if ($possible_issue !== $possible_volume) {
            // They don't match
            if (
              (strpos($possible_issue, "-") > 0 ||
                (int) $possible_issue > 1) &&
              (int) $possible_volume > 0
            ) {
              // Legit data
              if ($possible_issue === $orig_data) {
                $this->set("volume", $possible_volume);
                report_action(
                  "Citation had volume and issue the same. Changing volume."
                );
              } elseif ($possible_volume === $orig_data) {
                $this->set("issue", $possible_issue);
                report_action(
                  "Citation had volume and issue the same. Changing issue."
                );
              } else {
                $doi_crossref = $crossRef->doi;
                if (
                  !is_string($doi_crossref) ||
                  strlen($doi_crossref) < 2
                ) {
                  $doi_crossref = $doi_template;
                }
                report_inaction(
                  "Citation for doi:" .
                    echoable($doi_crossref) .
                    " has volume and issue set to " .
                    echoable($orig_data) .
                    " which disagrees with CrossRef (volume " .
                    echoable($possible_volume) .
                    ", issue " .
                    echoable($possible_issue) .
                    ")"
                ); // @codeCoverageIgnore
              }
            }
      ...
      if ($this->has_good_free_copy()) {
        ...
        if (
          $url !==
          str_ireplace(["nih.gov", "pubmed", "pmc", "doi"], "", $url)
        ) {
          $this->get_identifiers_from_url();
        }
      }
      ...
      if (
        ...
        (strpos($this->get("isbn"), "978-0-19") === 0 ||
          strpos($this->get("isbn"), "978019") === 0 ||
          strpos($this->get("isbn"), "978-019") === 0)
      ) {
      ...
      if ($this->wikiname() === "cite web") {
        ...
        if (
          ($this->has("arxiv") || $this->has("eprint")) &&
          stripos($this->get("url"), "arxiv") !== false
        ) {
          if ($this->name === "cite web") {
      ...
      if (
        !$this->blank(DOI_BROKEN_ALIASES) &&
        $this->has("jstor") &&
        (strpos($this->get("doi"), "10.2307") === 0 ||
          $this->get("doi") === $this->get("jstor") ||
          substr($this->get("doi"), 0, -2) === $this->get("jstor") ||
          substr($this->get("doi"), 0, -3) === $this->get("jstor"))
      ) {
      ...
      if (
        $this->has("journal") &&
        stripos($this->get("journal"), "arxiv") === false
      ) {
        ...
        if ($this->blank(["chapter", "isbn"])) {
          ...
          report_forget(
            "Citation has chapter/ISBN already, dropping dubious Journal title: " .
              echoable($this->get("journal"))
          );
          $this->forget("journal");
        } else {
          report_warning(
            echoable(
              "Citation should probably not have journal = " .
                $this->get("journal") .
                " as well as chapter / ISBN " .
                $this->get("chapter") .
                " " .
                $this->get("isbn")
            )
      ...
      if (
        preg_match('~^10\.1093/ref\:odnb/\d+$~', $this->get("doi")) &&
        $this->has("title") &&
        $this->wikiname() !== "cite encyclopedia" &&
        $this->wikiname() !== "cite encyclopaedia"
      ) {
        ...
        foreach (
          array_diff(WORK_ALIASES, ["encyclopedia", "encyclopaedia"])
          as $worker
        ) {
          $this->forget($worker);
        }
      ...
      if (
        preg_match("~^10\.1093/~", $this->get("doi")) &&
        $this->has("title") &&
        ($this->wikiname() === "cite web" ||
          $this->wikiname() === "cite journal") &&
        $this->blank(WORK_ALIASES) &&
        ...
        if ($this->has("chapter")) {
          ...
          if (substr($this->name, 0, 1) === "c") {
            $this->name =
              $spacing[1] . "cite document" . $spacing[2];
          } else {
            $this->name =
              $spacing[1] . "Cite document" . $spacing[2];
          }
      ...
      if (
        ...
        foreach (WORK_ALIASES as $worky) {
          foreach (
            ["Conference", "Symposium", "SIGGRAPH", "workshop"]
            as $thingy
          ) {
            if (
              stripos($this->get("title"), $thingy) !== false &&
              stripos($this->get($worky), $thingy) !== false
            ) {
              $this->forget($worky);
            }
          }
      ...
      if (
        stripos($this->get("journal"), "SIGGRAPH") !== false &&
        stripos($this->get("title"), "SIGGRAPH") !== false
      ) {
        $this->forget("journal");
      }
      if (
      ...
      if (
        $this->has("series") &&
        stripos($this->get("title"), $this->get("series")) !== false &&
        $this->get("series") !== "Surtees Society" &&
        !preg_match('~^\d+$~', $this->get("series"))
      ) {
        $this->forget("series");
      }
      ...
      if ($this->wikiname() === "cite book") {
        foreach (WORK_ALIASES as $worky) {
          ...
          if (
            strtolower($this->get("publisher")) ===
            strtolower($this->get($worky))
          ) {
            $this->forget($worky);
          }
        }
    ...
    **/
      ...
      if (!empty($this->param)) {
        ...
        if (!$this->incomplete()) {
          $drop_me_maybe = array_merge(
            $drop_me_maybe,
            LOTS_OF_EDITORS
          ); // Always drop empty editors at end, if "complete"
        }
        // Do it this way to avoid massive N*M work load (N=size of $param and M=size of $drop_me_maybe) which happens when checking if each one is blank
        foreach ($this->param as $key => $p) {
          if (
        ...
        foreach ($this->param as $p) {
          ...
          if ($alias !== "" && $this->blank($alias)) {
            if (
              ...
              if (
                $this->blank(
                  AUTHOR_PARAMETERS[(int) $matches[1]]
                )
              ) {
                $this->forget($alias);
              }
      ...
      if (
        ...
        if (
          ...
          $hostname_plus = (string) preg_replace(
            "~^(m\.|www\.)~",
            "",
            $hostname_plus
          );
          ...
          if (
            str_ireplace(
              CANONICAL_PUBLISHER_URLS,
              "",
              $hostname
            ) === $hostname &&
            str_ireplace(
              PROXY_HOSTS_TO_ALWAYS_DROP,
              "",
              $hostname
            ) === $hostname &&
            str_ireplace(PROXY_HOSTS_TO_DROP, "", $hostname) ===
              $hostname &&
            str_ireplace(HOSTS_TO_NOT_ADD, "", $hostname) ===
              $hostname
          ) {
  ...
  public function verify_doi(): bool
  {
    ...
    if ($chatty) {
      report_info(
        "Checking that DOI " . echoable($doi) . " is operational..."
      );
    }
    ...
    if ($doi_status === null) {
      report_warning(
        "DOI status unknown.  doi.org failed to respond to: " .
          doi_link($doi)
      ); // @codeCoverageIgnore
          ...
          if ($author_cardinality === 1) {
            ...
            if (under_two_authors($val_base)) {
              ...
              foreach ($authors as $i => $author_name) {
                $this->add_if_new(
                  "author" . (string) ((int) $i + 1),
                  format_author($author_name)
                );
              }
            }
          }
  ...
  private function number_of_authors(): int
  {
    ...
    foreach ($this->param as $p) {
      if (
        preg_match(
          "~(?:author|last|first|forename|initials|surname|given)(\d+)~",
          $p->param,
          $matches
        )
      ) {
    ...
    if ($max === 0) {
      foreach ($this->param as $p) {
        if (
          preg_match(
            '~(?:author|last|first|forename|initials|surname|given)$~',
            $p->param
          )
        ) {
          if (stripos($p->param, "editor") === false) {
  ...
  private function first_surname(): string
  {
    ...
    if (
      preg_match("~[^.,;\s]{2,}~u", $this->first_author(), $first_author)
    ) {
  ...
  public function rename(
    ...
    if ($old_param === "url" && $new_param === "chapter-url") {
      ...
      ($old_param === "chapter-url" || $old_param === "chapterurl") &&
      $new_param === "url"
    ) {
  ...
  public function get_without_comments_and_placeholders(string $name): string
  {
    ...
    $ret = safe_preg_replace(
      "~# # # CITATION_BOT_PLACEHOLDER.*?# # #~sui",
      "",
      $ret
    ); // Other place holders already escaped. Case insensitive
  ...
  public function set(string $par, string $val): bool
  {
    if ($par === "") {
      report_error(
        "null parameter passed to set with value of " . echoable($val)
      );
    }
    if (
      mb_stripos($this->get($par), "CITATION_BOT_PLACEHOLDER_COMMENT") !==
        false &&
      $par !== "ref"
    ) {
      return false;
  ...
  private function forgetter(string $par, bool $echo_forgetting): void
  {
    ...
    if (!$this->blank($par)) {
      ...
      if ($par === "url") {
        ...
        if (
          $this->has("work") &&
          stripos($this->get("work"), "www.") === 0
        ) {
          $this->forgetter("work", $echo_forgetting);
        }
        if (
          $this->blank(array_diff(WORK_ALIASES, ["website"])) &&
          bad_10_1093_doi($this->get("doi"))
        ) {
    ...
    if ($pos !== null) {
      if (
        $echo_forgetting &&
        $this->has($par) &&
        stripos($par, "CITATION_BOT_PLACEHOLDER") === false
      ) {
    ...
    if (
      strpos($par, "url") !== false &&
      $this->wikiname() === "cite web" &&
      $this->blank(array_diff(ALL_URL_TYPES, [$par]))
    ) {
      if ($this->has("journal")) {
        $this->change_name_to("cite journal");
        ...
        !$this->blank(["isbn", "lccn", "oclc", "ol", "chapter"])
      ) {
        $this->change_name_to("cite book");
  ...
  private function isbn10Toisbn13(
    string $isbn10,
    bool $ignore_year = false
  ): string {
  ...
  private function get_inline_doi_from_title(): void
  {
    if (
      preg_match(
        "~(?:\s)*(?:# # # CITATION_BOT_PLACEHOLDER_TEMPLATE )(\d+)(?: # # #)(?:\s)*~i",
        $this->get("title"),
        $match
      )
    ) {
      if (
        $inline_doi = self::$all_templates[
          $match[1]
        ]->inline_doi_information()
      ) {
        if ($this->add_if_new("doi", trim($inline_doi[0]))) {
          ...
          quietly(
            "report_modification",
            "Converting inline DOI to DOI parameter"
          );
  ...
  private function volume_issue_demix(string $data, string $param): void
  {
    ...
    if (
      in_array(
        $this->wikiname(),
        ["cite encyclopaedia", "cite encyclopedia", "cite book"],
        true
      )
    ) {
      return;
    ...
    if (
      ...
      preg_match(
        "~^(?:vol\. |Volume |vol |vol\.|)(\d+)[,\s]\s*(?:no\.|number|issue|Iss.|no )\s*(\d+(-|–|\–|\{\{ndash\}\})?\d*)$~i",
        $data,
        $matches
      ) ||
      ...
      if (
        $this->year() !== "" &&
        strpos($possible_issue, $this->year()) !== false
      ) {
        return;
      }
      ...
      if ($param === "volume") {
        if ($this->blank(ISSUE_ALIASES)) {
          ...
          str_replace(".", "", $this->get("issue")) ===
            str_replace(".", "", $possible_issue) ||
          str_replace(".", "", $this->get("number")) ===
            str_replace(".", "", $possible_issue)
        ) {
          $this->set("volume", $possible_volume);
        }
    ...
    if (
      ...
      ($this->has("chapter") ||
        $this->has("isbn") ||
        strpos($this->rawtext, "archive.org") !== false)
    ) {
      return;
    }
    ...
    if (
      !in_array(
        $this->wikiname(),
        ["citation", "cite journal", "cite web", "cite magazine"],
        true
      ) &&
      $this->get_without_comments_and_placeholders("issue") === "" &&
    ...
    if ($param === "volume") {
      if (
        preg_match(
          "~^(?:vol\.|volume\s+|vol\s+|vol:)\s*([\dLXVI]+)$~i",
          $data,
          $matches
        )
      ) {
    ...
    if ($param === "issue" || $param === "number") {
      if (
        preg_match(
          "~^(?:num\.|number\s+|num\s+|num:|number:|iss\.|issue\s+|iss\s+|iss:|issue:)\s*([\dLXVI]+)$~i",
          $data,
          $matches
        )
      ) {
      ...
      if ($param === "volume") {
        if (
          ...
          if ($this->blank(["issue", "number"])) {
            ...
            stripos(
              $this->get("issue") . $this->get("number"),
              $data
            ) !== false
          ) {
            $this->forget($param); // Duplicate data
  ...
  private function should_url2chapter(bool $force): bool
  {
    ...
    if (
      preg_match("~archive\.org/details/.+/page/n(\d+)~", $url, $matches)
    ) {
      if ((int) $matches[1] < 16) {
        return false;
    ...
    if (
      preg_match(
        '~10\.\1007\/97[89]-?[0-9]{1,5}\-?[0-9]+\-?[0-9]+\-?[0-9]\_\d{1,3}~',
        $url
      )
    ) {
      return true;
  ...
  public function clean_cite_odnb(): void
  {
    if ($this->has("url")) {
      while (
        preg_match(
          "~^(https?://www\.oxforddnb\.com/.+)(?:\;jsession|\?rskey|\#)~",
          $this->get("url"),
          $matches
        )
      ) {
    ...
    if ($this->has("doi")) {
      ...
      if (doi_works($doi) === false) {
        if (
          preg_match(
            "~^10\.1093/(?:\S+odnb-9780198614128-e-|ref:odnb|odnb/9780198614128\.013\.|odnb/)(\d+)$~",
            $doi,
            $matches
          )
        ) {
  ...
  public function has_good_free_copy(): bool
  {
    ...
    if (
      ($this->has("pmc") &&
        $this->blank("pmc-embargo-date") &&
        preg_match('~^\d+$~', $this->get("pmc"))) ||
      ($this->has("doi") &&
        $this->get("doi-access") === "free" &&
        $this->blank(DOI_BROKEN_ALIASES) &&
        doi_works($this->get("doi")))
