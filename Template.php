...
final class Template {
  ...
  public function get_identifiers_from_url($url_sent = NULL) {
    ...
    if (stripos($url, "jstor.org") !== FALSE) {
      ...
      } elseif (preg_match("~^https?://search\.proquest\.com/docvie/(\d{4,})$~i", $url, $match) && $this->blank('id')) {
          quietly('report_modification', 'Converting URL to ProQuest parameter');
          if (is_null($url_sent)) {
             $this->forget($url_type);
          }
          return $this->add('id', '{{ProQuest|' . $match[1] . '}}');  // Not 'add_if_new', since that formats it, and is blank
      }
    }
    return FALSE ;
  }
