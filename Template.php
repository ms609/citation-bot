  ...
  public function get_identifiers_from_url($url_sent = NULL) {
    ...
    if ($doi = extract_doi($url)[1]) {
      ...
      if ($this->has('doi') && mb_stripos($doi, $this->get('doi')) === 0) { // DOIs are case-insensitive
        if (doi_active($doi) && is_null($url_sent) && mb_strpos(strtolower($url), ".pdf") === FALSE && mb_strpos($url, "10.1093/") === FALSE && !preg_match('~^10.[^/]+/\(ISSN\)\d{3}-\d{3}[xX0-9]$~', $doi)) {
      ...
      if ($this->add_if_new('doi', $doi)) {
        if (doi_active($doi)) {
          if (is_null($url_sent)) {
            if (mb_strpos(strtolower($url), ".pdf") === FALSE && mb_strpos($url, "10.1093/") === FALSE && !preg_match('~^10.[^/]+/\(ISSN\)\d{3}-\d{3}[xX0-9]$~', $doi)) {
      ...
      if (mb_stripos($url, $this->get('doi')) !== FALSE) { // DOIs are case-insensitive
        if (doi_active($this->get('doi')) && is_null($url_sent) && mb_strpos(strtolower($url), ".pdf") === FALSE && mb_strpos($url, "10.1093/") === FALSE && !preg_match('~^10.[^/]+/\(ISSN\)\d{3}-\d{3}[xX0-9]$~', $this->get('doi'))) {
          report_forget("Recognized existing DOI in URL; dropping URL");
          $this->forget($url_type);
        }
        return FALSE;  // URL matched existing DOI, so we did not use it
