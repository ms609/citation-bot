    ...
    if ((strlen($p->param) > 0) && !in_array($p->param, PARAMETER_LIST)) {
      ...
      */
      ...
      if ($shortest < 12 && $shortest < $shortish) {
        $p->param = $closest;
        echo " replaced with $closest (likelihood " . (12 + 12 - $shortest) . "/24)"; // Scale arbitrarily re-based by adding 12 so users are more impressed by size of similarity
      } else {
        $similarity = similar_text($p->param, $closest) / strlen($p->param);
        if ($similarity > 0.6) {
          $p->param = $closest;
          echo " replaced with $closest (similarity " . (12 + round(12 * $similarity, 1)) . "/24)"; // Scale arbitrarily re-based by adding 12 so users are more impressed by size of similarity
        } else {
