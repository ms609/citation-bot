  ...
  public function add_if_new($param_name, $value, $api = NULL) {
    ...
    switch ($param_name) {
      ...
      case "page": case "pages":
        ...
        if (is_int($value) && intval($value) > 100000) return FALSE;  // Sometimes get HUGE values
    ...
    if (stripos($url, "jstor.org") !== FALSE) {
      ...
      if (stripos($url, 'plants.jstor.org')) {
        return FALSE; # Plants database, not journal
                ...
                preg_match("~^https?://(?:www\.)?jstor\.org\S+(?:stable|discovery)/(?:10\.7591/|)(\d{5,}|(?:j|J|histirel)\.[a-zA-Z0-9\.]+)$~", $url, $match)) {
        if (is_null($url_sent)) {
          $this->forget($url_type);
        }
        if ($this->get('jstor')) {
      ...
      switch ($pmatch[1]) {
        ...
        case 'archiveurl':
          if ($this->blank(['archive-date', 'archivedate'])) {
            if (preg_match('~^https?://(?:web\.archive\.org/web/|archive\.today/|archive\.\S\S/|webarchive\.loc\.gov/all/|www\.webarchive\.org\.uk/wayback/archive/)(\d{4})(\d{2})(\d{2})\d{6}/http~', $this->get($param), $matches)) {
