    ...
    if ($template->blank(['chapter', 'series']) &&
      ...
      if (stripos($archive_url, 'archive') !==FALSE) {
        ...
        if ($raw_html && (
          ...
          preg_match('~^\s*<html[\S\s]+<head[\S\s]+?<!-- End Wayback Rewrite JS Include -->\s*?<!-- WebPoet(tm) Web Page Pull\S+?-->[\S\s]+?<title>([\S\s]+?)<\/title>[\S\s]+?head[\S\s]+?<body~i', $raw_html, $match)
        )) {
          $title = trim($match[1]);
          if (stripos($title, 'archive') === FALSE &&
            ...
            if ($good_title) {
              ...
              if ($new === '') {
                 ...
                 $bad_count = mb_substr_count($new, '�') + mb_substr_count($new, '$') + mb_substr_count($new, '%');
                 if ($bad_count > 5) {
                     $template->set('title', $old); // UTF-8 craziness
                 }
              }
