...
final class Template
{
    ...
    public function final_tidy(): void {
        ...
        if ($this->should_be_processed()) {
            ...
            // Compact author-numbered parameters to ensure contiguous numbering
            $this->compact_author_numbers();
        }
    ...
    /**
     * Compact author-numbered parameters to be contiguous starting at 1.
     * This fixes gaps that can occur when filtering removes some authors but
     * leaves others with non-contiguous numbering (e.g., last2/first2 without last1).
     */
    public function compact_author_numbers(): void {
        // Define the author-related parameter suffixes we need to handle
        // These match the numbered parameters in constants/parameters.php
        $author_param_bases = [
            'last', 'first', 'author',
            'surname', 'forename', 'initials',
            'author-last', 'author-first',
            'author-given', 'given', 'author-surname',
        ];

        // Scan for authors from 1 to 35 (matches MAX in constants)
        $max_author_slots = 35;
        $author_slots = [];

        // First pass: collect all existing author slots
        for ($i = 1; $i <= $max_author_slots; $i++) {
            $has_data = false;
            $slot_data = [];

            // Check each parameter base for this slot number
            foreach ($author_param_bases as $base) {
                $param_name = $base . (string) $i;
                if ($this->has($param_name)) {
                    $has_data = true;
                    $slot_data[$param_name] = $this->get($param_name);
                }
            }

            // Also check the authorN-last and authorN-first variants
            $author_n_last = 'author' . (string) $i . '-last';
            $author_n_first = 'author' . (string) $i . '-first';
            $author_n_given = 'author' . (string) $i . '-given';
            $author_n_surname = 'author' . (string) $i . '-surname';

            if ($this->has($author_n_last)) {
                $has_data = true;
                $slot_data[$author_n_last] = $this->get($author_n_last);
            }
            if ($this->has($author_n_first)) {
                $has_data = true;
                $slot_data[$author_n_first] = $this->get($author_n_first);
            }
            if ($this->has($author_n_given)) {
                $has_data = true;
                $slot_data[$author_n_given] = $this->get($author_n_given);
            }
            if ($this->has($author_n_surname)) {
                $has_data = true;
                $slot_data[$author_n_surname] = $this->get($author_n_surname);
            }

            if ($has_data) {
                $author_slots[$i] = $slot_data;
            }
        }

        // If no gaps detected (slots are already contiguous from 1), nothing to do
        if (empty($author_slots)) {
            return;
        }

        $slot_numbers = array_keys($author_slots);
        sort($slot_numbers);

        // Check if already contiguous starting from 1
        $is_contiguous = true;
        for ($i = 0; $i < count($slot_numbers); $i++) {
            if ($slot_numbers[$i] !== ($i + 1)) {
                $is_contiguous = false;
                break;
            }
        }
        ...
        if ($is_contiguous) {
            return; // Already properly numbered
        }

        // Second pass: renumber to be contiguous starting at 1
        $new_slot_number = 1;
        foreach ($slot_numbers as $old_slot_number) {
            ...
            // Rename all parameters in this slot to the new slot number
            foreach ($author_slots[$old_slot_number] as $param_name => $value) {
                // Extract the base parameter name (without the number)
                if (preg_match('~^(author)(\d+)(-(?:last|first|given|surname))$~', $param_name, $matches)) {
                    // Handle authorN-suffix format
                    $new_param_name = $matches[1] . (string) $new_slot_number . $matches[3];
                } else {
                    // Handle suffixN format (last1, first1, etc.)
                    $new_param_name = preg_replace('~\d+$~', (string) $new_slot_number, $param_name);
                }

                // Rename the parameter
                $this->rename($param_name, $new_param_name);
            }

            $new_slot_number++;
        }

        // Third pass: remove any orphaned high-numbered parameters beyond the last used slot
        $last_used_slot = count($slot_numbers);
        for ($i = $last_used_slot + 1; $i <= $max_author_slots; $i++) {
            foreach ($author_param_bases as $base) {
                $param_name = $base . (string) $i;
                if ($this->has($param_name)) {
                    $this->forget($param_name);
            ...
            // Also clean up the authorN-suffix variants
            $this->forget('author' . (string) $i . '-last');
            $this->forget('author' . (string) $i . '-first');
            $this->forget('author' . (string) $i . '-given');
            $this->forget('author' . (string) $i . '-surname');
        }
    }
