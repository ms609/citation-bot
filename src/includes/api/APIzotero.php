    ...
    public static function process_zotero_response(string $zotero_response, Template $template, string $url, int $access_date): void {
        ...
        if (!isset($result->title)) {
            ...
            if (mb_strpos($zotero_response, 'unknown_error') !== false) { // @codeCoverageIgnoreStart
                ...
                report_minor_error("This URL cannot be encoded for the Zotero endpoint: " . $the_url); /* See Islam in New Zealand on en.  Also note the the wiki output formats work, but give different output.  https://phabricator.wikimedia.org/T413651  TODO: fix it if they do not */
            } else {
        ...
        // already identified as interviewer/subject via interviewer-last/first fields
        if ($template->has('interviewer-last') || $template->has('interviewer-first')) {
            unset($result->author);
            unset($result->creators);
        }
