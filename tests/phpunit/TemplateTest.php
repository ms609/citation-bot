...
class expandFnsTest extends PHPUnit_Framework_TestCase {
  ...
  public function testAmazonExpansion() {
    ...
    $parsed_text = $this->template_parse_text_helper($text);
    $this->assertEquals($parsed_text->expand_text(), '{{Cite book|asin = 0226845494}}'); 
  }
  public function testDoiExpansion() {
    $text = "{{Cite web | http://onlinelibrary.wiley.com/doi/10.1111/j.1475-4983.2012.01203.x/abstract}}";
    $parsed_text = $this->template_parse_text_helper($text);
    $this->assertEquals($parsed_text->expand_text(), '{{Cite journal | doi=10.1111/j.1475-4983.2012.01203.x}}'); 
  }
  public function testGarbageRemoval() {
    ...
    $parsed_text = $this->template_parse_text_helper($text);
    $this->assertEquals($parsed_text->expand_text(), '{{Cite journal| pages=10–11| edition = 3rd | journal=My Journal | issn=1234-4321 }}');
  }
  public function testEtAlHandling() {
    $text = "{{Cite book | authors=Smith, A; Jones, B; Western, C., et al.}}";
    $parsed_text = $this->template_parse_text_helper($text);
    $this->assertEquals($parsed_text->expand_text(), '{{Cite book | last1=Smith| first1=A|last2 = Jones|first2 = B|last3 = Western|first3 = C.|author4 = and others|displayauthors = 3}}'); 
  }
  public function testGoogleBooksExpansion() {
    ...
    $parsed_text = $this->template_parse_text_helper($text);
    $this->assertEquals($parsed_text->expand_text(), '{{Cite book| url=http://books.google.com/?id=SjpSkzjIzfsC| title=Wonderful Life: The Burgess Shale and the Nature of History| isbn=9780393307009| author1=Gould| first1=Stephen Jay| year=1990}}}');
  }
}
