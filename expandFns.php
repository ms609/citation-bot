...
function doi_active(string $doi): ?bool
{
    ...
    if ($works === null) {
        // Temporary problem - do not cache
...
function doi_works(string $doi): ?bool
{
    ...
    if ($works === null) {
        if (isset(NULL_DOI_LIST[$doi])) {
            // These are know to be bad, so only check one time during run
...
function is_doi_active(string $doi): ?bool
{
    ...
    $err =
        "CrossRef server error loading headers for DOI " .
        echoable($doi . " : " . (string) $response_code);
    bot_debug_log($err);
    report_warning($err);
    return null; // @codeCoverageIgnoreEnd
}
...
function throttle_dx(): void
{
    ...
    if ($left > 0 && $left < $min_time) {
        usleep($left);
    } // less than min_time is paranoia, but do not want an inifinite delay
    $last = $now;
}
...
function is_doi_works(string $doi): ?bool
{
    ...
    if (isset(NULL_DOI_ANNOYING[$doi])) {
        return false;
    } // TODO - this array should be checked from time to time
    ...
    if (strpos($registrant, "10.") === 0) {
        // We have to deal with valid handles in the DOI field - very rare, so only check actual DOIs
        $registrant = substr($registrant, 3);
        if (preg_match('~^[^1-3]\d\d\d\d\.\d\d*$~', $registrant)) {
            return false;
        } // 5 digits with subcode (0xxxx, 40000+); accepts: 10000–39999
        if (preg_match('~^[^1-6]\d\d\d\d$~', $registrant)) {
            return false;
        } // 5 digits without subcode (0xxxx, 60000+); accepts: 10000–59999
        if (preg_match('~^[^1-9]\d\d\d\.\d\d*$~', $registrant)) {
            return false;
        } // 4 digits with subcode (0xxx); accepts: 1000–9999
        if (preg_match('~^[^1-9]\d\d\d$~', $registrant)) {
            return false;
        } // 4 digits without subcode (0xxx); accepts: 1000–9999
        if (preg_match("~^\d\d\d\d\d\d+~", $registrant)) {
            return false;
        } // 6 or more digits
        if (preg_match('~^\d\d?\d?$~', $registrant)) {
            return false;
        } // less than 4 digits without subcode (3 digits with subcode is legitimate)
        if (preg_match("~^\d\d?\.[\d\.]+~", $registrant)) {
            return false;
        } // 1 or 2 digits with subcode
        if ($registrant === "5555") {
            return false;
        } // test registrant will never resolve
        if (preg_match("~[^\d\.]~", $registrant)) {
            return false;
        } // any character that isn't a digit or a dot
    }
    throttle_dx();
    ...
    if ($headers_test === false) {
        if (strpos($doi, "10.2277/") === 0) {
            return false;
        } // Rogue
        if (preg_match('~^10\.1038/nature\d{5}$~i', $doi)) {
            return false;
        } // Nature dropped the ball
        if (stripos($doi, "10.17312/harringtonparkpress/") === 0) {
            return false;
        }
        if (stripos($doi, "10.3149/csm.") === 0) {
            return false;
        }
        if (stripos($doi, "10.5047/meep.") === 0) {
            return false;
        }
        if (stripos($doi, "10.4435/BSPI.") === 0) {
            return false;
        }
        if (isset(NULL_DOI_LIST[$doi])) {
            return null;
    ...
    if ($headers_test === false) {
        // most likely bad - note that null means do not add or remove doi-broken-date from pages
        return null; // @codeCoverageIgnore
    }
    if (stripos($doi, "10.1126/scidip.") === 0) {
        if ((string) @$headers_test["1"] === "HTTP/1.1 404 Forbidden") {
            unset($headers_test["1"]);
        } // https://doi.org/10.1126/scidip.ado5059
    }
...
function interpret_doi_header(array $headers_test): ?bool
{
    ...
    if (strpos($resp2, "40") !== false) {
        ...
        if ($resp1 === "HTTP/1.1 301 Moved Permanently") {
            $resp2 = "";
        } // https://doi.org/10.5152/UCD.2016.3683
    }

    if (
        stripos($resp0 . $resp1 . $resp2, "404 Not Found") !== false ||
        stripos($resp0 . $resp1 . $resp2, "HTTP/1.1 404") !== false
    ) {
        return false;
    } // Bad
    if (
        stripos($resp0, "302 Found") !== false ||
        stripos($resp0, "HTTP/1.1 302") !== false
    ) {
        return true;
    } // Good
    ...
    if (
        stripos($resp0, "301 Moved Permanently") !== false ||
        stripos($resp0, "HTTP/1.1 301") !== false
    ) {
        // Could be DOI change or bad prefix
        if (
            stripos($resp1, "302 Found") !== false ||
            stripos($resp1, "HTTP/1.1 302") !== false
        ) {
            return true; // Good
        } elseif (
            stripos($resp1, "301 Moved Permanently") !== false ||
            stripos($resp1, "HTTP/1.1 301") !== false
        ) {
            // @codeCoverageIgnoreStart
            if (
                stripos($resp2, "200 OK") !== false ||
                stripos($resp2, "HTTP/1.1 200") !== false
            ) {
                return true;
            } else {
                return false;
    ...
    report_minor_error(
        "Unexpected response in is_doi_works " . echoable($resp0)
    );
    return null; // @codeCoverageIgnoreEnd
}
...
{
    if (
        isset($headers_test["Location"][0]) &&
        is_array(@$headers_test["Location"])
    ) {
        // Should not be an array, but on rare occasions we get one
        ...
        // @codeCoverageIgnoreStart
        bot_debug_log(
            "Got weird header from handle: " .
                echoable(print_r($headers_test, true))
...
function query_jstor_api(array $ids, array &$templates): void
{
    // $ids not used   // Pointer to save memory
...
function sanitize_doi(string $doi): string
{
    if (substr($doi, -1) === ".") {
        ...
        if (doi_works($try_doi)) {
            // If it works without dot, then remove it
            $doi = $try_doi;
        } elseif (doi_works($try_doi . ".x")) {
            // Missing the very common ending .x
            $doi = $try_doi . ".x";
        } elseif (!doi_works($doi)) {
            // It does not work, so just remove it to remove wikipedia error.  It's messed up
            $doi = $try_doi;
        }
    }
...
function extract_doi(string $text): array
{
    if (
        ...
        while (
            preg_match(REGEXP_DOI, $doi_candidate) &&
            !doi_works($doi_candidate)
        ) {
            $last_delimiter = 0;
            foreach (["/", ".", "#", "?"] as $delimiter) {
                $delimiter_position = (int) strrpos($doi_candidate, $delimiter);
                $last_delimiter =
                    $delimiter_position > $last_delimiter
                        ? $delimiter_position
                        : $last_delimiter;
            }
        ...
        if (!doi_works($doi) && !doi_works(sanitize_doi($doi))) {
            // Reject URLS like ...../25.10.2015/2137303/default.htm
            if (preg_match("~^10\.([12]\d{3})~", $doi, $new_match)) {
                if (
                    preg_match("~[0-3][0-9]\.10\." . $new_match[1] . "~", $text)
                ) {
                    return ["", ""];
                }
            }
...
function wikify_external_text(string $title): string
{
    $replacement = [];
    $placeholder = [];
    $title = safe_preg_replace_callback(
        '~(?:\$\$)([^\$]+)(?:\$\$)~iu',
    ...
    if (
        ...
        $title = str_replace(
            ['<mo stretchy="false">', "<mo stretchy='false'>"],
            "",
            $title
        );
    }
    ...
    if (mb_substr($title, -1) === ".") {
        ...
        if (mb_substr_count($title, ".") === 1) {
            // Only one period
            $title = mb_substr($title, 0, -1);
        } elseif (mb_substr_count($title, " ") === 0) {
            // No spaces at all and multiple periods
        } else {
            // Multiple periods and at least one space
            $last_word_start = (int) mb_strrpos(" " . $title, " ");
            $last_word = mb_substr($title, $last_word_start);
            if (
                mb_substr_count($last_word, ".") === 1 && // Do not remove if something like D.C. or D. C.
    ...
    while ($title !== $title_orig) {
        ...
        $title = safe_preg_replace_callback(
            '~(?:<Emphasis Type="Italic">)([^<]+)(?:</Emphasis>)~iu',
            static function (array $matches): string {
                return "''" . $matches[1] . "''";
            },
            $title
        ...
        $title = safe_preg_replace_callback(
            '~(?:<Emphasis Type="Bold">)([^<]+)(?:</Emphasis>)~iu',
            static function (array $matches): string {
                return "'''" . $matches[1] . "'''";
            },
            $title
...
function restore_italics(string $text): string
{
    ...
    $text = str_replace(
        "xAzathioprine therapy for patients with systemic lupus erythematosus",
        "Azathioprine therapy for patients with systemic lupus erythematosus",
        $text
    ); // Annoying stupid bad data
    $text = trim(
        str_replace(
            ["        ", "      ", "    ", "   ", "  "],
            [" ", " ", " ", " ", " "],
    ...
    while (
        ...
        $text = str_replace(
            $matches[0],
            $matches[1] . " ''" . $matches[2] . "''" . $pad . $matches[3],
            $text
        );
    }
    $text = trim(
    ...
    if (str_replace(CAMEL_CASE, "", $padded) !== $padded) {
        return $text;
    } // Words with capitals in the middle, but not the first character
...
function sanitize_string(string $str): string
{
    ...
    $replacement = [];
    $placeholder = [];
    $math_templates_present = preg_match_all(
        "~<\s*math\s*>.*<\s*/\s*math\s*>~",
        $str,
        $math_hits
    );
    ...
    $clean = ["&#91;", "&#93;", "&#124;", "&#123;", "&#125;", " what's "];
    $str = trim(
        str_replace($dirty, $clean, safe_preg_replace('~[;.,]+$~', "", $str))
    );
    if ($math_templates_present) {
...
function str_remove_irrelevant_bits(string $str): string
{
    ...
    $str = safe_preg_replace(
        "~\d?\d? ?The ?sequence ?of ?\S+ ?has ?been ?deposited ?in ?the ?GenBank ?database ?under ?accession ?number ?\S+ ?\d?~i",
        "",
        $str
    );
    $str = safe_preg_replace(
        '~(?:\:\.\,)? ?(?:an|the) official publication of the.+$~i',
        "",
        $str
    );
    $str = trim($str);
...
{
    return str_i_same(
        str_remove_irrelevant_bits($str1),
        str_remove_irrelevant_bits($str2)
    );
}
...
function titles_are_similar(string $title1, string $title2): bool
{
    ...
    $title1 = str_replace(
        "�",
        "",
        str_replace(array_keys(MAP_DIACRITICS), "", $title1)
    );
    $title2 = str_replace(
        "�",
        "",
        str_replace(array_keys(MAP_DIACRITICS), "", $title2)
    );
    if (!titles_are_dissimilar($title1, $title2)) {
        return true;
    }
...
function titles_are_dissimilar(string $inTitle, string $dbTitle): bool
{
    ...
    if (stripos($inTitle, "CITATION_BOT_PLACEHOLDER_") !== false) {
        $possible = preg_replace(
            "~# # # CITATION_BOT_PLACEHOLDER_[A-Z]+ \d+ # # #~isu",
            " ",
            $inTitle
        );
        if ($possible !== null) {
            ...
            // When PHP fails with unicode, try without it
            $inTitle = preg_replace(
                "~# # # CITATION_BOT_PLACEHOLDER_[A-Z]+ \d+ # # #~i",
                " ",
                $inTitle
            ); // @codeCoverageIgnore
            if ($inTitle === null) {
...
function titles_simple(string $inTitle): string
{
    ...
    $inTitle2 = safe_preg_replace_callback(
        "~^(?:Chapter \d+ \- )(.....+)~iu",
        static function (array $matches): string {
            return $matches[1];
        },
...
{
    return str_replace(
        array_keys(MAP_DIACRITICS),
        array_values(MAP_DIACRITICS),
        $input
    );
}

function straighten_quotes(string $str, bool $do_more): string
{
    // (?<!\') and (?!\') means that it cannot have a single quote right before or after it
    ...
    $str = safe_preg_replace(
        '~(?<!\')&#821[679];|&#39;|&#x201[89];|[\x{FF07}\x{2018}-\x{201B}`]|&[rl]s?[b]?quo;(?!\')~u',
        "'",
        $str
    );
    if (
        (mb_strpos($str, "&rsaquo;") !== false &&
            mb_strpos($str, "&[lsaquo;") !== false) ||
        (mb_strpos($str, "\x{2039}") !== false &&
            mb_strpos($str, "\x{203A}") !== false) ||
        (mb_strpos($str, "‹") !== false && mb_strpos($str, "›") !== false)
    ) {
        // Only replace single angle quotes if some of both
        $str = safe_preg_replace(
            "~&[lr]saquo;|[\x{2039}\x{203A}]|[‹›]~u",
            "'",
            $str
        ); // Websites tiles: Jobs ›› Iowa ›› Cows ›› Ames
    }
    $str = safe_preg_replace(
        "~&#822[013];|[\x{201C}-\x{201F}]|&[rlb][d]?quo;~u",
        '"',
        $str
    );
    if (
        (mb_strpos($str, "&raquo;") !== false &&
            mb_strpos($str, "&laquo;") !== false) ||
        (mb_strpos($str, "\x{00AB}") !== false &&
            mb_strpos($str, "\x{00AB}") !== false) ||
        (mb_strpos($str, "«") !== false && mb_strpos($str, "»") !== false)
    ) {
        // Only replace double angle quotes if some of both // Websites tiles: Jobs » Iowa » Cows » Ames
        if ($do_more) {
            $str = safe_preg_replace(
                "~&[lr]aquo;|[\x{00AB}\x{00BB}]|[«»]~u",
                '"',
                $str
            );
        } else {
            // Only outer funky quotes, not inner quotes
            if (
                preg_match(
                    "~^(?:&laquo;|&raquo;|\x{00AB}|\x{00BB}|«|»)~u",
                    $str
                ) &&
                preg_match(
                    '~(?:&laquo;|&raquo;|\x{00AB}|\x{00BB}|«|»)$~u',
                    $str
                ) // Only if there is an outer quote on both ends
            ) {
                $str = safe_preg_replace(
                    "~^(?:&laquo;|&raquo;|\x{00AB}|\x{00BB}|«|»)~u",
                    '"',
...
function title_capitalization(string $in, bool $caps_after_punctuation): string
{
    ...
    if (
        mb_substr($new_case, 0, 1) === "[" &&
        mb_substr($new_case, -1) === "]"
    ) {
    ...
    if (
        stripos($new_case, "www.") !== false ||
        stripos($new_case, "www-") !== false ||
        stripos($new_case, "http://") !== false
    ) {
        return $new_case; // Who knows - duplicate code above
    }

    if (
        $new_case === mb_strtoupper($new_case) &&
    ...
    $new_case = safe_preg_replace_callback(
        "~[^\w&][b-df-hj-np-tv-xz]{3,}(?=\W)~ui",
        static function (array $matches): string {
            return mb_strtoupper($matches[0]);
        }, // Three or more consonants.  NOT Y
        $new_case
    );
    $new_case = safe_preg_replace_callback(
        "~[^\w&][aeiou]{3,}(?=\W)~ui",
        static function (array $matches): string {
            return mb_strtoupper($matches[0]);
        }, // Three or more vowels.  NOT Y
        $new_case
    );
    $new_case = mb_substr($new_case, 1, -1); // Remove added spaces

    $new_case = mb_substr(
        str_replace(UC_SMALL_WORDS, LC_SMALL_WORDS, " " . $new_case . " "),
        1,
        -1
    );
    foreach (UC_SMALL_WORDS as $key => $_value) {
        ...
        foreach ([": ", ", ", ". ", "; "] as $char) {
            $new_case = str_replace(
                mb_substr($upper, 0, -1) . $char,
                mb_substr($lower, 0, -1) . $char,
                $new_case
            );
        }
    }
    ...
    if (
        $caps_after_punctuation ||
        substr_count($in, ".") / strlen($in) > 0.07
    ) {
        ...
        $new_case = safe_preg_replace_callback(
            "~[?.:!/]\s+[a-z]~u" /* Capitalize after punctuation */,
            static function (array $matches): string {
                return mb_strtoupper($matches[0]);
            },
            $new_case
        ...
        $new_case = safe_preg_replace_callback(
            "~(?<!<)/[a-z]~u" /* Capitalize after slash unless part of ending html tag */,
            static function (array $matches): string {
                return mb_strtoupper($matches[0]);
            },
            $new_case
    ...
    $new_case = safe_preg_replace_callback(
        ...
        static function (array $matches): string {
            return "''" .
                mb_ucfirst(mb_strtolower($matches["taxon"])) .
                "'' " .
                mb_strtolower($matches["nova"]);
        },
        $new_case
    );
    ...
    if ($new_case === "Now and then") {
        $new_case = "Now and Then";
    } // Odd journal name

    // Trust existing "ITS", "its", ...
    $its_in = preg_match_all(
        "~ its(?= )~iu",
        " " . trim($in) . " ",
        $matches_in,
        PREG_OFFSET_CAPTURE
    ...
    if ($its_in === $its_out && $its_in !== 0 && $its_in !== false) {
        ...
        foreach ($matches_in as $key => $_value) {
            if (
                $matches_in[$key][0] !== $matches_out[$key][0] &&
                $matches_in[$key][1] === $matches_out[$key][1]
            ) {
                $new_case = substr_replace(
                    $new_case,
                    trim($matches_in[$key][0]),
                    $matches_out[$key][1],
                    3
                ); // PREG_OFFSET_CAPTURE is ALWAYS in BYTES, even for unicode
            }
        }
    }
    ...
    if (
        ...
        $new_case = trim(
            str_replace($replace_me, $replace, " " . $new_case . " ")
        );
    }
...
function throttle(): void
{
    ...
    if (preg_match('~^(\d+)M$~', $mem_max, $matches)) {
        ...
        bot_debug_log(
            "Memory Limit should end in M, but got: " . echoable($mem_max)
        ); // @codeCoverageIgnore
    }
    $phase = $phase + 1;
    if ($phase < $cycles) {
    ...
    } // Super paranoid, this would be a freeze point
...
function tidy_date(string $string): string
{
    ...
    if (!preg_match("~\d{2}~", $string)) {
        return "";
    } // Not two numbers next to each other
    if (preg_match('~^\d{2}\-\-$~', $string)) {
        return "";
    }
    // Google sends ranges
    if (
        ...
        if ($matches[1] === $matches[3]) {
            return date("j F", strtotime($matches[1] . $matches[2])) .
                " – " .
                date("j F Y", strtotime($matches[3] . $matches[4]));
        } else {
            return date("j F Y", strtotime($matches[1] . $matches[2])) .
                " – " .
                date("j F Y", strtotime($matches[3] . $matches[4]));
        }
    }
    ...
    if (strlen($string) !== mb_strlen($string)) {
        // Convert all multi-byte characters to dashes
        $cleaned = "";
    ...
    if (str_i_same("19xx", $string)) {
        return "";
    } //archive.org gives this if unknown
    ...
    } // Remove time zone stuff from standard date format
    if (preg_match('~^\-?\d+$~', $string)) {
        $string = intval($string);
        if ($string < -2000 || $string > (int) date("Y") + 10) {
            return "";
        } // A number that is not a year; probably garbage
    ...
    if (
        preg_match('~^(\d{1,2}) ([A-Za-z]+\.?), ?(\d{4})$~', $string, $matches)
    ) {
        // strtotime('3 October, 2016') gives 2019-10-03.  The comma is evil and strtotime is stupid
    ...
    if ($time) {
        ...
        if ($year < -2000 || $year > (int) date("Y") + 10) {
            return "";
        } // We got an invalid year
        if ($year < 100 && $year > -100) {
            return "";
        }
        if ($day === "01") {
            // Probably just got month and year
    ...
    if (preg_match("~^(\d{1,2}/\d{1,2}/\d{4})[^0-9]~", $string, $matches)) {
        return tidy_date($matches[1]);
    } // Recursion to clean up 3/27/2000
    ...
    if (
        ...
        preg_match("~^(\d\d?)\.(\d\d?)\.(\d{2}(?:\d{2})?)~", $string, $matches)
    ) {
    ...
    if (preg_match('~\s(\d{4})$~', $string, $matches)) {
        return $matches[1];
    } // Last ditch effort - ends in a year
    return ""; // And we give up
}

function not_bad_10_1093_doi(string $url): bool
{
    // We assume DOIs are bad, unless on good list
    if ($url === "") {
        return true;
    }
...
function remove_comments(string $string): string
{
    ...
    $string = preg_replace(
        "~# # # CITATION_BOT_PLACEHOLDER_COMMENT \d+ # # #~isu",
        "",
        $string
    );
...
function prior_parameters(string $par, array $list = []): array
{
    ...
    if (
        preg_match("~(\D+)(\d+)~", $par, $match) &&
        stripos($par, "s2cid") === false
    ) {
    ...
    switch ($par) {
        ...
        case "author-link":
            return prior_parameters(
                "dummy",
                array_merge(FLATTENED_AUTHOR_PARAMETERS, $list)
            );
        case "title-link":
        case "titlelink":
        ...
        case "doi":
            return prior_parameters(
                "location",
                array_merge(["publisher", "edition"], $list)
            );
        ...
        case "url":
            return prior_parameters(
                "lccn",
                array_merge(["issn", "ol", "oclc", "asin", "s2cid"], $list)
            );
        case "chapter-url":
        case "article-url":
        case "chapterurl":
        ...
        case "archivedate":
            return prior_parameters(
                "archive-url",
                array_merge(["archiveurl", "accessdate", "access-date"], $list)
            );
        case "id":
        case "type":
        case "via":
...
function check_doi_for_jstor(string $doi, Template $template): void
{
    ...
    if (preg_match('~^\d+$~', $doi)) {
        return;
    } // Just numbers - this WILL match a JSTOR, but who knows what it really is!
    ...
    if (
        ...
        stripos($ris, "A problem occurred trying to deliver RIS data") ===
            false &&
        substr_count($ris, "-") > 3
    ) {
        // It is actually a working JSTOR
...
function can_safely_modify_dashes(string $value): bool
{
    ...
    substr_count($value, "-") +
        substr_count($value, "–") +
        substr_count($value, ",") +
        substr_count($value, "dash") <
        3 && // This line helps us ignore with 1-5–1-6 stuff
...
function str_i_same(string $str1, string $str2): bool
{
    ...
    if (strcasecmp($str1, $str2) === 0) {
        return true;
    } // Quick non-multi-byte compare short cut
...
function edit_a_list_of_pages(
    ...
    if ($total > MAX_PAGES) {
        report_warning(
            "Number of links is huge (" .
                (string) $total .
                ")  Cancelling run (maximum size is " .
                (string) MAX_PAGES .
                ").  Listen to Obi-Wan Kenobi:  You want to go home and rethink your life."
        );
        bot_html_footer();
        return;
    }
    ...
    foreach ($pages_in_category as $page_title) {
        ...
        if ($page->get_text_from($page_title) && $page->expand_text()) {
            if (SAVETOFILES_MODE) {
                ...
                $filename =
                    preg_replace('~[\/\\:*?"<>|\s]~', "_", $page_title) . ".md";
...
function check_memory_usage(string $where): void
{
    ...
    if ($mem_used > 24) {
        bot_debug_log(
            "Memory Usage is up to " . (string) $mem_used . "MB in " . $where
        );
    }
    $mem_used = (int) (memory_get_peak_usage() / 1048576);
    if ($mem_used > 128) {
        bot_debug_log(
            "Peak memory Usage is up to " .
                (string) $mem_used .
                "MB in " .
                $where
        );
...
function safe_preg_replace_callback(
    string $regex,
    callable $replace,
    string $old
): string {
...
function numberToRomanRepresentation(int $number): string
{
...
function convert_to_utf8(string $value): string
{
    ...
    if (
        $encode1 === false ||
        $encode1 === "UTF-8" ||
        $encode1 === "Windows-1252"
    ) {
        return $value;
    }
    $encode2 = mb_detect_encoding(
        $value,
        [
            "UTF-8",
            "EUC-CN",
...
function is_encoding_reasonable(string $encode): bool
{
    // common "default" ones that are often wrong
...
function smart_decode(
    ...
    if (in_array($encode, ["utf-8-sig", "x-user-defined"], true)) {
        // Known wonky ones
        return "";
    }
    ...
    if ($try == "") {
        bot_debug_log(
            "Bad Encoding: " . $encode . " for " . echoable($archive_url)
        ); // @codeCoverageIgnore
    }
    return $try;
}
...
function normalize_google_books(
    string &$url,
    int &$removed_redundant,
    string &$removed_parts,
    array &$gid
): void {
    // PASS BY REFERENCE!!!!!!
    $removed_redundant = 0;
    ...
    if (preg_match("~(&q=[^&]+)&~", $hash, $matcher)) {
        ...
        if (isset($book_array["q"])) {
            ...
            if ($dum_dq !== $dum_q) {
                ...
                $book_array["dq"] = urlencode(
                    urldecode(substr($matcher[1], 3))
                );
            }
        } else {
            $book_array["q"] = urlencode(urldecode(substr($matcher[1], 3)));
    ...
    if (isset($book_array["vq"]) && isset($book_array["pg"])) {
        // VQ wins if and only if a page is set
        unset($book_array["q"]);
        unset($book_array["dq"]);
        $book_array["q"] = $book_array["vq"];
        unset($book_array["vq"]);
    ...
    if (isset($book_array["q"]) && isset($book_array["dq"])) {
        ...
        // Prefer Q parameters to DQ
        if (!isset($book_array["pg"]) && !isset($book_array["lpg"])) {
            // DQ requires that a page be set
    ...
    if (isset($book_array["q"])) {
        if (
            (stripos($book_array["q"], "isbn") === 0 &&
                $book_array["q"] !== "ISBN" &&
                $book_array["q"] !== "isbn") || // Sometimes the search is for the term isbn
    ...
    if (isset($book_array["dq"])) {
        if (
            (stripos($book_array["dq"], "isbn") === 0 &&
                $book_array["dq"] !== "ISBN" &&
                $book_array["dq"] !== "isbn") || // Sometimes the search is for the term isbn
    ...
    if (isset($book_array["lpg"])) {
        // Currently NOT POSSIBLE - failsafe code for changes
    ...
    if (
        preg_match(
            '~^(https://books\.google\.com/books\?id=[^#^&]+)(?:&printsec=frontcover|)(?:#v=onepage|v=snippet|)$~',
            $url,
            $matches
        )
    ) {
...
function doi_is_bad(string $doi): bool
{
    ...
    if ($doi === "10.5284/1000184") {
        return true;
    } // DOI for the entire database
    if ($doi === "10.1267/science.040579197") {
        return true;
    } // PMID test doi
    if ($doi === "10.2307/3511692") {
        return true;
    } // common review
    if ($doi === "10.1377/forefront") {
        return true;
    } // over-truncated
    if ($doi === "10.1126/science") {
        return true;
    } // over-truncated
    if (strpos($doi, "10.5779/hypothesis") === 0) {
        return true;
    } // SPAM took over
    if (strpos($doi, "10.5555/") === 0) {
        return true;
    } // Test DOI prefix
    if (strpos($doi, "10.5860/choice.") === 0) {
        return true;
    } // Paywalled book review
    if (strpos($doi, "10.1093/law:epil") === 0) {
        return true;
    } // Those do not work
    if (strpos($doi, "10.1093/oi/authority") === 0) {
        return true;
    } // Those do not work
    if (strpos($doi, "10.10520/") === 0 && !doi_works($doi)) {
        return true;
    } // Has doi in the URL, but is not a doi
    if (strpos($doi, "10.1967/") === 0 && !doi_works($doi)) {
        return true;
    } // Retired DOIs
    if (strpos($doi, "10.1043/0003-3219(") === 0 && !doi_works($doi)) {
        return true;
    } // Per-email.  The Angle Orthodontist will NEVER do these, since they have <> and [] in them
    if (strpos($doi, "10.3316/") === 0 && !doi_works($doi)) {
        return true;
    } // These do not work - https://search.informit.org/doi/10.3316/aeipt.207729 etc.
    if (strpos($doi, "10.1002/was.") === 0 && !doi_works($doi)) {
        return true;
    } // do's not doi's
    if (strpos($doi, "10.48550/arxiv") === 0) {
        return true;
    }
    return false;
}
...
function get_possible_dois(string $doi): array
{
    ...
    if (strpos($doi, "10.1093") === 0 && doi_works($doi) !== true) {
        if (
            preg_match(
                '~^10\.1093/(?:ref:|)odnb/9780198614128\.001\.0001/odnb\-9780198614128\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match('~^10\.1093/9780198614128.013.(\d+)$~', $doi, $matches)
        ) {
            $trial[] = "10.1093/odnb/9780198614128.013." . $matches[1];
        }
        if (
            preg_match(
                '~^10\.1093/anb/9780198606697\.001\.0001/anb\-9780198606697\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/benz/9780199773787\.001\.0001/acref-9780199773787\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/gao/9781884446054\.001\.0001/oao\-9781884446054\-e\-7000(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/gao/9781884446054\.001\.0001/oao\-9781884446054\-e\-700(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/acref/9780195301731\.001\.0001/acref\-9780195301731\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/ww/(9780199540891|9780199540884)\.001\.0001/ww\-9780199540884\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/gmo/9781561592630\.001\.0001/omo-9781561592630-e-00000(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/gmo/9781561592630\.001\.0001/omo-9781561592630-e-100(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/gmo/9781561592630\.001\.0001/omo-9781561592630-e-5000(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/gmo/9781561592630\.001\.0001/omo-9781561592630-e-400(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/gmo/9781561592630\.001\.0001/omo-9781561592630-e-2000(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/acrefore/9780199366439\.001\.0001/acrefore\-9780199366439\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/acrefore/9780190228613\.001\.0001/acrefore\-9780190228613\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/acrefore/9780199389414\.001\.0001/acrefore\-9780199389414\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/acrefore/9780199329175\.001\.0001/acrefore\-9780199329175\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/acrefore/9780190277734\.001\.0001/acrefore\-9780190277734\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/acrefore/9780190846626\.001\.0001/acrefore\-9780190846626\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/acrefore/9780190228620\.001\.0001/acrefore\-9780190228620\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/acrefore/9780199340378\.001\.0001/acrefore\-9780199340378\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/acrefore/9780190854584\.001\.0001/acrefore\-9780190854584\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/acrefore/9780199381135\.001\.0001/acrefore\-9780199381135\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/acrefore/9780190236557\.001\.0001/acrefore\-9780190236557\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/acrefore/9780190228637\.001\.0001/acrefore\-9780190228637\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/acrefore/9780190201098\.001\.0001/acrefore\-9780190201098\-e\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/oso/(\d{13})\.001\.0001/oso\-(\d{13})\-chapter\-(\d+)$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/med/9780199592548\.001\.0001/med\-9780199592548-chapter-(\d+)$~',
                $doi,
                $matches
            )
        ) {
            $trial[] =
                "10.1093/med/9780199592548.003." .
                str_pad($matches[1], 4, "0", STR_PAD_LEFT);
        }
        if (
            preg_match(
                '~^10\.1093/oso/(\d{13})\.001\.0001/oso\-(\d{13})$~',
                $doi,
                $matches
            )
        ) {
        ...
        if (
            preg_match(
                '~^10\.1093/oxfordhb/(\d{13})\.001\.0001/oxfordhb\-(\d{13})-e-(\d+)$~',
                $doi,
                $matches
            )
        ) {
            if ($matches[1] === $matches[2]) {
                $trial[] =
                    "10.1093/oxfordhb/" . $matches[1] . ".013." . $matches[3];
                $trial[] =
                    "10.1093/oxfordhb/" . $matches[1] . ".013.0" . $matches[3];
                $trial[] =
                    "10.1093/oxfordhb/" . $matches[1] . ".003." . $matches[3];
                $trial[] =
                    "10.1093/oxfordhb/" . $matches[1] . ".003.0" . $matches[3];
            }
        }
    }
    $replacements = ["&lt;" => "<", "&gt;" => ">"];
    ...
    while ($changed) {
        ...
        if ($pos = strrpos($try, "#")) {
            $extension = substr($try, $pos);
            if (
                strpos(strtolower($extension), "#page_scan_tab_contents") === 0
            ) {
                $try = substr($try, 0, $pos);
                $trial[] = $try;
                $changed = true;
        ...
        if (preg_match('~^(.+)v\d{1,2}$~', $try, $matches)) {
            // Versions
            $try = $matches[1];
            $trial[] = $try;
            $changed = true;
...
function clean_up_oxford_stuff(Template $template, string $param): void
{
    if (
        preg_match(
            '~^https?://(latinamericanhistory|classics|psychology|americanhistory|africanhistory|internationalstudies|climatescience|religion|environmentalscience|politics)\.oxfordre\.com(/.+)$~',
            $template->get($param),
            $matches
        )
    ) {
        $template->set(
            $param,
            "https://oxfordre.com/" . $matches[1] . $matches[2]
        );
    }
    ...
    while (
        preg_match(
            "~^(https?://www\.oxforddnb\.com/.+)(?:\?print|\?p=email|\;jsession|\?result=|\?rskey|\#|/version/\d+|\?backToResults)~",
            $template->get($param),
            $matches
        )
    ) {
    ...
    while (
        preg_match(
            "~^(https?://www\.anb\.org/.+)(?:\?print|\?p=email|\;jsession|\?result=|\?rskey|\#|/version/\d+|\?backToResults)~",
            $template->get($param),
            $matches
        )
    ) {
    ...
    while (
        preg_match(
            "~^(https?://www\.oxfordartonline\.com/.+)(?:\?print|\?p=email|\;jsession|\?result=|\?rskey|\#|/version/\d+|\?backToResults)~",
            $template->get($param),
            $matches
        )
    ) {
    ...
    while (
        preg_match(
            "~^(https?://www\.ukwhoswho\.com/.+)(?:\?print|\?p=email|\;jsession|\?result=|\?rskey|\#|/version/\d+|\?backToResults)~",
            $template->get($param),
            $matches
        )
    ) {
    ...
    while (
        preg_match(
            "~^(https?://www\.oxfordmusiconline\.com/.+)(?:\?print|\?p=email|\;jsession|\?result=|\?rskey|\#|/version/\d+|\?backToResults)~",
            $template->get($param),
            $matches
        )
    ) {
    ...
    while (
        preg_match(
            "~^(https?://oxfordre\.com/.+)(?:\?print|\?p=email|\;jsession|\?result=|\?rskey|\#|/version/\d+|\?backToResults)~",
            $template->get($param),
            $matches
        )
    ) {
    ...
    while (
        preg_match(
            "~^(https?://oxfordaasc\.com/.+)(?:\?print|\?p=email|\;jsession|\?result=|\?rskey|\#|/version/\d+|\?backToResults)~",
            $template->get($param),
            $matches
        )
    ) {
    ...
    while (
        preg_match(
            "~^(https?://oxford\.universitypressscholarship\.com/.+)(?:\?print|\?p=email|\;jsession|\?result=|\?rskey|\#|/version/\d+|\?backToResults)~",
            $template->get($param),
            $matches
        )
    ) {
    ...
    while (
        preg_match(
            "~^(https?://oxfordreference\.com/.+)(?:\?print|\?p=email|\;jsession|\?result=|\?rskey|\#|/version/\d+|\?backToResults)~",
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://www\.oxforddnb\.com/view/10\.1093/(?:ref:|)odnb/9780198614128\.001\.0001/odnb\-9780198614128\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
        ...
        if (
            ...
            preg_match(
                '~^(.+) # # # (?:CITATION_BOT_PLACEHOLDER_TEMPLATE|citation_bot_placeholder_template) \d+ # # # Oxford Dictionary of National Biography$~i',
                $the_title,
                $matches
            ) ||
            preg_match(
            ...
            preg_match(
                '~^(.+) &#\d+; Oxford Dictionary of National Biography$~',
                $the_title,
                $matches
            )
        ) {
    ...
    if (
        preg_match(
            '~^https?://www\.anb\.org/(?:view|abstract)/10\.1093/anb/9780198606697\.001\.0001/anb\-9780198606697\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://www\.oxfordartonline\.com/(?:benezit/|)(?:view|abstract)/10\.1093/benz/9780199773787\.001\.0001/acref-9780199773787\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://www\.oxfordartonline\.com/(?:groveart/|)(?:view|abstract)/10\.1093/gao/9781884446054\.001\.0001/oao\-9781884446054\-e\-7000(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://www\.oxfordartonline\.com/(?:groveart/|)(?:view|abstract)/10\.1093/gao/9781884446054\.001\.0001/oao\-9781884446054\-e\-700(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxfordaasc\.com/view/10\.1093/acref/9780195301731\.001\.0001/acref\-9780195301731\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://www\.ukwhoswho\.com/(?:view|abstract)/10\.1093/ww/(9780199540891|9780199540884)\.001\.0001/ww\-9780199540884\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://www\.oxfordmusiconline\.com/(?:grovemusic/|)(?:view|abstract)/10\.1093/gmo/9781561592630\.001\.0001/omo-9781561592630-e-00000(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://www\.oxfordmusiconline\.com/(?:grovemusic/|)(?:view|abstract)/10\.1093/gmo/9781561592630\.001\.0001/omo-9781561592630-e-100(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://www\.oxfordmusiconline\.com/(?:grovemusic/|)(?:view|abstract)/10\.1093/gmo/9781561592630\.001\.0001/omo-9781561592630-e-5000(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://www\.oxfordmusiconline\.com/(?:grovemusic/|)(?:view|abstract)/10\.1093/gmo/9781561592630\.001\.0001/omo-9781561592630-e-400(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://www\.oxfordmusiconline\.com/(?:grovemusic/|)(?:view|abstract)/10\.1093/gmo/9781561592630\.001\.0001/omo-9781561592630-e-2000(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxfordre\.com/(?:|latinamericanhistory/)(?:view|abstract)/10\.1093/acrefore/9780199366439\.001\.0001/acrefore\-9780199366439\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxfordre\.com/(?:|communication/)(?:view|abstract)/10\.1093/acrefore/9780190228613\.001\.0001/acrefore\-9780190228613\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxfordre\.com/(?:|environmentalscience/)(?:view|abstract)/10\.1093/acrefore/9780199389414\.001\.0001/acrefore\-9780199389414\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxfordre\.com/(?:|americanhistory/)(?:view|abstract)/10\.1093/acrefore/9780199329175\.001\.0001/acrefore\-9780199329175\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxfordre\.com/(?:|africanhistory/)(?:view|abstract)/10\.1093/acrefore/9780190277734\.001\.0001/acrefore\-9780190277734\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxfordre\.com/(?:|internationalstudies/)(?:view|abstract)/10\.1093/acrefore/9780190846626\.001\.0001/acrefore\-9780190846626\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxfordre\.com/(?:|climatescience/)(?:view|abstract)/10\.1093/acrefore/9780190228620\.001\.0001/acrefore\-9780190228620\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxfordre\.com/(?:|religion/)(?:view|abstract)/10\.1093/acrefore/9780199340378\.001\.0001/acrefore\-9780199340378\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxfordre\.com/(?:|anthropology/)(?:view|abstract)/10\.1093/acrefore/9780190854584\.001\.0001/acrefore\-9780190854584\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxfordre\.com/(?:|classics/)(?:view|abstract)/10\.1093/acrefore/9780199381135\.001\.0001/acrefore\-9780199381135\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxfordre\.com/(?:|psychology/)(?:view|abstract)/10\.1093/acrefore/9780190236557\.001\.0001/acrefore\-9780190236557\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxfordre\.com/(?:|politics/)(?:view|abstract)/10\.1093/acrefore/9780190228637\.001\.0001/acrefore\-9780190228637\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxfordre\.com/(?:|literature/)(?:view|abstract)/10\.1093/acrefore/9780190201098\.001\.0001/acrefore\-9780190201098\-e\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxford\.universitypressscholarship\.com/(?:view|abstract)/10\.1093/(oso|acprof:oso)/(\d{13})\.001\.0001/oso\-(\d{13})\-chapter\-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
        if ($matches[2] === $matches[3]) {
            ...
            $new_doi =
                ...
                $matches[2] .
                ".003." .
                str_pad($matches[4], 4, "0", STR_PAD_LEFT);
            if (doi_works($new_doi)) {
                if (
                    $template->has("doi") &&
                    $template->has("doi-broken-date")
                ) {
                    $template->set("doi", "");
                    $template->forget("doi-broken-date");
                    $template->add_if_new("doi", $new_doi);
    ...
    if (
        preg_match(
            '~^https?://(?:www\.|)oxfordmedicine\.com/(?:view|abstract)/10\.1093/med/9780199592548\.001\.0001/med\-9780199592548-chapter-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://oxford\.universitypressscholarship\.com/(?:view|abstract)/10\.1093/oso/(\d{13})\.001\.0001/oso\-(\d{13})$~',
            $template->get($param),
            $matches
        )
    ) {
    ...
    if (
        preg_match(
            '~^https?://(?:www\.|)oxfordhandbooks\.com/(?:view|abstract)/10\.1093/oxfordhb/(\d{13})\.001\.0001/oxfordhb\-(\d{13})-e-(\d+)$~',
            $template->get($param),
            $matches
        )
    ) {
...
function conference_doi(string $doi): bool
{
    if (stripos($doi, "10.1007/978-3-662-44777") === 0) {
        return false;
    } // Manual override of stuff
    if (
        strpos($doi, "10.1109/") === 0 ||
...
function clean_dates(string $input): string
{
    // See https://en.wikipedia.org/wiki/Help:CS1_errors#bad_date
    if ($input === "0001-11-30") {
        return "";
    }
    $days_of_week = [
    ...
    if (preg_match('~^(\d{4})[\-\/](\d{4})$~', $input, $matches)) {
        // Hyphen or slash in year range (use en dash)
        return $matches[1] . "–" . $matches[2];
    }
    if (preg_match('~^(\d{4})\/ed$~i', $input, $matches)) {
        // 2002/ed
        return $matches[1];
    }
    if (
        preg_match(
            '~^First published(?: |\: | in | in\: | in\:)(\d{4})$~i',
            $input,
            $matches
        )
    ) {
    ...
    if (
        ...
        // Slash or hyphen in date range (use en dash)
        return $matches[1] . "–" . $matches[2] . " " . $matches[3];
    }
    if (
        preg_match(
            '~^([A-Z][a-z]+ \d{4})[\-\–]([A-Z][a-z]+ \d{4})$~',
            $input,
            $matches
        )
    ) {
    ...
    if (preg_match('~^([A-Z][a-z]+), (\d{4})$~', $input, $matches)) {
        // Comma with month/season and year
        return $matches[1] . " " . $matches[2];
    }
    if (
        preg_match('~^([A-Z][a-z]+), (\d{4})[\-\–](\d{4})$~', $input, $matches)
    ) {
        // Comma with month/season and years
        return $matches[1] . " " . $matches[2] . "–" . $matches[3];
    }
    if (preg_match('~^([A-Z][a-z]+) 0(\d),? (\d{4})$~', $input, $matches)) {
        // Zero-padding
        return $matches[1] . " " . $matches[2] . ", " . $matches[3];
    }
    if (preg_match('~^([A-Z][a-z]+ \d{1,2})( \d{4})$~', $input, $matches)) {
        // Missing comma in format which requires it
        return $matches[1] . "," . $matches[2];
    }
    if (
        preg_match(
            '~^Collected[\s\:]+((?:|[A-Z][a-z]+ )\d{4})$~',
            $input,
            $matches
        )
    ) {
        // Collected 1999 stuff
        return $matches[1];
    }
    if (
        preg_match(
            '~^Effective[\s\:]+((?:|[A-Z][a-z]+ )\d{4})$~',
            $input,
            $matches
        )
    ) {
        // Effective 1999 stuff
        return $matches[1];
    }
    if (preg_match('~^(\d+ [A-Z][a-z]+ \d{4})\.$~', $input, $matches)) {
        // 8 December 2022. (period on end)
        return $matches[1];
    }
    if (preg_match('~^0(\d [A-Z][a-z]+ \d{4})$~', $input, $matches)) {
        // 08 December 2022 - leading zero
        return $matches[1];
    }
    if (
        preg_match(
    ...
    if (preg_match('~^(\d{4})\s*(?:&|and)\s*(\d{4})$~', $input, $matches)) {
        // &/and between years
        $first = (int) $matches[1];
...
function simplify_google_search(string $url): string
{
    ...
    if (preg_match("~^https?://.*google.com/search/~", $url)) {
        return $url;
    } // Not a search if the slash is there
    $orig_url = $url;
    $hash = "";
    if (strpos($url, "#")) {
        $url_parts = explode("#", $url, 2);
    ...
    foreach ($url_parts as $part) {
        ...
        switch ($part_start0) {
            ...
            default:
                // @codeCoverageIgnoreStart
                report_minor_error(
                    "Unexpected Google URL component:  " .
                        echoable($part) .
                        " in " .
                        echoable($orig_url)
                );
...
function addISBNdashes(string $isbn): string
{
    ...
    if (strlen($new) === 10) {
        ...
        $v = $split;
        return substr($new, 0, $v[0]) .
            "-" .
            substr($new, $v[0], $v[1]) .
            "-" .
            substr($new, $v[0] + $v[1], $v[2]) .
            "-" .
            substr($new, $v[0] + $v[1] + $v[2], 1);
        ...
        $v = $split;
        return substr($new, 0, 3) .
            "-" .
            substr($new, 3, $v[0]) .
            "-" .
            substr($new, 3 + $v[0], $v[1]) .
            "-" .
            substr($new, 3 + $v[0] + $v[1], $v[2]) .
            "-" .
            substr($new, 3 + $v[0] + $v[1] + $v[2], 1);
