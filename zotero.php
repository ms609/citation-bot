...
function expand_by_zotero(&$template, $url = NULL) {
  ...
  while (isset($result->author[$i])) {
      if (author_is_human(@$result->author[$i][1] . @$result->author[$i][0])) $template->validate_and_add('author' . ($i+1), @$result->author[$i][1], @$result->author[$i][0],
    ...
    if (in_array($result->itemType, ['journalArticle', 'newspaperArticle', 'report'])) {
      ...
      while (isset($result->creators[$i])) {
        ...
        if (isset($result->creators[$i]->firstName) && isset($result->creators[$i]->lastName)) {
         ...
         if (author_is_human($result->creators[$i]->lastName . $result->creators[$i]->firstName) $template->validate_and_add($authorParam, $result->creators[$i]->lastName, $result->creators[$i]->firstName,
                                      isset($result->rights) ? $result->rights : '', FALSE);
        }
        $i++;
      }
